...
fn createLoadedMap(runtime: *mapvina.RuntimeHandle) !mapvina.MapHandle {
...
test "style source JSON descriptors expose type info and copied attribution" {
    ...
    const empty_features = [_]mapvina.JsonValue{};
    const data_members = [_]mapvina.JsonMember{
        .{ .key = "type", .value = .{ .string = "FeatureCollection" } },
        .{ .key = "features", .value = .{ .array = empty_features[0..] } },
    };
    const source_members = [_]mapvina.JsonMember{
    ...
    try testing.expectEqual(mapvina.StyleSourceType.geojson, (try map.getStyleSourceType(testing.allocator, "empty-json")).?);
    const info = (try map.getStyleSourceInfo(testing.allocator, "empty-json")).?;
    try testing.expectEqual(mapvina.StyleSourceType.geojson, info.source_type);
    try testing.expectEqual(@as(usize, "empty-json".len), info.id_size);
    try testing.expect(!info.has_attribution);

    const tile_values = [_]mapvina.JsonValue{.{ .string = "https://example.com/{z}/{x}/{y}.pbf" }};
    const vector_members = [_]mapvina.JsonMember{
    ...
    try testing.expectEqual(mapvina.StyleSourceType.vector, vector_info.source_type);
...
test "style source removal reports state and copies missing results" {
    ...
    const empty_features = [_]mapvina.Feature{};
...
test "tile source helpers add vector raster and raster DEM sources" {
    ...
    try testing.expectEqual(mapvina.StyleSourceType.vector, (try map.getStyleSourceType(testing.allocator, "vector-helper")).?);
    ...
    try testing.expectEqual(mapvina.StyleSourceType.vector, (try map.getStyleSourceType(testing.allocator, "vector-url-helper")).?);
    ...
    try testing.expectEqual(mapvina.StyleSourceType.raster, (try map.getStyleSourceType(testing.allocator, "raster-helper")).?);
    ...
    try testing.expectEqual(mapvina.StyleSourceType.raster_dem, (try map.getStyleSourceType(testing.allocator, "dem")).?);
    ...
    const linear_values = [_]mapvina.JsonValue{.{ .string = "linear" }};
    const linear = mapvina.JsonValue{ .array = linear_values[0..] };
    const elevation_values = [_]mapvina.JsonValue{.{ .string = "elevation" }};
    const elevation = mapvina.JsonValue{ .array = elevation_values[0..] };
    const color_ramp_values = [_]mapvina.JsonValue{
    ...
    const zoom_values = [_]mapvina.JsonValue{.{ .string = "zoom" }};
    const zoom = mapvina.JsonValue{ .array = zoom_values[0..] };
    const invalid_color_ramp_values = [_]mapvina.JsonValue{
...
test "image source helpers add update and copy coordinates" {
    ...
    const coordinates = [4]mapvina.LatLng{
    ...
    try testing.expectEqual(mapvina.StyleSourceType.image, (try map.getStyleSourceType(testing.allocator, "image-url-source")).?);
    ...
    const updated_coordinates = [4]mapvina.LatLng{
...
test "style source JSON descriptors reject invalid source data and pass explicit-length IDs" {
    ...
    const source_members = [_]mapvina.JsonMember{.{ .key = "type", .value = .{ .string = "definitely-not-a-source-type" } }};
    try testing.expectError(
        error.InvalidArgument,
    ...
    const valid_source_members = [_]mapvina.JsonMember{
        .{ .key = "type", .value = .{ .string = "geojson" } },
        .{ .key = "data", .value = .{ .object = data_members[0..] } },
    };
...
const CustomGeometryState = struct {
    ...
    last_tile: mapvina.CanonicalTileId = .{ .z = 0, .x = 0, .y = 0 },
};

fn fetchCustomGeometryTile(context: ?*anyopaque, tile_id: mapvina.CanonicalTileId) void {
    const state: *CustomGeometryState = @ptrCast(@alignCast(context.?));
    state.fetch_count += 1;
    state.last_tile = tile_id;
}

fn cancelCustomGeometryTile(context: ?*anyopaque, tile_id: mapvina.CanonicalTileId) void {
    const state: *CustomGeometryState = @ptrCast(@alignCast(context.?));
...
test "custom geometry source helpers add sources and accept tile updates" {
    ...
    try testing.expectEqual(mapvina.StyleSourceType.custom_vector, (try map.getStyleSourceType(testing.allocator, "custom")).?);

    const tile_id = mapvina.CanonicalTileId{ .z = 0, .x = 0, .y = 0 };
    const empty_features = [_]mapvina.Feature{};
