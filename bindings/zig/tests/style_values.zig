...
fn expectListContains(list: mapvina.StringList, expected: []const u8) !void {
    for (list.items) |item| {
...
fn listIndexOf(list: mapvina.StringList, expected: []const u8) !usize {
...
fn expectObjectString(value: mapvina.OwnedJsonValue, key: []const u8, expected: []const u8) !void {
...
test "style layer JSON helpers manage lifecycle and order" {
    ...
    const layer_members = [_]mapvina.JsonMember{
...
test "layer filters accept nested semantic JSON arrays and return owned snapshots" {
    ...
    const get_values = [_]mapvina.JsonValue{ .{ .string = "get" }, .{ .string = "visible" } };
    const get_expr = mapvina.JsonValue{ .array = get_values[0..] };
    const filter_values = [_]mapvina.JsonValue{ .{ .string = "==" }, get_expr, .{ .bool = true } };
    const filter = mapvina.JsonValue{ .array = filter_values[0..] };
...
test "style light accepts full JSON and property updates" {
    ...
    const position_values = [_]mapvina.JsonValue{ .{ .double = 1.0 }, .{ .double = 2.0 }, .{ .double = 3.0 } };
    const light_members = [_]mapvina.JsonMember{
