...
fn waitForEvent(runtime: *mapvina.RuntimeHandle, event_type: mapvina.RuntimeEventType) !bool {
    for (0..1000) |_| {
        try runtime.runOnce();
...
fn waitForOwnedEvent(
    runtime: *mapvina.RuntimeHandle,
    event_type: mapvina.RuntimeEventType,
) !mapvina.OwnedRuntimeEvent {
    for (0..5000) |_| {
        try runtime.runOnce();
        while (try runtime.pollEventOwned(testing.allocator)) |event| {
            var owned_event = event;
...
fn waitForStyleLoaded(runtime: *mapvina.RuntimeHandle) !void {
...
test "network status APIs wrap process-global MapVina status" {
    const original_status = try mapvina.getNetworkStatus(null);
    defer mapvina.setNetworkStatus(original_status, null) catch @panic("network status restore failed");

    try mapvina.setNetworkStatus(.offline, null);
    try testing.expect(std.meta.eql(try mapvina.getNetworkStatus(null), mapvina.NetworkStatus.offline));

    try mapvina.setNetworkStatus(.online, null);
    try testing.expect(std.meta.eql(try mapvina.getNetworkStatus(null), mapvina.NetworkStatus.online));

    var diagnostics = mapvina.DiagnosticStore.init(testing.allocator);
    defer diagnostics.deinit();
    try testing.expectError(error.InvalidArgument, mapvina.setNetworkStatus(.{ .unknown = 999 }, &diagnostics));
...
test "ambient cache operations validate cache configuration" {
    ...
    var cached_runtime = try mapvina.RuntimeHandle.create(testing.allocator, .{ .cache_path = cache_path }, null);
...
test "asset URL style loads through public binding runtime asset path" {
    ...
    var runtime = try mapvina.RuntimeHandle.create(testing.allocator, .{ .asset_path = fixture.dir_path }, null);
    defer runtime.close() catch @panic("runtime close failed");
...
fn rewriteStyleUrl(context: ?*anyopaque, request: mapvina.ResourceTransformRequest) mapvina.ResourceTransformResponse {
...
test "resource transform can be cleared after map creation" {
    try mapvina.setNetworkStatus(.online, null);
    defer mapvina.setNetworkStatus(.online, null) catch @panic("network status restore failed");
...
fn passThroughStyleProvider(
    context: ?*anyopaque,
    request: mapvina.ResourceRequest,
    maybe_handle: ?mapvina.ResourceRequestHandle,
) mapvina.ResourceProviderDecision {
    const state: *PassThroughProviderState = @ptrCast(@alignCast(context.?));
    _ = state.calls.fetchAdd(1, .seq_cst);
    if (std.meta.eql(request.kind, mapvina.ResourceKind.style)) state.saw_style.store(true, .seq_cst);
    _ = maybe_handle;
    return .pass_through;
}
...
test "http style can load from ambient cache after online load" {
    ...
    {
        var runtime = try mapvina.RuntimeHandle.create(testing.allocator, .{ .cache_path = cache_path, .maximum_cache_size = 1024 * 1024 }, null);
        defer runtime.close() catch @panic("runtime close failed");
        var map = try mapvina.MapHandle.create(&runtime, .{});
        defer map.close() catch @panic("map close failed");
        try map.setStyleUrl(testing.allocator, style_url);
    ...
    var cached_runtime = try mapvina.RuntimeHandle.create(testing.allocator, .{ .cache_path = cache_path }, null);
    defer cached_runtime.close() catch @panic("cached runtime close failed");
    var cached_map = try mapvina.MapHandle.create(&cached_runtime, .{});
...
fn customStyleProvider(
    ...
    state.saw_style.store(std.meta.eql(request.kind, mapvina.ResourceKind.style), .seq_cst);
    state.saw_all_loading.store(std.meta.eql(request.loading_method, mapvina.ResourceLoadingMethod.all), .seq_cst);
    state.saw_regular_priority.store(std.meta.eql(request.priority, mapvina.ResourcePriority.regular), .seq_cst);
    state.saw_online_usage.store(std.meta.eql(request.usage, mapvina.ResourceUsage.online), .seq_cst);
    state.saw_permanent_storage.store(std.meta.eql(request.storage_policy, mapvina.ResourceStoragePolicy.permanent), .seq_cst);
...
const PmtilesRangeProviderState = struct {
    ...
    fn markStyle(self: *PmtilesRangeProviderState, request: mapvina.ResourceRequest) void {
        self.saw_style_absent_range.store(request.range == null, .seq_cst);
    }

    fn markPmtilesRequest(self: *PmtilesRangeProviderState, request: mapvina.ResourceRequest) void {
        self.saw_source_kind.store(std.meta.eql(request.kind, mapvina.ResourceKind.source), .seq_cst);
        self.saw_network_only_loading.store(std.meta.eql(request.loading_method, mapvina.ResourceLoadingMethod.network_only), .seq_cst);
...
fn waitForPmtilesRangeRequest(runtime: *mapvina.RuntimeHandle, state: *PmtilesRangeProviderState) !void {
    for (0..1000) |_| {
        try runtime.runOnce();
...
fn offlineTileDefinition() mapvina.OfflineRegionDefinition {
    return .{ .tile_pyramid = .{
...
fn expectOfflineTileRegion(region: *const mapvina.OwnedOfflineRegion, expected_metadata: []const u8) !void {
...
test "offline tile-pyramid regions copy definitions and metadata" {
    ...
    var region_id: mapvina.OfflineRegionId = 0;

    {
        var runtime = try mapvina.RuntimeHandle.create(testing.allocator, .{ .cache_path = cache_path }, null);
        defer runtime.close() catch @panic("runtime close failed");
        ...
        try testing.expect(std.meta.eql(status.download_state, mapvina.OfflineRegionDownloadState.inactive));
...
test "offline region definitions reject invalid public values" {
    ...
    var nested_geometries: [66]mapvina.Geometry = undefined;
...
fn expectOfflineGeometryRegion(region: *const mapvina.OwnedOfflineRegion, expected_metadata: []const u8) !void {
    try testing.expect(region.id > 0);
...
test "offline database merge returns copied region list" {
    ...
    {
        var side_runtime = try mapvina.RuntimeHandle.create(testing.allocator, .{ .cache_path = side_cache_path }, null);
    ...
    var main_runtime = try mapvina.RuntimeHandle.create(testing.allocator, .{ .cache_path = main_cache_path }, null);
...
test "offline geometry regions expose copied geometry values" {
    ...
    const coordinates = [_]mapvina.LatLng{
...
const AsyncProviderState = struct {
    ...
    handle: ?mapvina.ResourceRequestHandle = null,
    saw_style: std.atomic.Value(bool) = std.atomic.Value(bool).init(false),
    saw_all_loading: std.atomic.Value(bool) = std.atomic.Value(bool).init(false),
    saw_regular_priority: std.atomic.Value(bool) = std.atomic.Value(bool).init(false),
    saw_online_usage: std.atomic.Value(bool) = std.atomic.Value(bool).init(false),
    ...
    fn store(self: *AsyncProviderState, request: mapvina.ResourceRequest, handle: mapvina.ResourceRequestHandle) void {
        self.saw_style.store(std.meta.eql(request.kind, mapvina.ResourceKind.style), .seq_cst);
        self.saw_all_loading.store(std.meta.eql(request.loading_method, mapvina.ResourceLoadingMethod.all), .seq_cst);
        self.saw_regular_priority.store(std.meta.eql(request.priority, mapvina.ResourcePriority.regular), .seq_cst);
        self.saw_online_usage.store(std.meta.eql(request.usage, mapvina.ResourceUsage.online), .seq_cst);
        self.saw_permanent_storage.store(std.meta.eql(request.storage_policy, mapvina.ResourceStoragePolicy.permanent), .seq_cst);
    ...
    fn takeHandle(self: *AsyncProviderState) ?mapvina.ResourceRequestHandle {
        self.lockHandle();
        defer self.unlockHandle();
...
fn waitForProviderHandle(runtime: *mapvina.RuntimeHandle, state: *AsyncProviderState) !mapvina.ResourceRequestHandle {
    for (0..1000) |_| {
        try runtime.runOnce();
...
fn completeStyleOnThread(handle: mapvina.ResourceRequestHandle, out_error: *?anyerror) void {
    handle.complete(.{ .bytes = support.style_json }) catch |err| {
        out_error.* = err;
        return;
    };
...
test "offline region download errors are runtime events" {
    ...
    defer event.deinit();
    try testing.expect(std.meta.eql(event.payload_type, mapvina.RuntimeEventPayloadType.offline_region_response_error));
    ...
    try testing.expect(std.meta.eql(payload.reason, mapvina.ResourceErrorReason.not_found));
    try testing.expect(event.message.len > 0);
}

fn waitForRequestCancellation(runtime: *mapvina.RuntimeHandle, handle: mapvina.ResourceRequestHandle) !void {
    for (0..5000) |_| {
...
test "resource provider observes cancellation before late completion" {
    ...
    var runtime = try mapvina.RuntimeHandle.init(&diagnostics);
    defer runtime.close() catch @panic("runtime close failed");
...
test "offline region download control emits copied status events" {
    ...
    for (0..5000) |_| {
        ...
        while (try runtime.pollEventOwned(testing.allocator)) |event| {
            var owned_event = event;
            defer owned_event.deinit();
            if (!std.meta.eql(owned_event.event_type, mapvina.RuntimeEventType.offline_region_status_changed)) continue;
            const payload = owned_event.payload.offline_region_status;
            try testing.expectEqual(region_id, payload.region_id);
            try testing.expect(
                std.meta.eql(payload.status.download_state, mapvina.OfflineRegionDownloadState.active) or
                    std.meta.eql(payload.status.download_state, mapvina.OfflineRegionDownloadState.inactive),
            );
            observed = true;
            break;
        }
