...
fn runRuntimeOnThread(runtime: *mapvina.RuntimeHandle, out_error: *?anyerror) void {
...
fn pollRuntimeOnThread(runtime: *mapvina.RuntimeHandle, out_error: *?anyerror) void {
...
fn closeRuntimeOnThread(runtime: *mapvina.RuntimeHandle, out_error: *?anyerror) void {
...
fn createRuntimeOnThread(out_error: *?anyerror) void {
    var runtime = mapvina.RuntimeHandle.init(null) catch |err| {
        out_error.* = err;
        return;
    };
    runtime.close() catch |err| {
...
test "runtime rejects second runtime on same owner and permits distinct owner" {
    ...
    try testing.expectError(error.InvalidState, mapvina.RuntimeHandle.init(null));
...
test "runtime option strings reject embedded NUL before C calls" {
    ...
    try testing.expectError(
        error.InvalidString,
        mapvina.RuntimeHandle.create(testing.allocator, .{ .asset_path = "asset\x00path" }, &diagnostics),
    );
}
...
test "owned runtime events copy message and resolve map identity" {
    ...
    var found: ?mapvina.OwnedRuntimeEvent = null;
    for (0..1000) |_| {
        try runtime.runOnce();
        while (try runtime.pollEventOwned(testing.allocator)) |event| {
            if (std.meta.eql(event.event_type, mapvina.RuntimeEventType.map_loading_failed)) {
                found = event;
                break;
            }
            var discard = event;
    ...
    try testing.expect(std.meta.eql(event.payload, mapvina.RuntimeEventPayload.none));
...
test "runtime event polling reports empty queues" {
    ...
    try testing.expectEqual(@as(?mapvina.RuntimeEvent, null), try runtime.pollEvent());
    try testing.expectEqual(@as(?mapvina.OwnedRuntimeEvent, null), try runtime.pollEventOwned(testing.allocator));
}
