...
const center = mapvina.LatLng{ .latitude = 37.7749, .longitude = -122.4194 };

fn expectCenterPoint(point: mapvina.ScreenPoint) !void {
    try testing.expectApproxEqAbs(@as(f64, 256.0), point.x, 0.001);
    try testing.expectApproxEqAbs(@as(f64, 256.0), point.y, 0.001);
}

fn expectLatLngApprox(expected: mapvina.LatLng, actual: mapvina.LatLng) !void {
...
test "map converts between lat lngs and screen points" {
    ...
    var points: [coordinates.len]mapvina.ScreenPoint = undefined;
    try map.pixelsForLatLngs(testing.allocator, coordinates[0..], points[0..]);
    try expectCenterPoint(points[0]);

    var roundtrip: [points.len]mapvina.LatLng = undefined;
...
test "standalone projection converts and updates camera" {
    ...
    const helper_camera = mapvina.CameraOptions{ .center = .{ .latitude = 0.0, .longitude = 0.0 }, .zoom = 3.0 };
...
test "projected meters convert to and from lat lng" {
    const origin = mapvina.LatLng{ .latitude = 0.0, .longitude = 0.0 };
    const origin_meters = try mapvina.projectedMetersForLatLng(origin, null);
    ...
    const meters = try mapvina.projectedMetersForLatLng(center, null);
    const roundtrip = try mapvina.latLngForProjectedMeters(meters, null);
...
test "projection free helpers preserve native diagnostics" {
    ...
    try testing.expectError(
        ...
        mapvina.projectedMetersForLatLng(.{ .latitude = std.math.inf(f64), .longitude = 0.0 }, &diagnostics),
    );
    ...
    try testing.expectError(error.InvalidArgument, mapvina.latLngForProjectedMeters(.{ .northing = std.math.nan(f64), .easting = 0.0 }, null));
}
