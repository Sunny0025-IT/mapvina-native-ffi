...
fn nativePointer(ptr: *anyopaque) mapvina.NativePointer {
    return .{ .ptr = ptr };
}
...
test "Metal surface renders to window-attached layer through public binding" {
    ...
    var surface = try mapvina.attachMetalSurface(&map, .{
...
test "surface public descriptors report invalid native arguments" {
    ...
    if (build_options.supports_metal) {
        ...
        try testing.expectError(error.InvalidArgument, mapvina.attachMetalSurface(&map, .{ .extent = .{ .width = 0 }, .layer = nativePointer(layer) }));
        try testing.expectError(error.InvalidArgument, mapvina.attachMetalSurface(&map, .{ .extent = .{ .height = 0 }, .layer = nativePointer(layer) }));
        try testing.expectError(error.InvalidArgument, mapvina.attachMetalSurface(&map, .{ .extent = .{ .scale_factor = 0 }, .layer = nativePointer(layer) }));
    } else if (build_options.supports_vulkan) {
        const fake = nativePointer(@ptrFromInt(1));
        const descriptor = mapvina.VulkanSurfaceDescriptor{
            .context = .{
                .instance = fake,
        ...
        try testing.expectError(error.InvalidArgument, mapvina.attachVulkanSurface(&map, .{ .extent = .{ .width = 0 }, .context = descriptor.context, .surface = descriptor.surface }));
        try testing.expectError(error.InvalidArgument, mapvina.attachVulkanSurface(&map, .{ .extent = .{ .height = 0 }, .context = descriptor.context, .surface = descriptor.surface }));
        try testing.expectError(error.InvalidArgument, mapvina.attachVulkanSurface(&map, .{ .extent = .{ .scale_factor = 0 }, .context = descriptor.context, .surface = descriptor.surface }));
    }
}
...
test "unsupported public surface backends report unsupported" {
    ...
    if (build_options.supports_metal) {
        const fake = nativePointer(@ptrFromInt(1));
        try testing.expectError(error.Unsupported, mapvina.attachVulkanSurface(&map, .{
            .context = .{
                .instance = fake,
                .physical_device = fake,
                .device = fake,
        ...
        try testing.expectError(error.Unsupported, mapvina.attachMetalSurface(&map, .{ .layer = nativePointer(@ptrFromInt(1)) }));
    }
}
