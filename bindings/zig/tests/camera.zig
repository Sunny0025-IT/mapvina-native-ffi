...
test "camera commands accept valid public descriptors" {
    ...
    const anchor = mapvina.ScreenPoint{ .x = 256, .y = 256 };
    const rotate_start = mapvina.ScreenPoint{ .x = 200, .y = 200 };
    const rotate_end = mapvina.ScreenPoint{ .x = 220, .y = 210 };
    const animation = mapvina.AnimationOptions{
        .duration_ms = 0,
...
test "camera fitting computes camera and visible bounds" {
    ...
    const bounds = mapvina.LatLngBounds{
    ...
    const coordinates = [_]mapvina.LatLng{ bounds.southwest, bounds.northeast };
...
test "camera constraints and free camera options round-trip public values" {
    ...
    const constraints = mapvina.BoundOptions{
        .bounds = .{
...
test "camera public descriptors report invalid native arguments" {
    ...
    const inverted_bounds = mapvina.LatLngBounds{
