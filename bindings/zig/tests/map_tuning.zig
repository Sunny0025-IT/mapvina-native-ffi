...
test "map debug options round trip and diagnostics toggles" {
    ...
    const debug = mapvina.DebugOptions{
        .tile_borders = true,
        .collision = true,
        .depth_buffer = true,
    };
...
test "map viewport options update selected fields through public descriptors" {
    ...
    try testing.expectEqual(mapvina.NorthOrientation.right, snapshot.north_orientation.?);
    try testing.expectEqual(mapvina.ConstrainMode.width_and_height, snapshot.constrain_mode.?);
    try testing.expectEqual(mapvina.ViewportMode.flipped_y, snapshot.viewport_mode.?);
    ...
    try testing.expectEqual(mapvina.NorthOrientation.down, snapshot.north_orientation.?);
    try testing.expectEqual(mapvina.ConstrainMode.width_and_height, snapshot.constrain_mode.?);
}
...
test "map tile options update selected fields through public descriptors" {
    ...
    try testing.expectEqual(mapvina.TileLodMode.distance, snapshot.lod_mode.?);
