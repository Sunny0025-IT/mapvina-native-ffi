...
test "supported render backend is exposed semantically" {
    const support_mask = mapvina.supportedRenderBackends();
...
fn expectFeaturePropertyString(feature: *const mapvina.QueriedFeature, key: []const u8, expected: []const u8) !void {
...
fn waitForRenderedFeatureQuery(
    ...
    session: *mapvina.RenderSessionHandle,
    geometry: mapvina.RenderedQueryGeometry,
    options: mapvina.RenderedFeatureQueryOptions,
) !mapvina.FeatureQueryResult {
    for (0..1000) |_| {
...
fn ownedGeometryAsBorrowed(geometry: mapvina.OwnedGeometry) mapvina.Geometry {
...
fn ownedJsonAsBorrowed(value: mapvina.OwnedJsonValue) mapvina.JsonValue {
    return switch (value) {
        .null => .null,
        .bool => |item| .{ .bool = item },
        .uint => |item| .{ .uint = item },
...
fn queriedFeatureAsBorrowed(allocator: std.mem.Allocator, queried: *const mapvina.QueriedFeature) !struct { feature: mapvina.Feature, properties: []mapvina.JsonMember } {
    const properties = try allocator.alloc(mapvina.JsonMember, queried.feature.properties.len);
...
fn callRenderSessionOnThread(session: *mapvina.RenderSessionHandle, call: RenderSessionThreadCall, out_error: *?anyerror) void {
...
fn expectRenderSessionCallWrongThread(session: *mapvina.RenderSessionHandle, call: RenderSessionThreadCall) !void {
...
const TestOwnedTextureDescriptor = struct {
    extent: mapvina.RenderTargetExtent = .{},
};
...
const TestOwnedTextureContext = if (build_options.supports_vulkan) VulkanAttachContext else if (build_options.supports_metal) struct {
    ...
    pub fn descriptor(self: *const @This()) mapvina.MetalContextDescriptor {
...
fn attachTestOwnedTexture(map: *mapvina.MapHandle, descriptor: TestOwnedTextureDescriptor) !TestOwnedTextureSession {
    var context = try TestOwnedTextureContext.init();
    errdefer context.deinit();

    var session = if (build_options.supports_vulkan)
        try mapvina.attachVulkanOwnedTexture(map, .{
            .extent = descriptor.extent,
            .context = context.descriptor(),
        })
    else if (build_options.supports_metal)
        try mapvina.attachMetalOwnedTexture(map, .{
            .extent = descriptor.extent,
            .context = context.descriptor(),
        })
    else
...
fn createMovedMetalSessionWithFrame(device: *anyopaque) !struct {
    runtime: mapvina.RuntimeHandle,
    map: mapvina.MapHandle,
    session: mapvina.RenderSessionHandle,
    frame: mapvina.MetalOwnedTextureFrameHandle,
} {
    ...
    var session = try mapvina.attachMetalOwnedTexture(&map, .{
...
const VulkanAttachContext = if (build_options.supports_vulkan) struct {
    ...
    pub fn init() !VulkanAttachContext {
        ...
        app_info.pApplicationName = "mapvina-native-zig-binding-tests";
        app_info.applicationVersion = 1;
        app_info.pEngineName = "mapvina-native-zig-binding-tests";
    ...
    pub fn descriptor(self: *const VulkanAttachContext) mapvina.VulkanContextDescriptor {
        return .{
...
const VulkanBorrowedImage = if (build_options.supports_vulkan) struct {
    ...
    pub fn descriptor(self: *const VulkanBorrowedImage) mapvina.VulkanBorrowedTextureDescriptor {
        return .{
...
test "still-image map modes drive owned texture rendering" {
    inline for (.{ mapvina.MapMode.static, mapvina.MapMode.tile }) |mode| {
        var runtime = try mapvina.RuntimeHandle.init(null);
        defer runtime.close() catch @panic("runtime close failed");

        var map = try mapvina.MapHandle.create(&runtime, .{ .mode = mode });
        defer map.close() catch @panic("map close failed");
...
test "render session feature state set get and remove" {
    ...
    const selector = mapvina.FeatureStateSelector{ .source_id = "point", .feature_id = "feature-1" };
    const state_members = [_]mapvina.JsonMember{
...
test "render session queries cluster feature extensions" {
    ...
    const args_members = [_]mapvina.JsonMember{
...
test "unsupported backend owned texture attachment reports unsupported" {
    ...
    const fake_pointer = mapvina.NativePointer{ .ptr = @ptrFromInt(1) };
    if (build_options.supports_metal) {
        try testing.expectError(error.Unsupported, mapvina.attachVulkanOwnedTexture(&map, .{
            .context = .{
    ...
    if (build_options.supports_vulkan) {
        try testing.expectError(error.Unsupported, mapvina.attachMetalOwnedTexture(&map, .{
...
test "Metal borrowed texture renders through public bindings" {
    ...
    var session = try mapvina.attachMetalBorrowedTexture(&map, .{
...
test "Vulkan owned texture frame handle scopes native pointers" {
    ...
    var session = try mapvina.attachVulkanOwnedTexture(&map, .{
        .extent = .{ .width = 32, .height = 32, .scale_factor = 1.0 },
        .context = context.descriptor(),
    });
    defer session.close() catch {};
...
test "Vulkan borrowed texture renders through public bindings" {
    ...
    var session = try mapvina.attachVulkanBorrowedTexture(&map, borrowed.descriptor());
    defer session.close() catch {};
