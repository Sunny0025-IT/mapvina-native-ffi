...
test "GeoJSON descriptors add and update sources through public binding" {
    ...
    const properties = [_]mapvina.JsonMember{
        .{ .key = "name", .value = .{ .string = "San Francisco" } },
        .{ .key = "visible", .value = .{ .bool = true } },
    };
    const features = [_]mapvina.Feature{.{
    ...
    try testing.expectEqual(mapvina.StyleSourceType.geojson, (try map.getStyleSourceType(testing.allocator, "geo-url")).?);
...
test "geometry descriptor graphs support nested collections" {
    ...
    const line = [_]mapvina.LatLng{
        .{ .latitude = 37.0, .longitude = -123.0 },
        .{ .latitude = 38.0, .longitude = -122.0 },
    };
    const ring = [_]mapvina.LatLng{
        .{ .latitude = 37.0, .longitude = -123.0 },
        .{ .latitude = 38.0, .longitude = -123.0 },
        .{ .latitude = 38.0, .longitude = -122.0 },
        .{ .latitude = 37.0, .longitude = -123.0 },
    };
    const rings = [_][]const mapvina.LatLng{ring[0..]};
    const children = [_]mapvina.Geometry{
        .{ .line_string = line[0..] },
        .{ .polygon = rings[0..] },
    };
...
test "geometry coordinate spans remain stable across nested materialization" {
    ...
    var first_line: [96]mapvina.LatLng = undefined;
    var second_line: [96]mapvina.LatLng = undefined;
    ...
    const lines = [_][]const mapvina.LatLng{ first_line[0..], second_line[0..] };
