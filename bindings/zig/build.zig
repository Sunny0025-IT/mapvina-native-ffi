...
pub fn cmakeArtifactDir(b: *std.Build) std.Build.LazyPath {
    return b.option(
        std.Build.LazyPath,
        "cmake-artifact-dir",
        "Directory containing the CMake-built mapvina-native-c library",
...
pub fn mapvinaNativeModule(b: *std.Build, options: DependencyOptions) *std.Build.Module {
    return dependency(b, options).module("mapvina_native");
}
...
/// Links a Zig module to the MapVina Native C library and its backend-specific dependencies.
///
...
pub fn linkMapvinaNativeC(b: *std.Build, module_: *std.Build.Module, options: LinkOptions) void {
    addIncludePaths(module_, options.include_dirs);
    if (options.target.result.os.tag == .windows) {
        module_.addObjectFile(options.cmake_artifact_dir.path(b, "mapvina-native-c.lib"));
    } else {
        module_.addLibraryPath(options.cmake_artifact_dir);
        module_.addRPath(options.cmake_artifact_dir);
        module_.linkSystemLibrary("mapvina-native-c", .{});
    }
...
fn addMapvinaNativeModule(b: *std.Build, options: BuildOptions) *std.Build.Module {
    const mapvina_native = b.addModule("mapvina_native", .{
        .root_source_file = b.path("src/mapvina_native.zig"),
        .target = options.target,
        .optimize = options.optimize,
    });
    linkMapvinaNativeC(b, mapvina_native, repoLinkOptions(options));
    return mapvina_native;
}
...
fn addTestCompile(b: *std.Build, options: BuildOptions, root_source_file: std.Build.LazyPath) *std.Build.Step.Compile {
    ...
    linkMapvinaNativeC(b, tests.root_module, repoLinkOptions(options));
    return tests;
}

fn addBindingTests(b: *std.Build, options: BuildOptions, mapvina_native: *std.Build.Module) *std.Build.Step.Compile {
    const tests = addTestCompile(b, options, b.path("tests/main.zig"));
    tests.root_module.addImport("mapvina_native", mapvina_native);
...
pub fn build(b: *std.Build) void {
    ...
    const mapvina_native = addMapvinaNativeModule(b, options);
    ...
    const binding_tests = addBindingTests(b, options, mapvina_native);
