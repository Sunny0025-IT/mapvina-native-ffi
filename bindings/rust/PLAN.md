...
and Java JNI can share `mapvina-native-core` without adopting the public Rust
...
Bridge bindings should build on `mapvina-native-core`, not on the public
`mapvina-native` crate. They may touch `mapvina-native-sys` directly only in
host-runtime trampoline code or where a C ABI feature has no core abstraction
yet.

`mapvina-native-core` owns bridge-neutral C ABI adaptation:
