...
use crate::geometry::{
    Geometry, MAX_GEOMETRY_COLLECTION_DEPTH, NativeGeometry, geometry_from_native,
    geometry_from_native_with_depth, geometry_try_to_native, geometry_try_to_native_with_depth,
};
...
impl Feature {
    ...
    unsafe fn from_native(raw: &sys::mln_feature, depth: usize) -> Result<Self> {
        ...
        let geometry = unsafe { geometry_from_native_with_depth(&*raw.geometry, depth + 1) }?;
...
impl GeoJson {
    ...
    /// `raw` and all nested pointers must be valid for the duration of this
    /// call. The returned value owns all copied data.
    unsafe fn from_native(raw: &sys::mln_geojson) -> Result<Self> {
        match raw.type_ {
            sys::MLN_GEOJSON_TYPE_GEOMETRY => {
                ...
                Ok(Self::Geometry(unsafe { geometry_from_native(&*geometry) }?))
            }
...
pub fn feature_try_to_native(feature: &Feature, depth: usize) -> Result<NativeFeature> {
...
pub unsafe fn feature_from_native(raw: &sys::mln_feature, depth: usize) -> Result<Feature> {
    // SAFETY: The caller promises raw and nested pointers are valid for this call.
    unsafe { Feature::from_native(raw, depth) }
}

pub fn geojson_try_to_native(geojson: &GeoJson) -> Result<NativeGeoJson> {
    geojson.try_to_native()
}
...
impl NativeFeature {
    fn new(feature: &Feature, depth: usize) -> Result<Self> {
        check_geojson_depth(depth)?;
        let geometry = geometry_try_to_native_with_depth(&feature.geometry, depth + 1)?;
...
mod tests {
    ...
    fn geojson_depth_limit_counts_feature_geometry_boundary() {
        ...
        geometry_try_to_native(&geometry).unwrap();
