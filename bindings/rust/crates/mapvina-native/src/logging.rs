...
use mapvina_core::LogSeverityMask;
use mapvina_native_core as mapvina_core;

pub use mapvina_core::LogRecord;
...
/// Installs or replaces the process-global MapVina Native log callback.
///
/// MapVina Native may invoke the callback from logging or worker threads. The
/// callback state must therefore be `Send + Sync + 'static`. The callback
/// should return quickly and avoid calling MapVina Native APIs. Panics are
...
/// Clears the process-global MapVina Native log callback.
pub fn clear_log_callback() -> Result<()> {
    ...
    mapvina_core::check(unsafe { sys::mln_log_clear_callback() })?;

    lock_log_callback_state().current = None;
    Ok(())
}

/// Configures severities that MapVina Native may dispatch asynchronously.
pub fn set_async_log_severity_mask(mask: LogSeverityMask) -> Result<()> {
    ...
    mapvina_core::check(unsafe { sys::mln_log_set_async_severity_mask(mask.bits()) })
}

/// Restores MapVina Native's default async log severity mask.
...
fn invoke_callback(
    ...
    let Ok(record) = (unsafe {
        mapvina_core::logging::copy_log_record(raw_severity, raw_event, code, message)
    }) else {
        return 0;
    };
...
mod tests {
    ...
    use super::*;
    use crate::ErrorKind;
    use mapvina_core::{LogEvent, LogSeverity};
