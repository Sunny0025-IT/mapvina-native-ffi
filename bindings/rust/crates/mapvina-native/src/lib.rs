//! Safe Rust binding for the MapVina Native C API.
//!
...
//! adaptation lives in `mapvina-native-core`.
...
pub fn network_status() -> Result<NetworkStatus> {
    ...
    mapvina_core::check(unsafe { sys::mln_network_status_get(&mut raw_status) })?;
...
fn set_network_status_raw(raw_status: u32) -> Result<()> {
    ...
    mapvina_core::check(unsafe { sys::mln_network_status_set(raw_status) })
}
