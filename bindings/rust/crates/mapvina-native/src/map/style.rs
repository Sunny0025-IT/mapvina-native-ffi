...
use mapvina_native_core as mapvina_core;
use mapvina_native_core::ptr::const_ptr_or_null;
use mapvina_native_core::values::lat_lngs_to_native;
use mapvina_native_sys as sys;
...
impl super::MapHandle {
    /// Loads a style URL through MapVina Native style APIs.
    ///
    pub fn set_style_url(&self, url: &str) -> Result<()> {
        let map = self.inner.as_ptr()?;
        let url = mapvina_core::string::c_string(url)?;
        ...
        // returning.
        mapvina_core::check(unsafe { sys::mln_map_set_style_url(map, url.as_ptr()) })?;
        Ok(())
    }

    /// Loads inline style JSON through MapVina Native style APIs.
    pub fn set_style_json(&self, json: &str) -> Result<()> {
        let map = self.inner.as_ptr()?;
        let json = mapvina_core::string::c_string(json)?;
        ...
        mapvina_core::check(unsafe { sys::mln_map_set_style_json(map, json.as_ptr()) })?;
    ...
    pub fn add_custom_geometry_source(
        ...
        let map = self.inner.as_ptr()?;
        let source_id_view = mapvina_core::string::string_view(source_id);
    ...
    pub fn set_custom_geometry_source_tile_data(
        ...
        let map = self.inner.as_ptr()?;
        let source_id = mapvina_core::string::string_view(source_id);
    ...
    pub fn add_vector_source_url(
        ...
        let map = self.inner.as_ptr()?;
        let source_id = mapvina_core::string::string_view(source_id);
        let url = mapvina_core::string::string_view(url);
    ...
    pub fn add_image_source_image(
        ...
        let source_id = mapvina_core::string::string_view(source_id);
        let coordinates = lat_lngs_to_native(coordinates);
        let image = mapvina_core::values::premultiplied_rgba8_image_to_native(image);
        // SAFETY: map is live, source_id is an explicit-length view valid for
    ...
    pub fn set_style_image(
        ...
        let map = self.inner.as_ptr()?;
        let image_id = mapvina_core::string::string_view(image_id);
        let image = mapvina_core::values::premultiplied_rgba8_image_to_native(image);
    ...
    pub fn style_image_info(&self, image_id: &str) -> Result<Option<StyleImageInfo>> {
        let map = self.inner.as_ptr()?;
        let image_id = mapvina_core::string::string_view(image_id);
        let mut info = mapvina_core::style::empty_style_image_info();
        let mut found = false;
        // SAFETY: map is live, image_id is an explicit-length view valid for
        ...
        Ok(found.then(|| mapvina_core::values::style_image_info_from_native(&info)))
    }
    ...
    pub fn copy_style_image_premultiplied_rgba8(
        ...
        let map = self.inner.as_ptr()?;
        let image_id = mapvina_core::string::string_view(image_id);
        let mut raw_info = mapvina_core::style::empty_style_image_info();
        ...
        let info = mapvina_core::values::style_image_info_from_native(&raw_info);
        ...
        mapvina_core::style::style_image_from_copied_premultiplied_rgba8(info, data, copied_size)
            .map(Some)
    }
    ...
    pub fn style_source_info(&self, source_id: &str) -> Result<Option<SourceInfo>> {
        let map = self.inner.as_ptr()?;
        let source_id = mapvina_core::string::string_view(source_id);
        let mut info = mapvina_core::style::empty_style_source_info();
        let mut found = false;
        // SAFETY: map is live, source_id is an explicit-length view valid for
        // this call, info has its ABI size initialized, and found points to
        // writable storage.
        ...
        Ok(Some(mapvina_core::style::style_source_info_from_native(
            &info,
            attribution,
        )))
    }
    ...
    pub fn add_style_layer_json(
        ...
        let before_layer_id = mapvina_core::string::string_view(before_layer_id.unwrap_or(""));
    ...
    pub fn add_hillshade_layer(
        ...
        let map = self.inner.as_ptr()?;
        let layer_id = mapvina_core::string::string_view(layer_id);
        let source_id = mapvina_core::string::string_view(source_id);
        let before_layer_id = mapvina_core::string::string_view(before_layer_id.unwrap_or(""));
    ...
    pub fn style_layer_json(&self, layer_id: &str) -> Result<Option<JsonValue>> {
        let map = self.inner.as_ptr()?;
        let layer_id = mapvina_core::string::string_view(layer_id);
        let mut out = mapvina_core::ptr::OutPtr::<sys::mln_json_snapshot>::new();
        let mut found = false;
        ...
        let snapshot = unsafe { mapvina_core::json::copy_json_snapshot(out.into_option()) }?;
    ...
    pub fn set_style_light_json(&self, light_json: &JsonValue) -> Result<()> {
        ...
        mapvina_core::check(unsafe { sys::mln_map_set_style_light_json(map, light_json.as_ptr()) })
    }

    /// Sets one style light property.
    pub fn set_style_light_property(&self, property_name: &str, value: &JsonValue) -> Result<()> {
        let map = self.inner.as_ptr()?;
        let property_name = mapvina_core::string::string_view(property_name);
    ...
    pub fn style_light_property(&self, property_name: &str) -> Result<Option<JsonValue>> {
        ...
        // SAFETY: On success, the C API returns either null or an owned JSON
        // snapshot handle for this call; core copies and releases it.
        unsafe { mapvina_core::json::copy_json_snapshot(out.into_option()) }
    }
    ...
    pub fn style_source_ids(&self) -> Result<Vec<String>> {
        let map = self.inner.as_ptr()?;
        let mut out = mapvina_core::ptr::OutPtr::<sys::mln_style_id_list>::new();
        ...
        mapvina_core::check(unsafe { sys::mln_map_list_style_source_ids(map, out.as_mut_ptr()) })?;
        // SAFETY: On success, the C API returns an owned style ID list handle;
        // core copies and releases it.
        unsafe { mapvina_core::style::copy_style_id_list(out.into_non_null("mln_style_id_list")?) }
    }
    ...
    pub fn style_layer_ids(&self) -> Result<Vec<String>> {
        ...
        // this call. On success the returned handle is wrapped and destroyed by
        // the copying helper below.
        mapvina_core::check(unsafe { sys::mln_map_list_style_layer_ids(map, out.as_mut_ptr()) })?;
        // SAFETY: On success, the C API returns an owned style ID list handle;
        // core copies and releases it.
        unsafe { mapvina_core::style::copy_style_id_list(out.into_non_null("mln_style_id_list")?) }
    }
