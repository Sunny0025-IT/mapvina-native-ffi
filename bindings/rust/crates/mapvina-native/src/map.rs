...
use mapvina_native_core::ptr::{const_ptr_or_null, mut_ptr_or_null, option_ptr};
use mapvina_native_core::values::{
...
impl MapHandle {
    ...
    pub fn with_options(runtime: &RuntimeHandle, options: &MapOptions) -> Result<Self> {
        ...
        let mut out = mapvina_core::ptr::OutPtr::<sys::mln_map>::new();
    ...
    pub fn request_repaint(&self) -> Result<()> {
        ...
        mapvina_core::check(unsafe { sys::mln_map_request_repaint(map) })
    }
    ...
    pub fn request_still_image(&self) -> Result<()> {
        let map = self.inner.as_ptr()?;
        // SAFETY: map is a live map handle owned by this wrapper.
        mapvina_core::check(unsafe { sys::mln_map_request_still_image(map) })
    }

    /// Applies MapVina debug overlay mask bits.
    pub fn set_debug_options(&self, options: MapDebugOptions) -> Result<()> {
        let map = self.inner.as_ptr()?;
        // SAFETY: map is live. The C API validates unknown mask bits.
        mapvina_core::check(unsafe { sys::mln_map_set_debug_options(map, options.bits()) })
    }

    /// Reads MapVina debug overlay mask bits.
    pub fn debug_options(&self) -> Result<MapDebugOptions> {
        let map = self.inner.as_ptr()?;
        let mut raw = 0;
        // SAFETY: map is live and out_options points to writable u32 storage.
        mapvina_core::check(unsafe { sys::mln_map_get_debug_options(map, &mut raw) })?;
        Ok(MapDebugOptions::from_bits_retain(raw))
    }

    /// Enables or disables MapVina's rendering stats overlay view.
    pub fn set_rendering_stats_view_enabled(&self, enabled: bool) -> Result<()> {
        let map = self.inner.as_ptr()?;
        // SAFETY: map is live and enabled is passed by value.
        mapvina_core::check(unsafe { sys::mln_map_set_rendering_stats_view_enabled(map, enabled) })
    }

    /// Reads whether MapVina's rendering stats overlay view is enabled.
    ...
    /// Reads whether MapVina currently considers the map fully loaded.
    pub fn is_fully_loaded(&self) -> Result<bool> {
        let map = self.inner.as_ptr()?;
        let mut loaded = false;
        // SAFETY: map is live and out_loaded points to writable bool storage.
        mapvina_core::check(unsafe { sys::mln_map_is_fully_loaded(map, &mut loaded) })?;
        Ok(loaded)
    }

    /// Dumps map debug logs through MapVina Native logging.
    pub fn dump_debug_logs(&self) -> Result<()> {
        let map = self.inner.as_ptr()?;
        // SAFETY: map is live.
        mapvina_core::check(unsafe { sys::mln_map_dump_debug_logs(map) })
    }
    ...
    pub fn viewport_options(&self) -> Result<MapViewportOptions> {
        ...
        mapvina_core::check(unsafe { sys::mln_map_get_viewport_options(map, &mut raw) })?;
    ...
    pub fn set_viewport_options(&self, options: &MapViewportOptions) -> Result<()> {
        ...
        mapvina_core::check(unsafe { sys::mln_map_set_viewport_options(map, &raw) })
    }
    ...
    pub fn tile_options(&self) -> Result<MapTileOptions> {
        ...
        // SAFETY: map is live and raw has a valid size field for C to fill.
        mapvina_core::check(unsafe { sys::mln_map_get_tile_options(map, &mut raw) })?;
    ...
    pub fn set_tile_options(&self, options: &MapTileOptions) -> Result<()> {
        ...
        // SAFETY: map is live and raw is a materialized descriptor valid for
        // the duration of this call.
        mapvina_core::check(unsafe { sys::mln_map_set_tile_options(map, &raw) })
    }
    ...
    pub fn camera(&self) -> Result<CameraOptions> {
        ...
        // SAFETY: map is live and raw has a valid size field for C to fill.
        mapvina_core::check(unsafe { sys::mln_map_get_camera(map, &mut raw) })?;
    ...
    pub fn jump_to(&self, camera: &CameraOptions) -> Result<()> {
        ...
        // SAFETY: map is live and raw is a materialized descriptor valid for
        // the duration of this call.
        mapvina_core::check(unsafe { sys::mln_map_jump_to(map, &raw) })
    }
    ...
    pub fn move_by(&self, delta_x: f64, delta_y: f64) -> Result<()> {
        ...
        mapvina_core::check(unsafe { sys::mln_map_move_by(map, delta_x, delta_y) })
    }
    ...
    pub fn pitch_by(&self, pitch: f64) -> Result<()> {
        let map = self.inner.as_ptr()?;
        // SAFETY: map is live. The C API validates numeric values.
        mapvina_core::check(unsafe { sys::mln_map_pitch_by(map, pitch) })
    }
    ...
    pub fn cancel_transitions(&self) -> Result<()> {
        let map = self.inner.as_ptr()?;
        // SAFETY: map is live.
        mapvina_core::check(unsafe { sys::mln_map_cancel_transitions(map) })
    }
    ...
    pub fn bounds(&self) -> Result<BoundOptions> {
        ...
        // SAFETY: map is live and raw has a valid size field for C to fill.
        mapvina_core::check(unsafe { sys::mln_map_get_bounds(map, &mut raw) })?;
    ...
    pub fn set_bounds(&self, options: &BoundOptions) -> Result<()> {
        ...
        mapvina_core::check(unsafe { sys::mln_map_set_bounds(map, &raw) })
    }
    ...
    pub fn free_camera_options(&self) -> Result<FreeCameraOptions> {
        ...
        // SAFETY: map is live and raw has a valid size field for C to fill.
        mapvina_core::check(unsafe { sys::mln_map_get_free_camera_options(map, &mut raw) })?;
    ...
    pub fn set_free_camera_options(&self, options: &FreeCameraOptions) -> Result<()> {
        ...
        let raw = options.to_native();
        // SAFETY: map is live and raw is a valid descriptor for this call.
        mapvina_core::check(unsafe { sys::mln_map_set_free_camera_options(map, &raw) })
    }
    ...
    pub fn projection_mode(&self) -> Result<ProjectionMode> {
        ...
        // SAFETY: map is live and raw has a valid size field for C to fill.
        mapvina_core::check(unsafe { sys::mln_map_get_projection_mode(map, &mut raw) })?;
    ...
    pub fn set_projection_mode(&self, mode: &ProjectionMode) -> Result<()> {
        ...
        // SAFETY: map is live and raw is a valid descriptor for this call.
        mapvina_core::check(unsafe { sys::mln_map_set_projection_mode(map, &raw) })
    }
