...
use mapvina_native_core::values::{empty_lat_lng, empty_screen_point, lat_lngs_to_native};
...
impl MapProjectionHandle {
    pub(crate) fn new(map: &MapHandle) -> Result<Self> {
        ...
        let mut out = mapvina_core::ptr::OutPtr::<sys::mln_map_projection>::new();
        // SAFETY: map_ptr is a live map handle. out is a valid null-initialized
        // out-pointer owned by this call.
        mapvina_core::check(unsafe { sys::mln_map_projection_create(map_ptr, out.as_mut_ptr()) })?;
    ...
    pub fn camera(&self) -> Result<CameraOptions> {
        ...
        mapvina_core::check(unsafe { sys::mln_map_projection_get_camera(projection, &mut raw) })?;
    ...
    pub fn set_camera(&self, camera: &CameraOptions) -> Result<()> {
        ...
        mapvina_core::check(unsafe { sys::mln_map_projection_set_camera(projection, &raw) })
    }
