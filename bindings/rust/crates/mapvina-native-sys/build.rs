...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    let header = repo_root.join("include/mapvina_native_c.h");
    ...
    println!("cargo:rustc-link-lib=dylib=mapvina-native-c");
...
fn repo_root_from_manifest_dir(manifest_dir: &Path) -> Result<PathBuf, Box<dyn Error>> {
    manifest_dir
        .ancestors()
        .find(|ancestor| ancestor.join("include/mapvina_native_c.h").is_file())
        .map(Path::to_path_buf)
        .ok_or_else(|| {
            format!(
                "could not locate repository root containing include/mapvina_native_c.h from {}",
