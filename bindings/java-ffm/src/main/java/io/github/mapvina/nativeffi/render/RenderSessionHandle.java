...
import io.github.mapvina.nativeffi.internal.struct.QueryStructs;
import io.github.mapvina.nativeffi.internal.struct.RenderStructs;
...
public final class RenderSessionHandle implements AutoCloseable {
  ...
  public static RenderSessionHandle attachMetalOwnedTexture(
    ...
    try (var arena = Arena.ofConfined()) {
      ...
      Status.check(
          MapVinaNativeC.mln_metal_owned_texture_attach(
  ...
  public static RenderSessionHandle attachMetalBorrowedTexture(
    ...
    try (var arena = Arena.ofConfined()) {
      var outSession = MemoryUtil.allocatePointer(arena);
      Status.check(
          MapVinaNativeC.mln_metal_borrowed_texture_attach(
              map.nativeHandle(InternalAccess.INSTANCE),
  ...
  public static RenderSessionHandle attachVulkanOwnedTexture(
    ...
    try (var arena = Arena.ofConfined()) {
      var outSession = MemoryUtil.allocatePointer(arena);
      Status.check(
          MapVinaNativeC.mln_vulkan_owned_texture_attach(
              map.nativeHandle(InternalAccess.INSTANCE),
  ...
  public static RenderSessionHandle attachVulkanBorrowedTexture(
    ...
    try (var arena = Arena.ofConfined()) {
      var outSession = MemoryUtil.allocatePointer(arena);
      Status.check(
          MapVinaNativeC.mln_vulkan_borrowed_texture_attach(
              map.nativeHandle(InternalAccess.INSTANCE),
  ...
  public static RenderSessionHandle attachMetalSurface(
    ...
    try (var arena = Arena.ofConfined()) {
      var outSession = MemoryUtil.allocatePointer(arena);
      Status.check(
          MapVinaNativeC.mln_metal_surface_attach(
              map.nativeHandle(InternalAccess.INSTANCE),
  ...
  public static RenderSessionHandle attachVulkanSurface(
    ...
    try (var arena = Arena.ofConfined()) {
      var outSession = MemoryUtil.allocatePointer(arena);
      Status.check(
          MapVinaNativeC.mln_vulkan_surface_attach(
              map.nativeHandle(InternalAccess.INSTANCE),
  ...
  public void resize(int width, int height, double scaleFactor) {
    NativeAccess.ensureLoaded();
    Status.check(
        MapVinaNativeC.mln_render_session_resize(state.requireLive(), width, height, scaleFactor));
  }

  public void renderUpdate() {
    NativeAccess.ensureLoaded();
    Status.check(MapVinaNativeC.mln_render_session_render_update(state.requireLive()));
  }

  public void detach() {
    NativeAccess.ensureLoaded();
    Status.check(MapVinaNativeC.mln_render_session_detach(state.requireLive()));
  }

  public void reduceMemoryUse() {
    NativeAccess.ensureLoaded();
    Status.check(MapVinaNativeC.mln_render_session_reduce_memory_use(state.requireLive()));
  }

  public void clearData() {
    NativeAccess.ensureLoaded();
    Status.check(MapVinaNativeC.mln_render_session_clear_data(state.requireLive()));
  }

  public void dumpDebugLogs() {
    NativeAccess.ensureLoaded();
    Status.check(MapVinaNativeC.mln_render_session_dump_debug_logs(state.requireLive()));
  }
  ...
  public void setFeatureState(FeatureStateSelector selector, JsonValue value) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_render_session_set_feature_state(
              state.requireLive(),
  ...
  public JsonValue getFeatureState(FeatureStateSelector selector) {
    ...
    try (var arena = Arena.ofConfined()) {
      ...
      Status.check(
          MapVinaNativeC.mln_render_session_get_feature_state(
  ...
  public void removeFeatureState(FeatureStateSelector selector) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_render_session_remove_feature_state(
  ...
  public FeatureExtensionResult queryFeatureExtension(
    ...
    try (var arena = Arena.ofConfined()) {
      ...
      Status.check(
          MapVinaNativeC.mln_render_session_query_feature_extensions(
              state.requireLive(),
  ...
  public TextureImageInfo textureImageInfo() {
    NativeAccess.ensureLoaded();
    try (var arena = Arena.ofConfined()) {
      var outInfo = MapVinaNativeC.mln_texture_image_info_default(arena);
      var status =
          MapVinaNativeC.mln_texture_read_premultiplied_rgba8(
              state.requireLive(), MemorySegment.NULL, 0, outInfo);
      var info = RenderStructs.textureImageInfo(outInfo);
      if (status == MapvinaStatus.OK.nativeCode()
          || (status == MapvinaStatus.INVALID_ARGUMENT.nativeCode() && info.byteLength() > 0)) {
        return info;
      }
      Status.check(status);
  ...
  public MetalOwnedTextureFrameHandle acquireMetalOwnedTextureFrame() {
    ...
    try {
      Status.check(
          MapVinaNativeC.mln_metal_owned_texture_acquire_frame(state.requireLive(), frameSegment));
  ...
  public VulkanOwnedTextureFrameHandle acquireVulkanOwnedTextureFrame() {
    ...
    try {
      Status.check(
          MapVinaNativeC.mln_vulkan_owned_texture_acquire_frame(
  ...
  private List<QueriedFeature> queryRenderedFeaturesInternal(
    ...
    try (var arena = Arena.ofConfined()) {
      var outResult = MemoryUtil.allocatePointer(arena);
      Status.check(
          MapVinaNativeC.mln_render_session_query_rendered_features(
              state.requireLive(),
  ...
  private List<QueriedFeature> querySourceFeaturesInternal(
    ...
    try (var arena = Arena.ofConfined()) {
      var outResult = MemoryUtil.allocatePointer(arena);
      Status.check(
          MapVinaNativeC.mln_render_session_query_source_features(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(sourceId, "sourceId"), arena),
              hasOptions
  ...
  public void close() {
    NativeAccess.ensureLoaded();
    state.closeOnce(MapVinaNativeC::mln_render_session_destroy);
  }
  ...
  void releaseMetalFrame(MemorySegment frameSegment, Throwable callbackFailure) {
    try {
      Status.check(
          MapVinaNativeC.mln_metal_owned_texture_release_frame(state.requireLive(), frameSegment));
  ...
  void releaseVulkanFrame(MemorySegment frameSegment, Throwable callbackFailure) {
    try {
      Status.check(
          MapVinaNativeC.mln_vulkan_owned_texture_release_frame(
              state.requireLive(), frameSegment));
    } catch (Throwable releaseFailure) {
      if (callbackFailure != null) {
        callbackFailure.addSuppressed(releaseFailure);
