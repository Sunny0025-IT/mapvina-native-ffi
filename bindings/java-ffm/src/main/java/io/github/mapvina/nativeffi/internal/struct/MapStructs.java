...
import static io.github.mapvina.nativeffi.internal.struct.CoreStructs.edgeInsets;
import static io.github.mapvina.nativeffi.internal.struct.CoreStructs.latLngBounds;
import static io.github.mapvina.nativeffi.internal.struct.CoreStructs.quaternion;
import static io.github.mapvina.nativeffi.internal.struct.CoreStructs.screenPoint;
import static io.github.mapvina.nativeffi.internal.struct.CoreStructs.unitBezier;
import static io.github.mapvina.nativeffi.internal.struct.CoreStructs.vec3;
...
import io.github.mapvina.nativeffi.internal.c.mln_animation_options;
import io.github.mapvina.nativeffi.internal.c.mln_bound_options;
import io.github.mapvina.nativeffi.internal.c.mln_camera_fit_options;
import io.github.mapvina.nativeffi.internal.c.mln_camera_options;
import io.github.mapvina.nativeffi.internal.c.mln_free_camera_options;
import io.github.mapvina.nativeffi.internal.c.mln_map_options;
import io.github.mapvina.nativeffi.internal.c.mln_map_tile_options;
import io.github.mapvina.nativeffi.internal.c.mln_map_viewport_options;
import io.github.mapvina.nativeffi.internal.c.mln_projection_mode;
import io.github.mapvina.nativeffi.map.ConstrainMode;
import io.github.mapvina.nativeffi.map.MapOptions;
import io.github.mapvina.nativeffi.map.NorthOrientation;
import io.github.mapvina.nativeffi.map.ProjectionModeOptions;
import io.github.mapvina.nativeffi.map.TileLodMode;
import io.github.mapvina.nativeffi.map.TileOptions;
import io.github.mapvina.nativeffi.map.ViewportMode;
import io.github.mapvina.nativeffi.map.ViewportOptions;
...
public final class MapStructs {
  ...
  public static MemorySegment mapOptions(MapOptions options, Arena arena) {
    var segment = MapVinaNativeC.mln_map_options_default(arena);
  ...
  public static MemorySegment cameraOptions(CameraOptions options, Arena arena) {
    var segment = MapVinaNativeC.mln_camera_options_default(arena);
    var fields = 0;
    if (options.hasCenter()) {
      fields |= MapVinaNativeC.MLN_CAMERA_OPTION_CENTER();
  ...
  public static CameraOptions cameraOptions(MemorySegment segment) {
    ...
    if ((fields & MapVinaNativeC.MLN_CAMERA_OPTION_CENTER()) != 0) {
      options.center(mln_camera_options.latitude(segment), mln_camera_options.longitude(segment));
    }
    if ((fields & MapVinaNativeC.MLN_CAMERA_OPTION_CENTER_ALTITUDE()) != 0) {
      options.centerAltitude(mln_camera_options.center_altitude(segment));
    }
    if ((fields & MapVinaNativeC.MLN_CAMERA_OPTION_PADDING()) != 0) {
      options.padding(edgeInsets(mln_camera_options.padding(segment)));
    }
    if ((fields & MapVinaNativeC.MLN_CAMERA_OPTION_ANCHOR()) != 0) {
      options.anchor(screenPoint(mln_camera_options.anchor(segment)));
    }
    if ((fields & MapVinaNativeC.MLN_CAMERA_OPTION_ZOOM()) != 0) {
      options.zoom(mln_camera_options.zoom(segment));
    }
    if ((fields & MapVinaNativeC.MLN_CAMERA_OPTION_BEARING()) != 0) {
      options.bearing(mln_camera_options.bearing(segment));
    }
    if ((fields & MapVinaNativeC.MLN_CAMERA_OPTION_PITCH()) != 0) {
      options.pitch(mln_camera_options.pitch(segment));
    }
    if ((fields & MapVinaNativeC.MLN_CAMERA_OPTION_ROLL()) != 0) {
      options.roll(mln_camera_options.roll(segment));
    }
    if ((fields & MapVinaNativeC.MLN_CAMERA_OPTION_FOV()) != 0) {
  ...
  public static MemorySegment animationOptions(AnimationOptions options, Arena arena) {
    var segment = MapVinaNativeC.mln_animation_options_default(arena);
    var fields = 0;
    if (options.hasDurationMs()) {
      fields |= MapVinaNativeC.MLN_ANIMATION_OPTION_DURATION();
      mln_animation_options.duration_ms(segment, options.durationMs());
    }
    if (options.hasVelocity()) {
      fields |= MapVinaNativeC.MLN_ANIMATION_OPTION_VELOCITY();
      mln_animation_options.velocity(segment, options.velocity());
    }
    if (options.hasMinZoom()) {
      fields |= MapVinaNativeC.MLN_ANIMATION_OPTION_MIN_ZOOM();
      mln_animation_options.min_zoom(segment, options.minZoom());
    }
    if (options.hasEasing()) {
      fields |= MapVinaNativeC.MLN_ANIMATION_OPTION_EASING();
  ...
  public static MemorySegment cameraFitOptions(CameraFitOptions options, Arena arena) {
    var segment = MapVinaNativeC.mln_camera_fit_options_default(arena);
    var fields = 0;
    if (options.hasPadding()) {
      fields |= MapVinaNativeC.MLN_CAMERA_FIT_OPTION_PADDING();
      mln_camera_fit_options.padding(segment, edgeInsets(options.padding(), arena));
    }
    if (options.hasBearing()) {
      fields |= MapVinaNativeC.MLN_CAMERA_FIT_OPTION_BEARING();
      mln_camera_fit_options.bearing(segment, options.bearing());
    }
    if (options.hasPitch()) {
      fields |= MapVinaNativeC.MLN_CAMERA_FIT_OPTION_PITCH();
  ...
  public static MemorySegment boundOptions(BoundOptions options, Arena arena) {
    var segment = MapVinaNativeC.mln_bound_options_default(arena);
    var fields = 0;
    if (options.hasBounds()) {
      fields |= MapVinaNativeC.MLN_BOUND_OPTION_BOUNDS();
      mln_bound_options.bounds(segment, latLngBounds(options.bounds(), arena));
    }
    if (options.hasMinZoom()) {
      fields |= MapVinaNativeC.MLN_BOUND_OPTION_MIN_ZOOM();
      mln_bound_options.min_zoom(segment, options.minZoom());
    }
    if (options.hasMaxZoom()) {
      fields |= MapVinaNativeC.MLN_BOUND_OPTION_MAX_ZOOM();
      mln_bound_options.max_zoom(segment, options.maxZoom());
    }
    if (options.hasMinPitch()) {
      fields |= MapVinaNativeC.MLN_BOUND_OPTION_MIN_PITCH();
      mln_bound_options.min_pitch(segment, options.minPitch());
    }
    if (options.hasMaxPitch()) {
      fields |= MapVinaNativeC.MLN_BOUND_OPTION_MAX_PITCH();
  ...
  public static BoundOptions boundOptions(MemorySegment segment) {
    ...
    if ((fields & MapVinaNativeC.MLN_BOUND_OPTION_BOUNDS()) != 0) {
      options.bounds(latLngBounds(mln_bound_options.bounds(segment)));
    }
    if ((fields & MapVinaNativeC.MLN_BOUND_OPTION_MIN_ZOOM()) != 0) {
      options.minZoom(mln_bound_options.min_zoom(segment));
    }
    if ((fields & MapVinaNativeC.MLN_BOUND_OPTION_MAX_ZOOM()) != 0) {
      options.maxZoom(mln_bound_options.max_zoom(segment));
    }
    if ((fields & MapVinaNativeC.MLN_BOUND_OPTION_MIN_PITCH()) != 0) {
      options.minPitch(mln_bound_options.min_pitch(segment));
    }
    if ((fields & MapVinaNativeC.MLN_BOUND_OPTION_MAX_PITCH()) != 0) {
  ...
  public static TileOptions tileOptions(MemorySegment segment) {
    ...
    if ((fields & MapVinaNativeC.MLN_MAP_TILE_OPTION_PREFETCH_ZOOM_DELTA()) != 0) {
      options.prefetchZoomDelta(mln_map_tile_options.prefetch_zoom_delta(segment));
    }
    if ((fields & MapVinaNativeC.MLN_MAP_TILE_OPTION_LOD_MIN_RADIUS()) != 0) {
      options.lodMinRadius(mln_map_tile_options.lod_min_radius(segment));
    }
    if ((fields & MapVinaNativeC.MLN_MAP_TILE_OPTION_LOD_SCALE()) != 0) {
      options.lodScale(mln_map_tile_options.lod_scale(segment));
    }
    if ((fields & MapVinaNativeC.MLN_MAP_TILE_OPTION_LOD_PITCH_THRESHOLD()) != 0) {
      options.lodPitchThreshold(mln_map_tile_options.lod_pitch_threshold(segment));
    }
    if ((fields & MapVinaNativeC.MLN_MAP_TILE_OPTION_LOD_ZOOM_SHIFT()) != 0) {
      options.lodZoomShift(mln_map_tile_options.lod_zoom_shift(segment));
    }
    if ((fields & MapVinaNativeC.MLN_MAP_TILE_OPTION_LOD_MODE()) != 0) {
  ...
  public static MemorySegment projectionModeOptions(ProjectionModeOptions options, Arena arena) {
    var segment = MapVinaNativeC.mln_projection_mode_default(arena);
    var fields = 0;
    if (options.hasAxonometric()) {
      fields |= MapVinaNativeC.MLN_PROJECTION_MODE_AXONOMETRIC();
      mln_projection_mode.axonometric(segment, options.axonometric());
    }
    if (options.hasXSkew()) {
      fields |= MapVinaNativeC.MLN_PROJECTION_MODE_X_SKEW();
      mln_projection_mode.x_skew(segment, options.xSkew());
    }
    if (options.hasYSkew()) {
      fields |= MapVinaNativeC.MLN_PROJECTION_MODE_Y_SKEW();
  ...
  public static ProjectionModeOptions projectionModeOptions(MemorySegment segment) {
    ...
    if ((fields & MapVinaNativeC.MLN_PROJECTION_MODE_AXONOMETRIC()) != 0) {
      options.axonometric(mln_projection_mode.axonometric(segment));
    }
    if ((fields & MapVinaNativeC.MLN_PROJECTION_MODE_X_SKEW()) != 0) {
      options.xSkew(mln_projection_mode.x_skew(segment));
    }
    if ((fields & MapVinaNativeC.MLN_PROJECTION_MODE_Y_SKEW()) != 0) {
  ...
  public static MemorySegment freeCameraOptions(FreeCameraOptions options, Arena arena) {
    var segment = MapVinaNativeC.mln_free_camera_options_default(arena);
    var fields = 0;
    if (options.hasPosition()) {
      fields |= MapVinaNativeC.MLN_FREE_CAMERA_OPTION_POSITION();
      mln_free_camera_options.position(segment, vec3(options.position(), arena));
    }
    if (options.hasOrientation()) {
      fields |= MapVinaNativeC.MLN_FREE_CAMERA_OPTION_ORIENTATION();
  ...
  public static FreeCameraOptions freeCameraOptions(MemorySegment segment) {
    ...
    if ((fields & MapVinaNativeC.MLN_FREE_CAMERA_OPTION_POSITION()) != 0) {
      options.position(vec3(mln_free_camera_options.position(segment)));
    }
    if ((fields & MapVinaNativeC.MLN_FREE_CAMERA_OPTION_ORIENTATION()) != 0) {
