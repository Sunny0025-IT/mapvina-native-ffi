...
import io.github.mapvina.nativeffi.internal.c.mln_offline_geometry_region_definition;
import io.github.mapvina.nativeffi.internal.c.mln_offline_region_definition;
import io.github.mapvina.nativeffi.internal.c.mln_offline_region_info;
import io.github.mapvina.nativeffi.internal.c.mln_offline_region_status;
import io.github.mapvina.nativeffi.internal.c.mln_offline_tile_pyramid_region_definition;
import io.github.mapvina.nativeffi.internal.c.mln_rendering_stats;
import io.github.mapvina.nativeffi.internal.c.mln_runtime_options;
import io.github.mapvina.nativeffi.internal.c.mln_tile_id;
...
import io.github.mapvina.nativeffi.runtime.RuntimeOptions;
...
public final class RuntimeStructs {
  ...
  public static MemorySegment runtimeOptions(RuntimeOptions options, Arena arena) {
    var segment = MapVinaNativeC.mln_runtime_options_default(arena);
    ...
    if (options.maximumCacheSize().isPresent()) {
      mln_runtime_options.flags(
          ...
          mln_runtime_options.flags(segment)
              | MapVinaNativeC.MLN_RUNTIME_OPTION_MAXIMUM_CACHE_SIZE());
  ...
  public static MemorySegment offlineRegionDefinition(
    ...
    if (definition instanceof OfflineRegionDefinition.TilePyramid tilePyramid) {
      mln_offline_region_definition.type(
          segment, MapVinaNativeC.MLN_OFFLINE_REGION_DEFINITION_TILE_PYRAMID());
    ...
    if (definition instanceof OfflineRegionDefinition.GeometryRegion geometryRegion) {
      mln_offline_region_definition.type(
          segment, MapVinaNativeC.MLN_OFFLINE_REGION_DEFINITION_GEOMETRY());
  ...
  public static OfflineRegionDefinition offlineRegionDefinition(MemorySegment segment) {
    ...
    var data = mln_offline_region_definition.data(segment);
    if (type == MapVinaNativeC.MLN_OFFLINE_REGION_DEFINITION_TILE_PYRAMID()) {
    ...
    if (type == MapVinaNativeC.MLN_OFFLINE_REGION_DEFINITION_GEOMETRY()) {
  ...
  public static OfflineRegionInfo offlineRegionSnapshot(MemorySegment snapshot) {
    try (var arena = Arena.ofConfined()) {
      ...
      Status.check(MapVinaNativeC.mln_offline_region_snapshot_get(snapshot, info));
      return offlineRegionInfo(info);
    } finally {
      MapVinaNativeC.mln_offline_region_snapshot_destroy(snapshot);
    }
  }
  ...
  public static List<OfflineRegionInfo> offlineRegionList(MemorySegment list) {
    try (var arena = Arena.ofConfined()) {
      ...
      Status.check(MapVinaNativeC.mln_offline_region_list_count(list, outCount));
      ...
      for (var index = 0; index < count; index++) {
        var info = mln_offline_region_info.allocate(arena);
        mln_offline_region_info.size(info, (int) mln_offline_region_info.sizeof());
        Status.check(MapVinaNativeC.mln_offline_region_list_get(list, index, info));
        regions.add(offlineRegionInfo(info));
      }
      return List.copyOf(regions);
    } finally {
      MapVinaNativeC.mln_offline_region_list_destroy(list);
    }
  }
}
