...
import static io.github.mapvina.nativeffi.internal.struct.CoreStructs.screenPointArray;
...
import io.github.mapvina.nativeffi.internal.c.mln_feature_extension_result_info;
import io.github.mapvina.nativeffi.internal.c.mln_feature_state_selector;
import io.github.mapvina.nativeffi.internal.c.mln_queried_feature;
import io.github.mapvina.nativeffi.internal.c.mln_rendered_feature_query_options;
import io.github.mapvina.nativeffi.internal.c.mln_screen_box;
import io.github.mapvina.nativeffi.internal.c.mln_source_feature_query_options;
...
import io.github.mapvina.nativeffi.query.FeatureExtensionResult;
import io.github.mapvina.nativeffi.query.FeatureStateSelector;
import io.github.mapvina.nativeffi.query.QueriedFeature;
import io.github.mapvina.nativeffi.query.RenderedFeatureQueryOptions;
import io.github.mapvina.nativeffi.query.RenderedQueryGeometry;
import io.github.mapvina.nativeffi.query.SourceFeatureQueryOptions;
...
public final class QueryStructs {
  ...
  public static MemorySegment renderedQueryGeometry(RenderedQueryGeometry geometry, Arena arena) {
    if (geometry instanceof RenderedQueryGeometry.Point point) {
      return MapVinaNativeC.mln_rendered_query_geometry_point(
    ...
    if (geometry instanceof RenderedQueryGeometry.Box box) {
      ...
      return MapVinaNativeC.mln_rendered_query_geometry_box(arena, nativeBox);
    }
    if (geometry instanceof RenderedQueryGeometry.LineString lineString) {
      var points = lineString.points();
      return MapVinaNativeC.mln_rendered_query_geometry_line_string(
          arena,
  ...
  public static MemorySegment renderedFeatureQueryOptions(
    ...
    var segment = MapVinaNativeC.mln_rendered_feature_query_options_default(arena);
    var fields = 0;
    if (options.hasLayerIds()) {
      fields |= MapVinaNativeC.MLN_RENDERED_FEATURE_QUERY_OPTION_LAYER_IDS();
  ...
  public static MemorySegment sourceFeatureQueryOptions(
    ...
    var segment = MapVinaNativeC.mln_source_feature_query_options_default(arena);
    var fields = 0;
    if (options.hasSourceLayerIds()) {
      fields |= MapVinaNativeC.MLN_SOURCE_FEATURE_QUERY_OPTION_SOURCE_LAYER_IDS();
  ...
  public static List<QueriedFeature> featureQueryResult(MemorySegment result) {
    try (var arena = Arena.ofConfined()) {
      ...
      Status.check(MapVinaNativeC.mln_feature_query_result_count(result, outCount));
      ...
      for (var index = 0; index < count; index++) {
        ...
        Status.check(MapVinaNativeC.mln_feature_query_result_get(result, index, outFeature));
        features.add(queriedFeature(outFeature));
      }
      return List.copyOf(features);
    } finally {
      MapVinaNativeC.mln_feature_query_result_destroy(result);
    }
  }
  ...
  public static FeatureExtensionResult featureExtensionResult(MemorySegment result) {
    try (var arena = Arena.ofConfined()) {
      ...
      MapVinaNativeC.mln_feature_extension_result_destroy(result);
    }
  }
