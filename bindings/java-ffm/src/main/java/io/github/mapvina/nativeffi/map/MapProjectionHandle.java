...
import io.github.mapvina.nativeffi.camera.EdgeInsets;
...
public final class MapProjectionHandle implements AutoCloseable {
  ...
  public static MapProjectionHandle create(MapHandle map) {
    ...
    try (var arena = Arena.ofConfined()) {
      ...
      Status.check(MapVinaNativeC.mln_map_projection_create(map.nativeHandle(), outProjection));
  ...
  public CameraOptions camera() {
    ...
    try (var arena = Arena.ofConfined()) {
      ...
      Status.check(MapVinaNativeC.mln_map_projection_get_camera(state.requireLive(), outCamera));
  ...
  public void setCamera(CameraOptions camera) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_projection_set_camera(
  ...
  public void setVisibleCoordinates(List<LatLng> coordinates, EdgeInsets padding) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_projection_set_visible_coordinates(
              state.requireLive(),
  ...
  public void setVisibleGeometry(Geometry geometry, EdgeInsets padding) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_projection_set_visible_geometry(
              state.requireLive(),
  ...
  public ScreenPoint pixelForLatLng(LatLng coordinate) {
    ...
    try (var arena = Arena.ofConfined()) {
      ...
      Status.check(
          MapVinaNativeC.mln_map_projection_pixel_for_lat_lng(
  ...
  public LatLng latLngForPixel(ScreenPoint point) {
    ...
    try (var arena = Arena.ofConfined()) {
      ...
      Status.check(
          MapVinaNativeC.mln_map_projection_lat_lng_for_pixel(
  ...
  public void close() {
    NativeAccess.ensureLoaded();
    state.closeOnce(MapVinaNativeC::mln_map_projection_destroy);
  }
