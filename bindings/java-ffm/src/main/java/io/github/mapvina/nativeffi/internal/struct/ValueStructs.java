...
import static io.github.mapvina.nativeffi.internal.struct.CoreStructs.latLng;
import static io.github.mapvina.nativeffi.internal.struct.CoreStructs.latLngArray;
import static io.github.mapvina.nativeffi.internal.struct.CoreStructs.sizedArray;
import static io.github.mapvina.nativeffi.internal.struct.CoreStructs.sizedPointer;
...
import io.github.mapvina.nativeffi.geo.Feature;
import io.github.mapvina.nativeffi.geo.FeatureIdentifier;
...
import io.github.mapvina.nativeffi.internal.c.mln_coordinate_span;
import io.github.mapvina.nativeffi.internal.c.mln_feature;
import io.github.mapvina.nativeffi.internal.c.mln_feature_collection;
import io.github.mapvina.nativeffi.internal.c.mln_geojson;
import io.github.mapvina.nativeffi.internal.c.mln_geometry;
import io.github.mapvina.nativeffi.internal.c.mln_geometry_collection;
import io.github.mapvina.nativeffi.internal.c.mln_json_array;
import io.github.mapvina.nativeffi.internal.c.mln_json_member;
import io.github.mapvina.nativeffi.internal.c.mln_json_object;
import io.github.mapvina.nativeffi.internal.c.mln_json_value;
import io.github.mapvina.nativeffi.internal.c.mln_lat_lng;
import io.github.mapvina.nativeffi.internal.c.mln_multi_line_geometry;
import io.github.mapvina.nativeffi.internal.c.mln_multi_polygon_geometry;
import io.github.mapvina.nativeffi.internal.c.mln_polygon_geometry;
...
public final class ValueStructs {
  ...
  public static Optional<JsonValue> jsonSnapshot(MemorySegment snapshot) {
    ...
    try (var arena = Arena.ofConfined()) {
      ...
      Status.check(MapVinaNativeC.mln_json_snapshot_get(snapshot, outValue));
      ...
      MapVinaNativeC.mln_json_snapshot_destroy(snapshot);
    }
  }
  ...
  private static void writeJsonValue(
    ...
    if (value instanceof JsonValue.Null) {
      ...
      mln_json_value.type(segment, MapVinaNativeC.MLN_JSON_VALUE_TYPE_ARRAY());
      ...
      mln_json_value.type(segment, MapVinaNativeC.MLN_JSON_VALUE_TYPE_OBJECT());
  ...
  private static JsonValue readJsonValue(MemorySegment segment, int depth) {
    ...
    if (type == MapVinaNativeC.MLN_JSON_VALUE_TYPE_NULL()) {
      return JsonValue.nullValue();
    }
    if (type == MapVinaNativeC.MLN_JSON_VALUE_TYPE_BOOL()) {
      return JsonValue.of(mln_json_value.data.bool_value(data));
    }
    if (type == MapVinaNativeC.MLN_JSON_VALUE_TYPE_UINT()) {
      return JsonValue.unsigned(mln_json_value.data.uint_value(data));
    }
    if (type == MapVinaNativeC.MLN_JSON_VALUE_TYPE_INT()) {
      return JsonValue.of(mln_json_value.data.int_value(data));
    }
    if (type == MapVinaNativeC.MLN_JSON_VALUE_TYPE_DOUBLE()) {
      return JsonValue.of(mln_json_value.data.double_value(data));
    }
    if (type == MapVinaNativeC.MLN_JSON_VALUE_TYPE_STRING()) {
      return JsonValue.of(stringView(mln_json_value.data.string_value(data)));
    }
    if (type == MapVinaNativeC.MLN_JSON_VALUE_TYPE_ARRAY()) {
      var array = mln_json_value.data.array_value(data);
    ...
    if (type == MapVinaNativeC.MLN_JSON_VALUE_TYPE_OBJECT()) {
      var object = mln_json_value.data.object_value(data);
  ...
  private static void writeGeometry(
    ...
    if (geometry instanceof Geometry.Empty) {
      mln_geometry.type(segment, MapVinaNativeC.MLN_GEOMETRY_TYPE_EMPTY());
    } else if (geometry instanceof Geometry.Point point) {
      mln_geometry.type(segment, MapVinaNativeC.MLN_GEOMETRY_TYPE_POINT());
      mln_geometry.data.point(data, latLng(point.coordinate(), arena));
    } else if (geometry instanceof Geometry.LineString lineString) {
      mln_geometry.type(segment, MapVinaNativeC.MLN_GEOMETRY_TYPE_LINE_STRING());
      ...
      mln_geometry.type(segment, MapVinaNativeC.MLN_GEOMETRY_TYPE_POLYGON());
      ...
      mln_geometry.type(segment, MapVinaNativeC.MLN_GEOMETRY_TYPE_MULTI_POINT());
      ...
      mln_geometry.type(segment, MapVinaNativeC.MLN_GEOMETRY_TYPE_MULTI_LINE_STRING());
      ...
      mln_geometry.type(segment, MapVinaNativeC.MLN_GEOMETRY_TYPE_MULTI_POLYGON());
      ...
      mln_geometry.type(segment, MapVinaNativeC.MLN_GEOMETRY_TYPE_GEOMETRY_COLLECTION());
  ...
  private static void writeFeatureIdentifier(
    ...
    if (identifier instanceof FeatureIdentifier.Null) {
      mln_feature.identifier_type(segment, MapVinaNativeC.MLN_FEATURE_IDENTIFIER_TYPE_NULL());
    } else if (identifier instanceof FeatureIdentifier.UInt value) {
      mln_feature.identifier_type(segment, MapVinaNativeC.MLN_FEATURE_IDENTIFIER_TYPE_UINT());
      mln_feature.identifier.uint_value(data, value.value());
    } else if (identifier instanceof FeatureIdentifier.Int value) {
      mln_feature.identifier_type(segment, MapVinaNativeC.MLN_FEATURE_IDENTIFIER_TYPE_INT());
      mln_feature.identifier.int_value(data, value.value());
    } else if (identifier instanceof FeatureIdentifier.DoubleValue value) {
      mln_feature.identifier_type(segment, MapVinaNativeC.MLN_FEATURE_IDENTIFIER_TYPE_DOUBLE());
      mln_feature.identifier.double_value(data, value.value());
    } else if (identifier instanceof FeatureIdentifier.StringValue value) {
      mln_feature.identifier_type(segment, MapVinaNativeC.MLN_FEATURE_IDENTIFIER_TYPE_STRING());
  ...
  private static FeatureIdentifier readFeatureIdentifier(MemorySegment segment) {
    ...
    if (type == MapVinaNativeC.MLN_FEATURE_IDENTIFIER_TYPE_NULL()) {
      return FeatureIdentifier.nullValue();
    }
    if (type == MapVinaNativeC.MLN_FEATURE_IDENTIFIER_TYPE_UINT()) {
      return FeatureIdentifier.unsigned(mln_feature.identifier.uint_value(data));
    }
    if (type == MapVinaNativeC.MLN_FEATURE_IDENTIFIER_TYPE_INT()) {
      return FeatureIdentifier.of(mln_feature.identifier.int_value(data));
    }
    if (type == MapVinaNativeC.MLN_FEATURE_IDENTIFIER_TYPE_DOUBLE()) {
      return FeatureIdentifier.of(mln_feature.identifier.double_value(data));
    }
    if (type == MapVinaNativeC.MLN_FEATURE_IDENTIFIER_TYPE_STRING()) {
  ...
  private static void writeGeoJson(MemorySegment segment, GeoJson geoJson, Arena arena) {
    ...
    if (geoJson instanceof GeoJson.GeometryValue geometryValue) {
      mln_geojson.type(segment, MapVinaNativeC.MLN_GEOJSON_TYPE_GEOMETRY());
      ...
      mln_geojson.type(segment, MapVinaNativeC.MLN_GEOJSON_TYPE_FEATURE());
      ...
      mln_geojson.type(segment, MapVinaNativeC.MLN_GEOJSON_TYPE_FEATURE_COLLECTION());
  ...
  private static GeoJson readGeoJson(MemorySegment segment) {
    ...
    if (type == MapVinaNativeC.MLN_GEOJSON_TYPE_GEOMETRY()) {
    ...
    if (type == MapVinaNativeC.MLN_GEOJSON_TYPE_FEATURE()) {
      return GeoJson.feature(
          readFeature(
              sizedPointer(mln_geojson.data.feature(data), mln_feature.sizeof(), "GeoJSON feature"),
              0));
    }
    if (type == MapVinaNativeC.MLN_GEOJSON_TYPE_FEATURE_COLLECTION()) {
