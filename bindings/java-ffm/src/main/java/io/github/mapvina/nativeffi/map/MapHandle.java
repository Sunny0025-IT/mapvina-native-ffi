package io.github.mapvina.nativeffi.map;
...
import io.github.mapvina.nativeffi.camera.AnimationOptions;
import io.github.mapvina.nativeffi.camera.BoundOptions;
import io.github.mapvina.nativeffi.camera.CameraFitOptions;
import io.github.mapvina.nativeffi.camera.CameraOptions;
import io.github.mapvina.nativeffi.camera.FreeCameraOptions;
import io.github.mapvina.nativeffi.geo.CanonicalTileId;
import io.github.mapvina.nativeffi.geo.GeoJson;
...
import io.github.mapvina.nativeffi.geo.ScreenPoint;
import io.github.mapvina.nativeffi.internal.access.InternalAccess;
import io.github.mapvina.nativeffi.internal.c.MapVinaNativeC;
import io.github.mapvina.nativeffi.internal.c.mln_lat_lng;
import io.github.mapvina.nativeffi.internal.c.mln_lat_lng_bounds;
import io.github.mapvina.nativeffi.internal.c.mln_screen_point;
import io.github.mapvina.nativeffi.internal.c.mln_style_source_info;
import io.github.mapvina.nativeffi.internal.lifecycle.HandleState;
...
import io.github.mapvina.nativeffi.internal.struct.MapStructs;
import io.github.mapvina.nativeffi.internal.struct.StyleStructs;
import io.github.mapvina.nativeffi.internal.struct.ValueStructs;
...
import io.github.mapvina.nativeffi.render.RenderSessionHandle;
...
import io.github.mapvina.nativeffi.runtime.RuntimeHandle;
import io.github.mapvina.nativeffi.style.CustomGeometrySourceOptions;
import io.github.mapvina.nativeffi.style.LocationIndicatorImageKind;
import io.github.mapvina.nativeffi.style.SourceInfo;
import io.github.mapvina.nativeffi.style.SourceType;
import io.github.mapvina.nativeffi.style.StyleImage;
...
public final class MapHandle implements AutoCloseable {
  ...
  public void setStyleUrl(String url) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_set_style_url(
  ...
  public void setStyleJson(String json) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_set_style_json(
              state.requireLive(),
  ...
  public void addStyleSourceJson(String sourceId, JsonValue sourceJson) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_add_style_source_json(
              state.requireLive(),
  ...
  public boolean removeStyleSource(String sourceId) {
    ...
    try (var arena = Arena.ofConfined()) {
      ...
      Status.check(
          MapVinaNativeC.mln_map_remove_style_source(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(sourceId, "sourceId"), arena),
              outRemoved));
  ...
  public boolean styleSourceExists(String sourceId) {
    ...
    try (var arena = Arena.ofConfined()) {
      ...
      Status.check(
          MapVinaNativeC.mln_map_style_source_exists(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(sourceId, "sourceId"), arena),
              outExists));
  ...
  public Optional<SourceType> styleSourceType(String sourceId) {
    ...
    try (var arena = Arena.ofConfined()) {
      ...
      Status.check(
          MapVinaNativeC.mln_map_get_style_source_type(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(sourceId, "sourceId"), arena),
              outType,
              outFound));
  ...
  public Optional<SourceInfo> styleSourceInfo(String sourceId) {
    ...
    try (var arena = Arena.ofConfined()) {
      ...
      var outFound = arena.allocate(ValueLayout.JAVA_BOOLEAN);
      Status.check(
          MapVinaNativeC.mln_map_get_style_source_info(
      ...
      if (mln_style_source_info.has_attribution(outInfo)) {
        ...
        if (attributionSize == 0) {
          ...
          Status.check(
              MapVinaNativeC.mln_map_copy_style_source_attribution(
                  state.requireLive(),
                  sourceIdView,
                  outAttribution,
                  attributionSize,
  ...
  public List<String> styleSourceIds() {
    ...
    try (var arena = Arena.ofConfined()) {
      ...
      Status.check(MapVinaNativeC.mln_map_list_style_source_ids(state.requireLive(), outList));
  ...
  public void addGeoJsonSourceUrl(String sourceId, String url) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_add_geojson_source_url(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(sourceId, "sourceId"), arena),
  ...
  public void addGeoJsonSourceData(String sourceId, GeoJson data) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_add_geojson_source_data(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(sourceId, "sourceId"), arena),
  ...
  public void setGeoJsonSourceUrl(String sourceId, String url) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_set_geojson_source_url(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(sourceId, "sourceId"), arena),
              CoreStructs.stringView(Objects.requireNonNull(url, "url"), arena)));
    }
  ...
  public void setGeoJsonSourceData(String sourceId, GeoJson data) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_set_geojson_source_data(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(sourceId, "sourceId"), arena),
              ValueStructs.geoJson(data, arena)));
    }
  ...
  public void addCustomGeometrySource(String sourceId, CustomGeometrySourceOptions options) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_add_custom_geometry_source(
  ...
  public void setCustomGeometrySourceTileData(
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_set_custom_geometry_source_tile_data(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(sourceId, "sourceId"), arena),
  ...
  public void invalidateCustomGeometrySourceTile(String sourceId, CanonicalTileId tileId) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_invalidate_custom_geometry_source_tile(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(sourceId, "sourceId"), arena),
              StyleStructs.canonicalTileId(tileId, arena)));
    }
  ...
  public void invalidateCustomGeometrySourceRegion(String sourceId, LatLngBounds bounds) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_invalidate_custom_geometry_source_region(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(sourceId, "sourceId"), arena),
  ...
  public boolean removeStyleImage(String imageId) {
    ...
    try (var arena = Arena.ofConfined()) {
      var outRemoved = arena.allocate(ValueLayout.JAVA_BOOLEAN);
      Status.check(
          MapVinaNativeC.mln_map_remove_style_image(
              state.requireLive(),
  ...
  public boolean styleImageExists(String imageId) {
    ...
    try (var arena = Arena.ofConfined()) {
      var outExists = arena.allocate(ValueLayout.JAVA_BOOLEAN);
      Status.check(
          MapVinaNativeC.mln_map_style_image_exists(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(imageId, "imageId"), arena),
              outExists));
      return outExists.get(ValueLayout.JAVA_BOOLEAN, 0);
  ...
  public Optional<StyleImageInfo> styleImageInfo(String imageId) {
    NativeAccess.ensureLoaded();
    try (var arena = Arena.ofConfined()) {
      var outInfo = MapVinaNativeC.mln_style_image_info_default(arena);
      var outFound = arena.allocate(ValueLayout.JAVA_BOOLEAN);
      Status.check(
          MapVinaNativeC.mln_map_get_style_image_info(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(imageId, "imageId"), arena),
              outInfo,
              outFound));
  ...
  public Optional<StyleImage> copyStyleImagePremultipliedRgba8(String imageId) {
    ...
    try (var arena = Arena.ofConfined()) {
      ...
      var outFound = arena.allocate(ValueLayout.JAVA_BOOLEAN);
      Status.check(
          MapVinaNativeC.mln_map_copy_style_image_premultiplied_rgba8(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(imageId, "imageId"), arena),
              outPixels,
              byteLength,
  ...
  public void addImageSourceUrl(String sourceId, List<LatLng> coordinates, String url) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_add_image_source_url(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(sourceId, "sourceId"), arena),
  ...
  public void addImageSourceImage(
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_add_image_source_image(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(sourceId, "sourceId"), arena),
              StyleStructs.imageSourceCoordinates(copiedCoordinates, arena),
              copiedCoordinates.size(),
  ...
  public void setImageSourceUrl(String sourceId, String url) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_set_image_source_url(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(sourceId, "sourceId"), arena),
              CoreStructs.stringView(Objects.requireNonNull(url, "url"), arena)));
    }
  ...
  public void setImageSourceImage(String sourceId, PremultipliedRgba8Image image) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_set_image_source_image(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(sourceId, "sourceId"), arena),
              StyleStructs.premultipliedRgba8Image(image, arena)));
    }
  ...
  public void setImageSourceCoordinates(String sourceId, List<LatLng> coordinates) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_set_image_source_coordinates(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(sourceId, "sourceId"), arena),
              StyleStructs.imageSourceCoordinates(copiedCoordinates, arena),
              copiedCoordinates.size()));
  ...
  public Optional<List<LatLng>> imageSourceCoordinates(String sourceId) {
    ...
    try (var arena = Arena.ofConfined()) {
      ...
      var outFound = arena.allocate(ValueLayout.JAVA_BOOLEAN);
      Status.check(
          MapVinaNativeC.mln_map_get_image_source_coordinates(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(sourceId, "sourceId"), arena),
              outCoordinates,
              4,
  ...
  public void addStyleLayerJson(JsonValue layerJson, String beforeLayerId) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_add_style_layer_json(
              state.requireLive(),
  ...
  public void addHillshadeLayer(String layerId, String sourceId, String beforeLayerId) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_add_hillshade_layer(
              state.requireLive(),
  ...
  public void addColorReliefLayer(String layerId, String sourceId, String beforeLayerId) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_add_color_relief_layer(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(layerId, "layerId"), arena),
              CoreStructs.stringView(Objects.requireNonNull(sourceId, "sourceId"), arena),
              CoreStructs.stringView(
  ...
  public void addLocationIndicatorLayer(String layerId, String beforeLayerId) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_add_location_indicator_layer(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(layerId, "layerId"), arena),
              CoreStructs.stringView(
                  Objects.requireNonNull(beforeLayerId, "beforeLayerId"), arena)));
  ...
  public void setLocationIndicatorLocation(String layerId, LatLng coordinate, double altitude) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_set_location_indicator_location(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(layerId, "layerId"), arena),
  ...
  public void setLocationIndicatorBearing(String layerId, double bearing) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_set_location_indicator_bearing(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(layerId, "layerId"), arena),
              bearing));
    }
  ...
  public void setLocationIndicatorAccuracyRadius(String layerId, double radius) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_set_location_indicator_accuracy_radius(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(layerId, "layerId"), arena),
              radius));
    }
  ...
  public void setLocationIndicatorImageName(
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_set_location_indicator_image_name(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(layerId, "layerId"), arena),
  ...
  public boolean removeStyleLayer(String layerId) {
    ...
    try (var arena = Arena.ofConfined()) {
      var outRemoved = arena.allocate(ValueLayout.JAVA_BOOLEAN);
      Status.check(
          MapVinaNativeC.mln_map_remove_style_layer(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(layerId, "layerId"), arena),
              outRemoved));
      return outRemoved.get(ValueLayout.JAVA_BOOLEAN, 0);
  ...
  public boolean styleLayerExists(String layerId) {
    ...
    try (var arena = Arena.ofConfined()) {
      var outExists = arena.allocate(ValueLayout.JAVA_BOOLEAN);
      Status.check(
          MapVinaNativeC.mln_map_style_layer_exists(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(layerId, "layerId"), arena),
              outExists));
      return outExists.get(ValueLayout.JAVA_BOOLEAN, 0);
  ...
  public Optional<String> styleLayerType(String layerId) {
    NativeAccess.ensureLoaded();
    try (var arena = Arena.ofConfined()) {
      var outType = io.github.mapvina.nativeffi.internal.c.mln_string_view.allocate(arena);
      var outFound = arena.allocate(ValueLayout.JAVA_BOOLEAN);
      Status.check(
          MapVinaNativeC.mln_map_get_style_layer_type(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(layerId, "layerId"), arena),
              outType,
              outFound));
  ...
  public List<String> styleLayerIds() {
    ...
    try (var arena = Arena.ofConfined()) {
      var outList = MemoryUtil.allocatePointer(arena);
      Status.check(MapVinaNativeC.mln_map_list_style_layer_ids(state.requireLive(), outList));
      return StyleStructs.styleIdList(outList.get(ValueLayout.ADDRESS, 0));
    }
  }
  ...
  public void moveStyleLayer(String layerId, String beforeLayerId) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_move_style_layer(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(layerId, "layerId"), arena),
              CoreStructs.stringView(
                  Objects.requireNonNull(beforeLayerId, "beforeLayerId"), arena)));
  ...
  public Optional<JsonValue> styleLayerJson(String layerId) {
    ...
    try (var arena = Arena.ofConfined()) {
      ...
      var outFound = arena.allocate(ValueLayout.JAVA_BOOLEAN);
      Status.check(
          MapVinaNativeC.mln_map_get_style_layer_json(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(layerId, "layerId"), arena),
              outLayer,
              outFound));
  ...
  public void setStyleLightJson(JsonValue lightJson) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_set_style_light_json(
  ...
  public void setStyleLightProperty(String propertyName, JsonValue value) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_set_style_light_property(
              state.requireLive(),
  ...
  public Optional<JsonValue> styleLightProperty(String propertyName) {
    ...
    try (var arena = Arena.ofConfined()) {
      ...
      Status.check(
          MapVinaNativeC.mln_map_get_style_light_property(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(propertyName, "propertyName"), arena),
              outValue));
  ...
  public void setLayerProperty(String layerId, String propertyName, JsonValue value) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_set_layer_property(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(layerId, "layerId"), arena),
              CoreStructs.stringView(Objects.requireNonNull(propertyName, "propertyName"), arena),
              ValueStructs.jsonValue(value, arena)));
  ...
  public Optional<JsonValue> layerProperty(String layerId, String propertyName) {
    ...
    try (var arena = Arena.ofConfined()) {
      var outValue = MemoryUtil.allocatePointer(arena);
      Status.check(
          MapVinaNativeC.mln_map_get_layer_property(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(layerId, "layerId"), arena),
              CoreStructs.stringView(Objects.requireNonNull(propertyName, "propertyName"), arena),
              outValue));
  ...
  public Optional<JsonValue> layerFilter(String layerId) {
    ...
    try (var arena = Arena.ofConfined()) {
      ...
      Status.check(
          MapVinaNativeC.mln_map_get_layer_filter(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(layerId, "layerId"), arena),
              outFilter));
  ...
  public void requestRepaint() {
    NativeAccess.ensureLoaded();
    Status.check(MapVinaNativeC.mln_map_request_repaint(state.requireLive()));
  }

  public void requestStillImage() {
    NativeAccess.ensureLoaded();
    Status.check(MapVinaNativeC.mln_map_request_still_image(state.requireLive()));
  }
  ...
  public void setDebugOptions(Set<DebugOption> options) {
    ...
    Status.check(MapVinaNativeC.mln_map_set_debug_options(state.requireLive(), mask));
  }
  ...
  public EnumSet<DebugOption> debugOptions() {
    ...
    try (var arena = Arena.ofConfined()) {
      ...
      Status.check(MapVinaNativeC.mln_map_get_debug_options(state.requireLive(), outOptions));
  ...
  public void setRenderingStatsViewEnabled(boolean enabled) {
    NativeAccess.ensureLoaded();
    Status.check(
        MapVinaNativeC.mln_map_set_rendering_stats_view_enabled(state.requireLive(), enabled));
  }
  ...
  public boolean isRenderingStatsViewEnabled() {
    ...
    try (var arena = Arena.ofConfined()) {
      ...
      Status.check(
          MapVinaNativeC.mln_map_get_rendering_stats_view_enabled(
  ...
  public boolean isFullyLoaded() {
    ...
    try (var arena = Arena.ofConfined()) {
      ...
      Status.check(MapVinaNativeC.mln_map_is_fully_loaded(state.requireLive(), outLoaded));
  ...
  public void dumpDebugLogs() {
    NativeAccess.ensureLoaded();
    Status.check(MapVinaNativeC.mln_map_dump_debug_logs(state.requireLive()));
  }

  public ViewportOptions viewportOptions() {
    NativeAccess.ensureLoaded();
    try (var arena = Arena.ofConfined()) {
      var outOptions = MapVinaNativeC.mln_map_viewport_options_default(arena);
      Status.check(MapVinaNativeC.mln_map_get_viewport_options(state.requireLive(), outOptions));
  ...
  public void setViewportOptions(ViewportOptions options) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_set_viewport_options(
  ...
  public TileOptions tileOptions() {
    NativeAccess.ensureLoaded();
    try (var arena = Arena.ofConfined()) {
      var outOptions = MapVinaNativeC.mln_map_tile_options_default(arena);
      Status.check(MapVinaNativeC.mln_map_get_tile_options(state.requireLive(), outOptions));
  ...
  public void setTileOptions(TileOptions options) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_set_tile_options(
  ...
  public CameraOptions camera() {
    NativeAccess.ensureLoaded();
    try (var arena = Arena.ofConfined()) {
      var outCamera = MapVinaNativeC.mln_camera_options_default(arena);
      Status.check(MapVinaNativeC.mln_map_get_camera(state.requireLive(), outCamera));
  ...
  public void jumpTo(CameraOptions camera) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_jump_to(
  ...
  public void moveBy(double deltaX, double deltaY) {
    NativeAccess.ensureLoaded();
    Status.check(MapVinaNativeC.mln_map_move_by(state.requireLive(), deltaX, deltaY));
  }
  ...
  public void rotateBy(ScreenPoint first, ScreenPoint second) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_rotate_by(
              state.requireLive(),
  ...
  public void pitchBy(double pitch) {
    NativeAccess.ensureLoaded();
    Status.check(MapVinaNativeC.mln_map_pitch_by(state.requireLive(), pitch));
  }
  ...
  public void cancelTransitions() {
    NativeAccess.ensureLoaded();
    Status.check(MapVinaNativeC.mln_map_cancel_transitions(state.requireLive()));
  }
  ...
  public LatLngBounds latLngBoundsForCamera(CameraOptions camera) {
    ...
    try (var arena = Arena.ofConfined()) {
      ...
      Status.check(
          MapVinaNativeC.mln_map_lat_lng_bounds_for_camera(
  ...
  public LatLngBounds latLngBoundsForCameraUnwrapped(CameraOptions camera) {
    ...
    try (var arena = Arena.ofConfined()) {
      var outBounds = mln_lat_lng_bounds.allocate(arena);
      Status.check(
          MapVinaNativeC.mln_map_lat_lng_bounds_for_camera_unwrapped(
              state.requireLive(), MapStructs.cameraOptions(camera, arena), outBounds));
      return CoreStructs.latLngBounds(outBounds);
    }
  }
  ...
  public BoundOptions bounds() {
    NativeAccess.ensureLoaded();
    try (var arena = Arena.ofConfined()) {
      var outOptions = MapVinaNativeC.mln_bound_options_default(arena);
      Status.check(MapVinaNativeC.mln_map_get_bounds(state.requireLive(), outOptions));
  ...
  public void setBounds(BoundOptions options) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_set_bounds(
  ...
  public FreeCameraOptions freeCameraOptions() {
    NativeAccess.ensureLoaded();
    try (var arena = Arena.ofConfined()) {
      var outOptions = MapVinaNativeC.mln_free_camera_options_default(arena);
      Status.check(
          MapVinaNativeC.mln_map_get_free_camera_options(state.requireLive(), outOptions));
  ...
  public void setFreeCameraOptions(FreeCameraOptions options) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_set_free_camera_options(
  ...
  public ProjectionModeOptions projectionMode() {
    NativeAccess.ensureLoaded();
    try (var arena = Arena.ofConfined()) {
      var outMode = MapVinaNativeC.mln_projection_mode_default(arena);
      Status.check(MapVinaNativeC.mln_map_get_projection_mode(state.requireLive(), outMode));
  ...
  public void setProjectionMode(ProjectionModeOptions mode) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_set_projection_mode(
  ...
  public ScreenPoint pixelForLatLng(LatLng coordinate) {
    ...
    try (var arena = Arena.ofConfined()) {
      ...
      Status.check(
          MapVinaNativeC.mln_map_pixel_for_lat_lng(
  ...
  public LatLng latLngForPixel(ScreenPoint point) {
    ...
    try (var arena = Arena.ofConfined()) {
      ...
      Status.check(
          MapVinaNativeC.mln_map_lat_lng_for_pixel(
  ...
  public List<ScreenPoint> pixelsForLatLngs(List<LatLng> coordinates) {
    ...
    try (var arena = Arena.ofConfined()) {
      ...
      Status.check(
          MapVinaNativeC.mln_map_pixels_for_lat_lngs(
              state.requireLive(),
              copiedCoordinates.isEmpty()
                  ? MemorySegment.NULL
  ...
  public List<LatLng> latLngsForPixels(List<ScreenPoint> points) {
    ...
    try (var arena = Arena.ofConfined()) {
      ...
      Status.check(
          MapVinaNativeC.mln_map_lat_lngs_for_pixels(
              state.requireLive(),
              copiedPoints.isEmpty()
                  ? MemorySegment.NULL
  ...
  private void addVectorSourceUrlInternal(
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_add_vector_source_url(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(sourceId, "sourceId"), arena),
              CoreStructs.stringView(Objects.requireNonNull(url, "url"), arena),
  ...
  private void addVectorSourceTilesInternal(
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_add_vector_source_tiles(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(sourceId, "sourceId"), arena),
              copiedTiles.isEmpty()
                  ? MemorySegment.NULL
  ...
  private void addRasterSourceUrlInternal(
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_add_raster_source_url(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(sourceId, "sourceId"), arena),
              CoreStructs.stringView(Objects.requireNonNull(url, "url"), arena),
              hasOptions ? StyleStructs.tileSourceOptions(options, arena) : MemorySegment.NULL));
  ...
  private void addRasterSourceTilesInternal(
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_add_raster_source_tiles(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(sourceId, "sourceId"), arena),
              copiedTiles.isEmpty()
                  ? MemorySegment.NULL
  ...
  private void addRasterDemSourceUrlInternal(
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_add_raster_dem_source_url(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(sourceId, "sourceId"), arena),
              CoreStructs.stringView(Objects.requireNonNull(url, "url"), arena),
              hasOptions ? StyleStructs.tileSourceOptions(options, arena) : MemorySegment.NULL));
  ...
  private void addRasterDemSourceTilesInternal(
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_add_raster_dem_source_tiles(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(sourceId, "sourceId"), arena),
              copiedTiles.isEmpty()
                  ? MemorySegment.NULL
  ...
  private void setStyleImageInternal(
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_set_style_image(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(imageId, "imageId"), arena),
              StyleStructs.premultipliedRgba8Image(image, arena),
  ...
  private void setLayerFilterInternal(String layerId, JsonValue filter, boolean hasFilter) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_set_layer_filter(
              state.requireLive(),
              CoreStructs.stringView(Objects.requireNonNull(layerId, "layerId"), arena),
  ...
  private void easeToInternal(
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_ease_to(
              state.requireLive(),
  ...
  private void moveByAnimatedInternal(
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_move_by_animated(
              state.requireLive(),
              deltaX,
              deltaY,
              hasAnimation ? MapStructs.animationOptions(animation, arena) : MemorySegment.NULL));
  ...
  private void scaleByInternal(double scale, ScreenPoint anchor, boolean hasAnchor) {
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_scale_by(
              state.requireLive(),
              scale,
  ...
  private void scaleByAnimatedInternal(
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_scale_by_animated(
              state.requireLive(),
              scale,
              hasAnchor ? CoreStructs.screenPoint(anchor, arena) : MemorySegment.NULL,
              hasAnimation ? MapStructs.animationOptions(animation, arena) : MemorySegment.NULL));
  ...
  private void rotateByAnimatedInternal(
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_rotate_by_animated(
              state.requireLive(),
              CoreStructs.screenPoint(first, arena),
              CoreStructs.screenPoint(second, arena),
              hasAnimation ? MapStructs.animationOptions(animation, arena) : MemorySegment.NULL));
  ...
  private void pitchByAnimatedInternal(
    ...
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_map_pitch_by_animated(
              state.requireLive(),
              pitch,
              hasAnimation ? MapStructs.animationOptions(animation, arena) : MemorySegment.NULL));
    }
  ...
  private CameraOptions cameraForLatLngBoundsInternal(
    ...
    try (var arena = Arena.ofConfined()) {
      var outCamera = MapVinaNativeC.mln_camera_options_default(arena);
      Status.check(
          MapVinaNativeC.mln_map_camera_for_lat_lng_bounds(
              state.requireLive(),
              CoreStructs.latLngBounds(bounds, arena),
  ...
  private CameraOptions cameraForLatLngsInternal(
    ...
    try (var arena = Arena.ofConfined()) {
      var outCamera = MapVinaNativeC.mln_camera_options_default(arena);
      Status.check(
          MapVinaNativeC.mln_map_camera_for_lat_lngs(
              state.requireLive(),
              copiedCoordinates.isEmpty()
                  ? MemorySegment.NULL
                  : CoreStructs.latLngArray(copiedCoordinates, arena),
  ...
  private CameraOptions cameraForGeometryInternal(
    ...
    try (var arena = Arena.ofConfined()) {
      var outCamera = MapVinaNativeC.mln_camera_options_default(arena);
      Status.check(
          MapVinaNativeC.mln_map_camera_for_geometry(
              state.requireLive(),
  ...
  public void close() {
    NativeAccess.ensureLoaded();
    state.closeOnce(
        MapVinaNativeC::mln_map_destroy,
        () -> {
          clearCustomGeometrySources();
  ...
  void releaseDetachedCustomGeometrySources() {
    try (var arena = Arena.ofConfined()) {
      ...
      while (iterator.hasNext()) {
        ...
        if (status != MapVinaNativeC.MLN_STATUS_OK()) {
          continue;
        }
