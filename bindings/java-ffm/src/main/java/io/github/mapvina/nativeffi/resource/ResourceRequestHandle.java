...
public final class ResourceRequestHandle implements AutoCloseable {
  ...
  ResourceRequestHandle(MemorySegment handle) {
    this(handle, MapVinaNativeC::mln_resource_request_release);
  }
  ...
  public synchronized void complete(ResourceResponse response) {
    ...
    if (completed) {
      throw new InvalidStateException(
          MapvinaStatus.INVALID_STATE.nativeCode(), "ResourceRequestHandle is already completed");
    }
    requireLive();
    try (var arena = Arena.ofConfined()) {
      Status.check(
          MapVinaNativeC.mln_resource_request_complete(
  ...
  public synchronized boolean isCancelled() {
    ...
    try (var arena = Arena.ofConfined()) {
      ...
      Status.check(MapVinaNativeC.mln_resource_request_cancelled(handle, outCancelled));
