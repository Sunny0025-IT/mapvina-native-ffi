...
import io.github.mapvina.nativeffi.log.LogEvent;
...
public final class LogCallbackState implements AutoCloseable {
  ...
  public static void set(LogCallback callback) {
    ...
    try {
      synchronized (LOCK) {
        Status.check(MapVinaNativeC.mln_log_set_callback(replacement.stub, MemorySegment.NULL));
        previous = current;
        current = replacement;
      }
  ...
  public static void clear() {
    ...
    LogCallbackState previous;
    synchronized (LOCK) {
      Status.check(MapVinaNativeC.mln_log_clear_callback());
      previous = current;
      current = null;
    }
    closeQuietly(previous);
