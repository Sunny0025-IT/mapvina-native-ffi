...
import io.github.mapvina.nativeffi.internal.c.mln_resource_provider;
import io.github.mapvina.nativeffi.internal.c.mln_resource_provider_callback;
import io.github.mapvina.nativeffi.internal.c.mln_resource_request;
import io.github.mapvina.nativeffi.internal.memory.MemoryUtil;
import io.github.mapvina.nativeffi.resource.ResourceKind;
import io.github.mapvina.nativeffi.resource.ResourceLoadingMethod;
import io.github.mapvina.nativeffi.resource.ResourcePriority;
import io.github.mapvina.nativeffi.resource.ResourceProviderDecision;
import io.github.mapvina.nativeffi.resource.ResourceRequest;
import io.github.mapvina.nativeffi.resource.ResourceStoragePolicy;
import io.github.mapvina.nativeffi.resource.ResourceUsage;
...
final class ResourceProviderStateTest {
  ...
  void copiesProviderRequestBeforeInvokingCallback() {
    ...
    assertEquals(MapVinaNativeC.MLN_RESOURCE_PROVIDER_DECISION_PASS_THROUGH(), decision);
    assertEquals("https://example.test/tile.pbf", seen.get().url());
    assertEquals(ResourceKind.TILE, seen.get().kind());
    assertEquals(MapVinaNativeC.MLN_RESOURCE_KIND_TILE(), seen.get().rawKind());
    assertEquals(ResourceLoadingMethod.NETWORK_ONLY, seen.get().loadingMethod());
    assertEquals(
        MapVinaNativeC.MLN_RESOURCE_LOADING_METHOD_NETWORK_ONLY(), seen.get().rawLoadingMethod());
    assertEquals(ResourcePriority.LOW, seen.get().priority());
    assertEquals(MapVinaNativeC.MLN_RESOURCE_PRIORITY_LOW(), seen.get().rawPriority());
    assertEquals(ResourceUsage.OFFLINE, seen.get().usage());
    assertEquals(MapVinaNativeC.MLN_RESOURCE_USAGE_OFFLINE(), seen.get().rawUsage());
    assertEquals(ResourceStoragePolicy.VOLATILE, seen.get().storagePolicy());
    assertEquals(
        MapVinaNativeC.MLN_RESOURCE_STORAGE_POLICY_VOLATILE(), seen.get().rawStoragePolicy());
  ...
  void closedPassThroughHandleDefersReleaseToNativeDecision() {
    try (var state =
        ...
        var arena = Arena.ofConfined()) {
      assertEquals(
          MapVinaNativeC.MLN_RESOURCE_PROVIDER_DECISION_PASS_THROUGH(),
  ...
  private static MemorySegment request(Arena arena) {
    ...
    mln_resource_request.kind(request, MapVinaNativeC.MLN_RESOURCE_KIND_TILE());
    mln_resource_request.loading_method(
        request, MapVinaNativeC.MLN_RESOURCE_LOADING_METHOD_NETWORK_ONLY());
    mln_resource_request.priority(request, MapVinaNativeC.MLN_RESOURCE_PRIORITY_LOW());
    mln_resource_request.usage(request, MapVinaNativeC.MLN_RESOURCE_USAGE_OFFLINE());
    mln_resource_request.storage_policy(
        request, MapVinaNativeC.MLN_RESOURCE_STORAGE_POLICY_VOLATILE());
