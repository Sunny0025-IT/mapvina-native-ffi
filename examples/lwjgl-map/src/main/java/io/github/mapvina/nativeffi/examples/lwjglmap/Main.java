...
public final class Main {
  ...
  public static void main(String[] args) throws Exception {
    ...
    if (!Mapvina.supportedRenderBackends().contains(RenderBackend.VULKAN)) {
      throw new IllegalStateException("The loaded MapVina native library does not support Vulkan");
    }
    System.out.println("lwjgl-map render target: " + mode.cliName());
    System.out.println("render target status: " + mode.status());
    var propertyPath = System.getProperty("io.github.mapvina.nativeffi.library.path");
    if (propertyPath != null) {
      System.out.println("MapVina native library: " + propertyPath);
    }

    try (var vulkan = VulkanContext.create("MapVina LWJGL Map", 1280, 720)) {
