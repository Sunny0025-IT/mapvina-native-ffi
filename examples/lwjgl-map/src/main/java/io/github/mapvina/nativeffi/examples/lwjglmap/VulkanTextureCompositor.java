...
import io.github.mapvina.nativeffi.render.VulkanOwnedTextureFrameHandle;
...
final class VulkanTextureCompositor implements AutoCloseable {
  ...
  void draw(VulkanOwnedTextureFrameHandle frameHandle) {
    ...
    if (frame.width() <= 0 || frame.height() <= 0) {
      throw new IllegalStateException("MapVina returned an empty Vulkan owned texture frame");
    }
    if (frame.layout() != VK_IMAGE_LAYOUT_SHADER_READ_ONLY_OPTIMAL) {
      throw new IllegalStateException(
          "MapVina owned texture frame is not shader-readable: layout=" + frame.layout());
    }
