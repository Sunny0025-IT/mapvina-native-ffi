...
fn handleMouseWheel(wheel: c.SDL_MouseWheelEvent, map: *mapvina.MapHandle) !Result {
...
fn adjustBearing(map: *mapvina.MapHandle, delta: f64) !void {
...
fn adjustBearingAnimated(map: *mapvina.MapHandle, delta: f64, animation: mapvina.AnimationOptions) !void {
    const camera = try currentCamera(map);
    try expectCameraStatus(map.easeTo(.{ .bearing = (camera.bearing orelse 0) + delta }, animation), "keyboard rotate failed");
}

fn adjustPitchAnimated(map: *mapvina.MapHandle, delta: f64, animation: mapvina.AnimationOptions) !void {
    const camera = try currentCamera(map);
...
fn resetPitchAndBearingAnimated(map: *mapvina.MapHandle, animation: mapvina.AnimationOptions) !void {
    try expectCameraStatus(map.easeTo(.{ .bearing = 0, .pitch = 0 }, animation), "camera reset failed");
}

fn currentCamera(map: *mapvina.MapHandle) !mapvina.CameraOptions {
...
fn expectCameraStatus(result: mapvina.Error!void, message: []const u8) !void {
    result catch |err| {
...
fn point(x: f64, y: f64) mapvina.ScreenPoint {
    return .{ .x = x, .y = y };
}

fn cameraAnimation(duration_ms: f64) mapvina.AnimationOptions {
