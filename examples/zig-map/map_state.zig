...
pub const MapState = struct {
    ...
    pub fn init(allocator: std.mem.Allocator, viewport: types.Viewport, backend: anytype) !MapState {
        var runtime = mapvina.RuntimeHandle.create(allocator, .{ .cache_path = ":memory:" }, null) catch |err| {
            diagnostics.logError("runtime create failed", err);
            return types.AppError.RuntimeCreateFailed;
        };
        errdefer runtime.close() catch {};

        var map = mapvina.MapHandle.create(&runtime, .{
...
pub fn drainEvents(runtime: *mapvina.RuntimeHandle, map: *mapvina.MapHandle) !bool {
...
fn loadStyle(allocator: std.mem.Allocator, map: *mapvina.MapHandle) !void {
...
fn setCamera(map: *mapvina.MapHandle) !void {
    map.jumpTo(.{
