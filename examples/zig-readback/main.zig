...
pub fn main(init_args: std.process.Init) !void {
    ...
    defer mapvina.setAsyncLogSeverityMask(.default, null) catch {};
    try logAndValidateRenderBackend();

    var runtime = try mapvina.RuntimeHandle.create(allocator, .{ .cache_path = ":memory:" }, null);
...
fn logAndValidateRenderBackend() !void {
    const support = mapvina.supportedRenderBackends();
...
fn renderBackendSupportLabel(support: mapvina.RenderBackendSupport) []const u8 {
...
const OwnedTextureContext = if (build_options.supports_vulkan) VulkanAttachContext else if (build_options.supports_metal) struct {
    ...
    fn descriptor(self: *const @This()) mapvina.MetalContextDescriptor {
...
const OwnedTextureTarget = struct {
    ...
    fn attach(map: *mapvina.MapHandle, descriptor: OwnedTextureDescriptor) !OwnedTextureTarget {
...
const VulkanAttachContext = if (build_options.supports_vulkan) struct {
    ...
    fn init() !VulkanAttachContext {
        ...
        app_info.pApplicationName = "mapvina-native-zig-readback";
        app_info.applicationVersion = 1;
        app_info.pEngineName = "mapvina-native-zig-readback";
    ...
    fn descriptor(self: *const VulkanAttachContext) mapvina.VulkanContextDescriptor {
        return .{
...
fn setInitialCamera(map: *mapvina.MapHandle) !void {
    try map.jumpTo(.{
...
fn renderTexture(
    ...
    texture: *mapvina.RenderSessionHandle,
) !void {
