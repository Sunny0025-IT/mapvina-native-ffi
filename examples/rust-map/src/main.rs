...
fn main() -> Result<(), Box<dyn Error>> {
    ...
    if !mapvina_native::supported_render_backends()
        .contains(mapvina_native::RenderBackendMask::VULKAN)
    {
        return Err("the loaded MapVina native library does not support Vulkan".into());
    }

    let event_loop = EventLoop::new()?;
    let window = WindowBuilder::new()
        .with_title("MapVina Rust Vulkan Map")
