...
impl Mode {
    ...
    pub fn status(self) -> &'static str {
        match self {
            Self::OwnedTexture => "samples MapVina-owned Vulkan frames into the winit swapchain",
...
impl RenderTarget {
    ...
    pub fn resize(&mut self, viewport: Viewport) -> mapvina_native::Result<()> {
        match self {
    ...
    pub fn render_update(&mut self) -> mapvina_native::Result<()> {
        match self {
            Self::VulkanOwnedTexture {
                session,
                compositor,
