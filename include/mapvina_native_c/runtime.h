/**
 * @file mapvina_native_c/runtime.h
 * Public C API declarations for runtime, resources, and events.
 */

#ifndef MAPVINA_NATIVE_C_RUNTIME_H
#define MAPVINA_NATIVE_C_RUNTIME_H
 ...
 * MapVina's online source stop starting network requests until reachability
...
typedef struct mln_runtime_event_render_frame {
  ...
  uint32_t mode;
  /** Whether MapVina needs another frame after this one. */
  bool needs_repaint;
 ...
 *
 * - MapVina may invoke the callback on a worker or network thread instead of
 ...
 * file source. That thread may be a MapVina worker or network thread instead
 ...
 * MapVina's resource callback. Malformed response contents are converted to
...
/**
 * Reports whether MapVina has cancelled a C API resource provider request.
 *
 ...
 *
 * It is forwarded to MapVina's OnlineFileSource, so it applies wherever native
...
/**
 * Runs a MapVina ambient cache maintenance operation for this runtime.
 *
 * When runtime options omit cache_path, this operates on MapVina's default
 ...
 * waits until MapVina's database callback reports completion and returns the
 * resulting status.
 *
 * Returns:
 * - MLN_STATUS_OK on success.
...
#endif  // MAPVINA_NATIVE_C_RUNTIME_H
