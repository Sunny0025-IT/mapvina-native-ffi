/**
 * @file mapvina_native_c/map.h
 * Public C API declarations for map lifecycle, shared types, and offline
 * regions.
 */

#ifndef MAPVINA_NATIVE_C_MAP_H
#define MAPVINA_NATIVE_C_MAP_H
...
/** Field mask values for MapVina axonometric rendering options. */
...
typedef struct mln_animation_options {
   ...
   * would overflow MapVina Native's internal duration are invalid.
   */
  double duration_ms;
...
/** Free camera position and orientation in MapVina Native camera space. */
...
/**
 * MapVina geometry input descriptor graph.
 *
...
typedef struct mln_offline_tile_pyramid_region_definition {
  ...
  double min_zoom;
  /**
   * Maximum zoom. Positive infinity follows MapVina Native behavior and lets
 ...
 *
 * The returned list owns the exact region list reported by MapVina Native's
...
/**
 * MapVina axonometric rendering options used for snapshots and commands.
 *
 * MapVina Native names this native type ProjectionMode. It controls the live
