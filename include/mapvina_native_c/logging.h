/**
 * @file mapvina_native_c/logging.h
 * Public C API declarations for logging.
 */

#ifndef MAPVINA_NATIVE_C_LOGGING_H
#define MAPVINA_NATIVE_C_LOGGING_H
...
/** Log severity values emitted by MapVina Native. */
...
/** Log event categories emitted by MapVina Native. */
...
/**
 * Receives a MapVina Native log record.
 *
 * The message pointer is borrowed for the callback duration. Returning non-zero
 * consumes the record. Returning zero lets MapVina Native's platform logger
 * handle it.
 */
...
/**
 * Installs a process-global MapVina Native log callback.
 *
 ...
 *
 * - MapVina may invoke it from logging or worker threads selected by the async
 *   severity mask.
 * - MapVina may invoke it while holding internal logging locks.
 * - The callback must be thread-safe, return quickly, and must not call this C
 *   API or MapVina Native APIs.
...
/**
 * Controls which log severities MapVina Native may dispatch asynchronously.
 *
 * MLN_LOG_SEVERITY_MASK_DEFAULT restores MapVina Native's default behavior:
...
#endif  // MAPVINA_NATIVE_C_LOGGING_H
