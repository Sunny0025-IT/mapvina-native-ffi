...
fn addCTests(b: *std.Build, options: BuildOptions) *std.Build.Step.Compile {
    ...
    mapvina_build.addRenderBackendOptions(b, c_tests.root_module, options.render_backend);
    mapvina_build.linkMapvinaNativeC(b, c_tests.root_module, .{
        .target = options.target,
...
pub fn build(b: *std.Build) void {
    ...
    const cmake_artifact_dir = mapvina_build.cmakeArtifactDir(b);
