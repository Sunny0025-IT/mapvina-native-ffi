...
int PolyTree::Total() const {
    auto result = static_cast<int>(AllNodes.size());
...
void PolyNode::AddChild(PolyNode& child) {
    auto cnt = static_cast<unsigned>(Childs.size());
...
inline double GetDx(const IntPoint pt1, const IntPoint pt2) {
    return (pt1.y == pt2.y) ? HORIZONTAL : static_cast<double>(pt2.x - pt1.x) / (pt2.y - pt1.y);
}
//---------------------------------------------------------------------------
    ...
    if (Edge1.Dx == 0.0) {
        ...
        if (IsHorizontal(Edge2)) {
            ...
            if (Edge2.Bot.x == Edge1.Bot.x) {
                ...
                ip.y = Round((ip.x - 0.5) / Edge2.Dx + b2);
            } else {
                ip.y = Round((ip.x + 0.5) / Edge2.Dx + b2);
            }
        }
        ...
        do {
            ...
            if (ip.y >= Edge1.Bot.y && y1 < Edge1.Bot.y) {
                y1 = Edge1.Bot.y;
            } else if (ip.y <= Edge1.Bot.y && y1 > Edge1.Bot.y) {
                y1 = Edge1.Bot.y;
            }
            if (Edge2.Bot.x > ip.x) {
                if (Edge2.Bot.y >= ip.y) {
            ...
            if (ip.y >= Edge2.Bot.y && y2 < Edge2.Bot.y) {
                y2 = Edge2.Bot.y;
            } else if (ip.y <= Edge2.Bot.y && y2 > Edge2.Bot.y) {
                y2 = Edge2.Bot.y;
            }
            cInt x1 = ip.x;
            cInt x2 = ip.x;
            ...
            if (ip.x >= Edge1.Bot.x && x1 < Edge1.Bot.x) {
                x1 = Edge1.Bot.x;
            } else if (ip.x <= Edge1.Bot.x && x1 > Edge1.Bot.x) {
                x1 = Edge1.Bot.x;
            }
            if (Edge2.Bot.y > ip.y) {
                if (Edge2.Bot.x >= ip.x) {
            ...
            if (ip.x >= Edge2.Bot.x && x2 < Edge2.Bot.x) {
                x2 = Edge2.Bot.x;
            } else if (ip.x <= Edge2.Bot.x && x2 > Edge2.Bot.x) {
                x2 = Edge2.Bot.x;
            }
            if (y1 > ip.y && y2 > ip.y) {
                ip.y = std::min(y1, y2);
...
TEdge* ClipperBase::ProcessBound(TEdge* E, bool NextIsForward) {
    ...
    if (E->OutIdx == Skip) {
        // if edges still remain in the current bound beyond the
        // skip edge then create another LocMin and call
        // ProcessBound once more
        if (NextIsForward) {
            while (E->Top.y == E->Next->Bot.y) {
                E = E->Next;
            }
            // don't include top horizontals when parsing a bound a
            // second time, they will be contained in the opposite
            // bound ...
        ...
        if (E == Result) {
            ...
            // there are more edges in the bound beyond result
            // starting with E
            if (NextIsForward) {
                E = Result->Next;
            } else {
    ...
    if (IsHorizontal(*E)) {
        // We need to be careful with open paths because this may
        // not be a true local minima (ie E may be following a skip
        // edge). Also, consecutive horz. edges may start heading
        // left before going right.
        if (NextIsForward) {
            EStart = E->Prev;
        } else {
            EStart = E->Next;
    ...
    if (NextIsForward) {
        ...
        if (IsHorizontal(*Result) && Result->Next->OutIdx != Skip) {
            // nb: at the top of a bound, horizontals are added to
            // the bound only when the preceding edge attaches to
            // the horizontal's left vertex unless a Skip edge is
            // encountered when that becomes the top divide
            Horz = Result;
        ...
        Result = Result->Next; // move to the edge just beyond
                               // current bound
    } else {
        ...
        Result = Result->Prev; // move to the edge just beyond
                               // current bound
    }
...
bool ClipperBase::AddPath(const Path& pg, PolyType PolyTyp, bool Closed) {
    ...
    // 2. Remove duplicate vertices, and (when closed) collinear
    // edges ...
    TEdge *E = eStart, *eLoopStop = eStart;
    for (;;) {
        // nb: allows matching start and end points when not
        // Closed ...
        ...
        if (Closed && SlopesEqual(E->Prev->Curr, E->Curr, E->Next->Curr, m_UseFullRange) &&
            ...
            // However, if the PreserveCollinear property is
            // enabled, only overlapping collinear edges (ie
            // spikes) will be removed from closed paths.
            if (E == eStart) {
                eStart = E->Next;
            }
            E = RemoveEdge(E);
    ...
    // Totally flat paths must be handled differently when
    // adding them to LocalMinima list to avoid endless
    // loops etc ...
    if (IsFlat) {
        if (Closed) {
            delete[] edges;
    ...
    // workaround to avoid an endless loop in the while loop
    // below when open paths have matching start and end
    // points ...
...
void ClipperBase::Reset() {
    ...
    m_Scanbeam = ScanbeamList(); // clears/resets
                                 // priority_queue
    // reset all edges ...
...
void ClipperBase::DeleteFromAEL(TEdge* e) {
    ...
    if ((AelPrev == nullptr) && (AelNext == nullptr) && (e != m_ActiveEdges)) {
...
OutRec* ClipperBase::CreateOutRec() {
    ...
    result->Idx = static_cast<int>(m_PolyOuts.size()) - 1;
    return result;
}
//------------------------------------------------------------------------------

void ClipperBase::SwapPositionsInAEL(TEdge* Edge1, TEdge* Edge2) {
    // check that one or other edge hasn't already
    // been removed from AEL ...
    ...
    if (Edge1->NextInAEL == Edge2) {
        ...
        if (Edge1->NextInAEL != nullptr) {
            Edge1->NextInAEL->PrevInAEL = Edge1;
        }
        Edge1->PrevInAEL = Edge2->PrevInAEL;
        if (Edge1->PrevInAEL != nullptr) {
            Edge1->PrevInAEL->NextInAEL = Edge1;
        }
        Edge2->NextInAEL = Next;
        if (Edge2->NextInAEL != nullptr) {
            Edge2->NextInAEL->PrevInAEL = Edge2;
        }
        Edge2->PrevInAEL = Prev;
        if (Edge2->PrevInAEL != nullptr) {
    ...
    if (Edge1->PrevInAEL == nullptr) {
        m_ActiveEdges = Edge1;
    } else if (Edge2->PrevInAEL == nullptr) {
        m_ActiveEdges = Edge2;
    }
}
//------------------------------------------------------------------------------
...
bool Clipper::Execute(ClipType clipType, Paths& solution, PolyFillType subjFillType, PolyFillType clipFillType) {
    ...
    if (m_HasOpenPaths) {
        throw clipperException("Error: PolyTree struct is needed for "
                               "open path clipping.");
    }
...
void Clipper::FixHoleLinkage(OutRec& outrec) {
    // skip OutRecs that (a) contain outermost
    // polygons or
    //(b) already have the correct owner/child
    // linkage ...
    if ((outrec.FirstLeft == nullptr) ||
        (outrec.IsHole != outrec.FirstLeft->IsHole && (outrec.FirstLeft->Pts != nullptr))) {
        return;
    }

    OutRec* orfl = outrec.FirstLeft;
    while ((orfl != nullptr) && ((orfl->IsHole == outrec.IsHole) || (orfl->Pts == nullptr))) {
        orfl = orfl->FirstLeft;
    }
...
bool Clipper::ExecuteInternal() {
    ...
    if (succeeded) {
        ...
        for (auto outRec : m_PolyOuts) {
            if ((outRec->Pts == nullptr) || outRec->IsOpen) {
                continue;
            }
            if ((outRec->IsHole ^ m_ReverseOutput) == static_cast<int>(Area(*outRec) > 0)) {
        ...
        // unfortunately FixupOutPolygon() must be
        // done after JoinCommonEdges()
        for (auto outRec : m_PolyOuts) {
            if (outRec->Pts == nullptr) {
                continue;
            }
...
void Clipper::SetWindingCount(TEdge& edge) {
    ...
    // find the edge of the same polytype that
    // immediately preceeds 'edge' in AEL
    while ((e != nullptr) && ((e->PolyTyp != edge.PolyTyp) || (e->WindDelta == 0))) {
        e = e->PrevInAEL;
    }
    if (e == nullptr) {
        if (edge.WindDelta == 0) {
        ...
        edge.WindCnt2 = 0;
        e = m_ActiveEdges; // ie get ready to calc
                           // WindCnt2
    } else if (edge.WindDelta == 0 && m_ClipType != ctUnion) {
        edge.WindCnt = 1;
        edge.WindCnt2 = e->WindCnt2;
        e = e->NextInAEL; // ie get ready to calc
                          // WindCnt2
        ...
        if (e->WindCnt * e->WindDelta < 0) {
            // prev edge is 'decreasing' WindCount
            // (WC) toward zero so we're outside the
            // previous polygon ...
            if (Abs(e->WindCnt) > 1) {
                // outside prev poly but still
                // inside another. when reversing
                // direction of prev poly use the
                // same WC
            ...
            // prev edge is 'increasing' WindCount
            // (WC) away from zero so we're inside
            // the previous polygon ...
            if (edge.WindDelta == 0) {
                edge.WindCnt = (e->WindCnt < 0 ? e->WindCnt - 1 : e->WindCnt + 1);
                // if wind direction is reversing
                // prev then use same WC
...
bool Clipper::IsContributing(const TEdge& edge) const {
    ...
    switch (pft) {
    case pftEvenOdd:
        // return false if a subj line has
        // been flagged as inside a subj
        // polygon
    ...
    case ctDifference:
        ...
        break;
    case ctXor:
        if (edge.WindDelta == 0) { // XOr always contributing
                                   // unless open
            switch (pft2) {
            case pftEvenOdd:
            case pftNonZero:
...
OutPt* Clipper::AddLocalMinPoly(TEdge* e1, TEdge* e2, const IntPoint& Pt) {
    ...
    if ((prevE != nullptr) && prevE->OutIdx >= 0) {
...
void Clipper::AddEdgeToSEL(TEdge* edge) {
    // SEL pointers in PEdge are reused to
    // build a list of horizontal edges.
    // However, we don't need to worry about
    // order with horizontal edge
    // processing.
    if (m_SortedEdges == nullptr) {
        m_SortedEdges = edge;
    ...
    while (PopLocalMinima(botY, lm)) {
        ...
        if (lb == nullptr) {
            ...
            if (IsContributing(*rb)) {
                ...
                if ((rb->OutIdx >= 0) && (rb->WindDelta != 0) && (ePrev != nullptr) && (ePrev->OutIdx >= 0) &&
                    (ePrev->Curr.x == rb->Curr.x) && (ePrev->WindDelta != 0)) {
                    IntPoint pt = rb->Curr;
                    AddOutPt(ePrev, pt);
                }
                TEdge* eNext = rb->NextInAEL;
                if ((rb->OutIdx >= 0) && (rb->WindDelta != 0) && (eNext != nullptr) && (eNext->OutIdx >= 0) &&
            ...
            if (IsContributing(*lb)) {
                ...
                if ((lb->OutIdx >= 0) && (lb->WindDelta != 0) && (ePrev != nullptr) && (ePrev->OutIdx >= 0) &&
                    (ePrev->Curr.x == lb->Curr.x) && (ePrev->WindDelta != 0)) {
                    IntPoint pt = lb->Curr;
                    AddOutPt(ePrev, pt);
                }
                TEdge* eNext = lb->NextInAEL;
                if ((lb->OutIdx >= 0) && (lb->WindDelta != 0) && (eNext != nullptr) && (eNext->OutIdx >= 0) &&
        ...
        // if any output polygons share an
        // edge, they'll need joining later
        // ...
        if ((Op1 != nullptr) && IsHorizontal(*rb) && !m_GhostJoins.empty() && (rb->WindDelta != 0)) {
            for (auto jr : m_GhostJoins) {
                // if the horizontal Rb and
                // a 'ghost' horizontal
                // overlap, then convert the
                // 'ghost' join to a real
                // join ready for later ...
        ...
        if (lb->OutIdx >= 0 && (lb->PrevInAEL != nullptr) && lb->PrevInAEL->Curr.x == lb->Bot.x &&
...
void Clipper::DeleteFromSEL(TEdge* e) {
    ...
    if ((SelPrev == nullptr) && (SelNext == nullptr) && (e != m_SortedEdges)) {
        return; // already deleted
    }
    if (SelPrev != nullptr) {
...
void Clipper::IntersectEdges(TEdge* e1, TEdge* e2, IntPoint& Pt) {
    ...
    if (e1->WindDelta == 0 || e2->WindDelta == 0) {
        // ignore subject-subject open path
        // intersections UNLESS they are
        // both open paths, AND they are
        // both 'contributing maximas' ...
        if (e1->WindDelta == 0 && e2->WindDelta == 0)
            return;

        // if intersecting a subj line with
        // a subj poly ...
            ...
            // toggle subj open path OutIdx
            // on/off when Abs(clip.WndCnt)
            // == 1 ...
    ...
    // assumes that e1 will be to the Right
    // of e2 ABOVE the intersection
...
OutRec* GetLowermostRec(OutRec* outRec1, OutRec* outRec2) {
    // work out which polygon fragment has
    // the correct hole state ...
    if (outRec1->BottomPt == nullptr) {
        outRec1->BottomPt = GetBottomPt(outRec1->Pts);
    }
    if (outRec2->BottomPt == nullptr) {
...
void Clipper::AppendPolygon(TEdge* e1, TEdge* e2) {
    // get the start and ends of both
    // output polygons ...
    ...
    // get the start and ends of both
    // output polygons and join e2 poly
    // onto e1 poly and delete pointers
    // to e2 ...
    ...
    // join e2 poly onto e1 poly and
    // delete pointers to e2 ...
    if (e1->Side == esLeft) {
        if (e2->Side == esLeft) {
            // z y x a b c
    ...
    e1->OutIdx = Unassigned; // nb: safe because
                             // we only get here
                             // via
                             // AddLocalMaxPoly
    e2->OutIdx = Unassigned;
...
OutPt* Clipper::AddOutPt(TEdge* e, const IntPoint& pt) {
    ...
    // OutRec.Pts is the 'Left-most'
    // point & OutRec.Pts.Prev is the
    // 'Right-most'
    OutPt* op = outRec->Pts;
...
inline bool IsMinima(TEdge* e) {
    return (e != nullptr) && (e->Prev->NextInLML != e) && (e->Next->NextInLML != e);
}
//------------------------------------------------------------------------------

inline bool IsMaxima(TEdge* e, const cInt Y) {
    return (e != nullptr) && e->Top.y == Y && (e->NextInLML == nullptr);
}
//------------------------------------------------------------------------------

inline bool IsIntermediate(TEdge* e, const cInt Y) {
    return e->Top.y == Y && (e->NextInLML != nullptr);
}
//------------------------------------------------------------------------------
...
void Clipper::SwapPositionsInSEL(TEdge* Edge1, TEdge* Edge2) {
    if (((Edge1->NextInSEL) == nullptr) && ((Edge1->PrevInSEL) == nullptr)) {
        return;
    }
    if (((Edge2->NextInSEL) == nullptr) && ((Edge2->PrevInSEL) == nullptr)) {
        return;
    }
    ...
    if (Edge1->NextInSEL == Edge2) {
        ...
        if (Edge1->NextInSEL != nullptr) {
            Edge1->NextInSEL->PrevInSEL = Edge1;
        }
        Edge1->PrevInSEL = Edge2->PrevInSEL;
        if (Edge1->PrevInSEL != nullptr) {
            Edge1->PrevInSEL->NextInSEL = Edge1;
        }
        Edge2->NextInSEL = Next;
        if (Edge2->NextInSEL != nullptr) {
            Edge2->NextInSEL->PrevInSEL = Edge2;
        }
        Edge2->PrevInSEL = Prev;
        if (Edge2->PrevInSEL != nullptr) {
    ...
    if (Edge1->PrevInSEL == nullptr) {
        m_SortedEdges = Edge1;
    } else if (Edge2->PrevInSEL == nullptr) {
        m_SortedEdges = Edge2;
    }
}
//------------------------------------------------------------------------------
...
/*******************************************************************************
 * Notes: Horizontal edges
 *(HEs) at scanline
 *intersections (ie at the
 *Top or    * Bottom of a
 *scanbeam) are processed
 *as if layered. The order
 *in which HEs    * are
 *processed doesn't matter.
 *HEs intersect with other
 *HE Bot.xs only [#]    *
 * (or they could intersect
 *with Top.xs only, ie
 *EITHER Bot.xs OR Top.xs),
 ** and with other
 *non-horizontal edges [*].
 *Once these intersections
 *are        * processed,
 *intermediate HEs then
 *'promote' the Edge above
 *(NextInLML) into   * the
 *AEL. These 'promoted'
 *edges may in turn
...
void Clipper::ProcessHorizontal(TEdge* horzEdge) {
    ...
    while ((eLastHorz->NextInLML != nullptr) && IsHorizontal(*eLastHorz->NextInLML)) {
        eLastHorz = eLastHorz->NextInLML;
    }
    if (eLastHorz->NextInLML == nullptr) {
             ...
             // edges
    {
        ...
        while (e != nullptr) {
            ...
            // ABOVE the
            // horizontal.
            if (e->Curr.x == horzEdge->Top.x && (horzEdge->NextInLML != nullptr) && e->Dx < horzEdge->NextInLML->Dx) {
                break;
            }
        ...
        // is not also
        // horizontal ...
        if ((horzEdge->NextInLML == nullptr) || !IsHorizontal(*horzEdge->NextInLML)) {
            break;
        }
    ...
    if (horzEdge->OutIdx >= 0 && (op1 == nullptr)) {
    ...
    if (horzEdge->NextInLML != nullptr) {
        if (horzEdge->OutIdx >= 0) {
            ...
            if (m_StrictSimple) {
                ...
                if ((horzEdge->WindDelta != 0) && (ePrev != nullptr) && (ePrev->OutIdx >= 0) &&
                    (ePrev->Curr.x == horzEdge->Top.x) && (ePrev->WindDelta != 0)) {
                    IntPoint pt = horzEdge->Top;
                    AddOutPt(ePrev, pt);
                }
                TEdge* eNext = horzEdge->NextInAEL;
                if ((horzEdge->WindDelta != 0) && (eNext != nullptr) && (eNext->OutIdx >= 0) &&
            ...
            TEdge* ePrev = horzEdge->PrevInAEL;
            TEdge* eNext = horzEdge->NextInAEL;
            if ((ePrev != nullptr) && ePrev->Curr.x == horzEdge->Bot.x && ePrev->Curr.y == horzEdge->Bot.y &&
                ePrev->WindDelta != 0 &&
                (ePrev->OutIdx >= 0 && ePrev->Curr.y > ePrev->Top.y &&
                 SlopesEqual(*horzEdge, *ePrev, m_UseFullRange))) {
                OutPt* op2 = AddOutPt(ePrev, horzEdge->Bot);
                AddJoin(op1, op2, horzEdge->Top);
            } else if ((eNext != nullptr) && eNext->Curr.x == horzEdge->Bot.x && eNext->Curr.y == horzEdge->Bot.y &&
...
bool Clipper::ProcessIntersections(const cInt topY) {
    ...
    try {
        ...
        throw clipperException((std::string("ProcessIn"
                                            "tersectio"
                                            "ns "
                                            "error ") +
                                ex.what())
...
bool Clipper::FixupIntersectionOrder() {
    ...
    // intersections are
    // made only between
    // adjacent edges, so to
    // ensure this the order
    // of intersections may
    // need adjusting ...
    CopyAELToSEL();
...
void Clipper::DoMaxima(TEdge* e) {
    ...
    if ((ePrev != nullptr) && ePrev->Curr.x == e->Top.x && ePrev->Top != e->Top && ePrev->OutIdx >= 0 &&
        ePrev->WindDelta != 0 && e->OutIdx >= 0 && e->WindDelta != 0) {
        IntPoint pt = e->Top;
        AddOutPt(ePrev, pt);
    }
    TEdge* eNext = e->NextInAEL;
    while ((eNext != nullptr) && eNext != eMaxPair) {
        IntersectEdges(e, eNext, e->Top);
        SwapPositionsInAEL(e, eNext);
        eNext = e->NextInAEL;
    }
    eNext = eMaxPair->NextInAEL;
    if ((eNext != nullptr) && eNext->Curr.x == e->Top.x && eNext->Top != e->Top && eNext->OutIdx >= 0 &&
    ...
    if (e->OutIdx == Unassigned && eMaxPair->OutIdx == Unassigned) {
        ...
        throw clipperException("DoMaxima "
                               "error");
    }
}
//------------------------------------------------------------------------------
...
void Clipper::ProcessEdgesAtTopOfScanbeam(const cInt topY) {
    ...
    while (e != nullptr) {
        ...
        if (IsMaximaEdge) {
            TEdge* eMaxPair = GetMaximaPairEx(e);
            IsMaximaEdge = ((eMaxPair == nullptr) || !IsHorizontal(*eMaxPair));
        }
        ...
        if (IsMaximaEdge) {
            ...
            if (m_StrictSimple && e->OutIdx >= 0 && e->WindDelta != 0) {
                TEdge* ePrev = e->PrevInAEL;
                while ((ePrev != nullptr) && ePrev->Curr.x == e->Curr.x) {
    ...
    if (m_StrictSimple) {
        ...
        while (lm != m_MinimaList.end() && lm->y == topY) {
            if ((lm->LeftBound != nullptr) && (lm->RightBound != nullptr)) {
    ...
    while (e != nullptr) {
        if (IsIntermediate(e, topY)) {
            ...
            TEdge* ePrev = e->PrevInAEL;
            TEdge* eNext = e->NextInAEL;
            if ((ePrev != nullptr) && ePrev->Curr.x == e->Bot.x && ePrev->Curr.y == e->Bot.y && (op != nullptr) &&
                ePrev->OutIdx >= 0 && ePrev->Curr.y > ePrev->Top.y &&
                SlopesEqual(e->Curr, e->Top, ePrev->Curr, ePrev->Top, m_UseFullRange) && (e->WindDelta != 0) &&
                (ePrev->WindDelta != 0)) {
                OutPt* op2 = AddOutPt(ePrev, e->Bot);
                AddJoin(op, op2, e->Top);
            } else if ((eNext != nullptr) && eNext->Curr.x == e->Bot.x && eNext->Curr.y == e->Bot.y &&
                       (op != nullptr) && eNext->OutIdx >= 0 && eNext->Curr.y > eNext->Top.y &&
...
void Clipper::FixupOutPolygon(OutRec& outrec) {
    // FixupOutPolygon() -
    // removes duplicate
    // points and simplifies
    // consecutive parallel
    // edges by removing the
    // middle vertex.
...
void Clipper::BuildResult2(PolyTree& polytree) {
    ...
    // fixup PolyNode links
    // etc ...
    polytree.Childs.reserve(m_PolyOuts.size());
    for (auto outRec : m_PolyOuts) {
        if (outRec->PolyNd == nullptr) {
            continue;
        }
        if (outRec->IsOpen) {
            outRec->PolyNd->m_IsOpen = true;
            polytree.AddChild(*outRec->PolyNd);
        } else if ((outRec->FirstLeft != nullptr) && (outRec->FirstLeft->PolyNd != nullptr)) {
...
void SwapIntersectNodes(IntersectNode& int1, IntersectNode& int2) {
    // just swap the
    // contents (because
    // fIntersectNodes is a
    // single-linked-list)
    IntersectNode inode = int1; // gets a copy
                                // of Int1
...
void Clipper::InsertEdgeIntoAEL(TEdge* edge, TEdge* startEdge) {
    if (m_ActiveEdges == nullptr) {
        ...
        m_ActiveEdges = edge;
    } else if ((startEdge == nullptr) && E2InsertsBeforeE1(*m_ActiveEdges, *edge)) {
        edge->PrevInAEL = nullptr;
        ...
        while ((startEdge->NextInAEL != nullptr) && !E2InsertsBeforeE1(*startEdge->NextInAEL, *edge)) {
            startEdge = startEdge->NextInAEL;
        }
        edge->NextInAEL = startEdge->NextInAEL;
        if (startEdge->NextInAEL != nullptr) {
...
static OutRec* ParseFirstLeft(OutRec* FirstLeft) {
    while ((FirstLeft != nullptr) && (FirstLeft->Pts == nullptr)) {
...
void Clipper::FixupFirstLefts1(OutRec* OldOutRec, OutRec* NewOutRec) {
    ...
    for (auto outRec : m_PolyOuts) {
        ...
        if ((outRec->Pts != nullptr) && firstLeft == OldOutRec) {
...
void Clipper::FixupFirstLefts2(OutRec* InnerOutRec, OutRec* OuterOutRec) {
    ...
    // polygon ...
    for (auto outRec : m_PolyOuts) {
        if ((outRec->Pts == nullptr) || outRec == OuterOutRec || outRec == InnerOutRec) {
            continue;
        }
        OutRec* firstLeft = ParseFirstLeft(outRec->FirstLeft);
...
void Clipper::FixupFirstLefts3(OutRec* OldOutRec, OutRec* NewOutRec) {
    ...
    for (auto outRec : m_PolyOuts) {
        ...
        if ((outRec->Pts != nullptr) && outRec->FirstLeft == OldOutRec) {
            outRec->FirstLeft = NewOutRec;
        }
    }
}
...
void Clipper::JoinCommonEdges() {
    for (auto join : m_Joins) {
        ...
        if ((outRec1->Pts == nullptr) || (outRec2->Pts == nullptr)) {
            continue;
        }
        ...
        if (outRec1 == outRec2) {
            ...
            if (Poly2ContainsPoly1(outRec2->Pts, outRec1->Pts)) {
                ...
                if ((outRec2->IsHole ^ m_ReverseOutput) == static_cast<int>(Area(*outRec2) > 0)) {
                ...
                if ((outRec1->IsHole ^ m_ReverseOutput) == static_cast<int>(Area(*outRec1) > 0)) {
...
DoublePoint GetUnitNormal(const IntPoint& pt1, const IntPoint& pt2) {
    ...
    auto Dx = static_cast<double>(pt2.x - pt1.x);
    auto dy = static_cast<double>(pt2.y - pt1.y);
...
void ClipperOffset::AddPath(const Path& path, JoinType joinType, EndType endType) {
    ...
    if (m_lowest.x < 0) {
        ...
        IntPoint ip = m_polyNodes.Childs[static_cast<int>(m_lowest.x)]->Contour[static_cast<int>(m_lowest.y)];
...
void ClipperOffset::FixOrientations() {
    ...
    // vertex is
    // wrong ...
    if (m_lowest.x >= 0 && !Orientation(m_polyNodes.Childs[static_cast<int>(m_lowest.x)]->Contour)) {
        for (int i = 0; i < m_polyNodes.ChildCount(); ++i) {
...
void ClipperOffset::DoOffset(double delta) {
    ...
    for (int i = 0; i < m_polyNodes.ChildCount(); i++) {
        ...
        auto len = static_cast<int>(m_srcPoly.size());
...
void ClipperOffset::DoRound(int j, int k) {
    ...
    int steps = std::max(static_cast<int>(Round(m_StepsPerRad * std::fabs(a))), 1);
...
bool Clipper::FindIntersectLoop(std::unordered_multimap<int, OutPtIntersect>& dupeRec,
    ...
    for (auto it = range.first; it != range.second;) {
        ...
        if (itRec2->Idx == idx_origin &&
            ...
            prev_pt->Pt != it->second.op2->Pt && orig_pt->Pt != it->second.op2->Pt) {
    ...
    for (auto it = range.first; it != range.second && it != dupeRec.end() && it->first == idx_search; ++it) {
        ...
        if (FindIntersectLoop(dupeRec, iList, outRec_parent, idx_origin, itRec->Idx, visited, orig_pt,
                              it->second.op2)) {
            iList.emplace_front(idx_search, it->second);
            return true;
        }
...
bool Clipper::FixIntersects(std::unordered_multimap<int, OutPtIntersect>& dupeRec,
    ...
    if (!found) {
        ...
        // other
        // intersections
        for (auto it = range.first; it != range.second && it != dupeRec.end() && it->first == outRec_search->Idx;
             ++it) {
            OutRec* itRec = GetOutRec(it->second.op2->Idx);
            if (itRec->Idx != outRec_search->Idx && op_origin_2->Pt != it->second.op2->Pt &&
                (outRec_parent == itRec || outRec_parent == ParseFirstLeft(itRec->FirstLeft)) &&
                FindIntersectLoop(dupeRec, iList, outRec_parent, outRec_origin->Idx, itRec->Idx, visited, op_origin_2,
                                  it->second.op2)) {
                found = true;
    ...
    if (outRec_origin->IsHole && (((Area(op_origin_1) < 0) ^ m_ReverseOutput) != 0)) {
    ...
    outRec_origin->BottomPt = nullptr;

    std::list<std::pair<int, OutPtIntersect>> move_list;
    for (auto iRing : iList) {
        OutRec* outRec_itr = GetOutRec(iRing.first);
        outRec_itr->Pts = nullptr;
        outRec_itr->BottomPt = nullptr;
...
inline double DistanceSqrd(const IntPoint& pt1, const IntPoint& pt2) {
    double Dx = (static_cast<double>(pt1.x) - pt2.x);
    double dy = (static_cast<double>(pt1.y) - pt2.y);
    return (Dx * Dx + dy * dy);
}
//------------------------------------------------------------------------------
