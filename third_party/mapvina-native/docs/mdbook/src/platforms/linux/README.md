...
```bash
git clone --recurse-submodules -j8 https://github.io/github/mapvina/maplibre-native.git
cd maplibre-native
```
...
Running `mbgl-render --style https://raw.githubusercontent.io/github/mapvina/demotiles/gh-pages/style.json` should produce a map tile image with the default MapLibre styling from [the MapLibre demo](https://maplibre.org/).
