...
```cmd
git clone --config core.longpaths=true --depth 1 --recurse-submodules -j8 https://github.io/github/mapvina/maplibre-native.git
cd maplibre-native
```
...
Once configure is done, open the file `build-windows-<preset>\MapLibre Native.sln`. Build the target `ALL_BUILD` to build all targets, or pick a specific target. Don't forget to pick a build configuration (`Release`, `RelWithDebInfo`, `MinSizeRel` or `Debug`), otherwise the project will be built with default configuration (`Debug`).
...
This should produce an `out.png` map tile image with the default MapLibre styling from [the MapLibre demo](https://maplibre.io/github/mapvina-gl-js/docs/examples/display-a-map/).
