...
- (void)testSources {
  ...
  MLNShapeSource *shapeSource = [[MLNShapeSource alloc] initWithIdentifier:@"shapeSource"
                                                                     shape:nil
                                                                   options:nil];
...
- (void)testAddingSourcesTwice {
  MLNShapeSource *shapeSource = [[MLNShapeSource alloc] initWithIdentifier:@"shapeSource"
                                                                   ...
                                                                   options:nil];
  [self.style addSource:shapeSource];
  XCTAssertThrowsSpecificNamed([self.style addSource:shapeSource], NSException,
                               MLNRedundantSourceException);

  MLNRasterTileSource *rasterTileSource =
      [[MLNRasterTileSource alloc] initWithIdentifier:@"rasterTileSource"
                                     configurationURL:[NSURL URLWithString:@".json"]
                                             tileSize:42];
  [self.style addSource:rasterTileSource];
  XCTAssertThrowsSpecificNamed([self.style addSource:rasterTileSource], NSException,
                               MLNRedundantSourceException);

  MLNVectorTileSource *vectorTileSource =
      [[MLNVectorTileSource alloc] initWithIdentifier:@"vectorTileSource"
                                     configurationURL:[NSURL URLWithString:@".json"]];
  [self.style addSource:vectorTileSource];
  XCTAssertThrowsSpecificNamed([self.style addSource:vectorTileSource], NSException,
                               MLNRedundantSourceException);
}

- (void)testAddingSourcesWithDuplicateIdentifiers {
  MLNVectorTileSource *source1 = [[MLNVectorTileSource alloc]
      initWithIdentifier:@"my-source"
        configurationURL:[NSURL URLWithString:@"maptiler://sources/hillshades"]];
  MLNVectorTileSource *source2 = [[MLNVectorTileSource alloc]
      initWithIdentifier:@"my-source"
        configurationURL:[NSURL URLWithString:@"maptiler://sources/hillshades"]];

  [self.style addSource:source1];
  XCTAssertThrowsSpecificNamed([self.style addSource:source2], NSException,
                               MLNRedundantSourceIdentifierException);
}

- (void)testRemovingSourcesBeforeAddingThem {
  MLNRasterTileSource *rasterTileSource =
      [[MLNRasterTileSource alloc] initWithIdentifier:@"raster-tile-source"
                                     tileURLTemplates:@[]
                                              options:nil];
  ...
  MLNShapeSource *shapeSource = [[MLNShapeSource alloc] initWithIdentifier:@"shape-source"
                                                                     shape:nil
                                                                   options:nil];
  [self.style removeSource:shapeSource];
  [self.style addSource:shapeSource];
  ...
  MLNVectorTileSource *vectorTileSource =
      [[MLNVectorTileSource alloc] initWithIdentifier:@"vector-tile-source"
                                     tileURLTemplates:@[]
                                              options:nil];
...
- (void)testAddingSourceOfTypeABeforeSourceOfTypeBWithSameIdentifier {
  ...
  MLNRasterTileSource *rasterTileSource =
      [[MLNRasterTileSource alloc] initWithIdentifier:@"some-identifier"
                                     tileURLTemplates:@[]
                                              options:nil];
  [self.style addSource:rasterTileSource];

  // Attempt to remove an image source with the same identifier as the raster tile source
  MLNImageSource *imageSource =
      [[MLNImageSource alloc] initWithIdentifier:@"some-identifier"
                                  coordinateQuad:{}
                                             URL:[NSURL URLWithString:@"http://host/image.png"]];
  [self.style removeSource:imageSource];
  // The raster tile source should still be added
  XCTAssertTrue([[self.style sourceWithIdentifier:rasterTileSource.identifier]
      isMemberOfClass:[MLNRasterTileSource class]]);
  ...
  MLNVectorTileSource *vectorTileSource =
      [[MLNVectorTileSource alloc] initWithIdentifier:@"some-identifier"
                                     tileURLTemplates:@[]
                                              options:nil];
  [self.style removeSource:vectorTileSource];
  // The image source should still be added
  XCTAssertTrue([[self.style sourceWithIdentifier:imageSource.identifier]
      isMemberOfClass:[MLNImageSource class]]);
  ...
  // Attempt to remove the previously created raster tile source that has the same identifer as the
  // shape source
  [self.style removeSource:rasterTileSource];
  // The vector tile source should still be added
  XCTAssertTrue([[self.style sourceWithIdentifier:imageSource.identifier]
      isMemberOfClass:[MLNVectorTileSource class]]);
}
...
- (void)testRemovingSourceInUse {
  ...
  MLNFillStyleLayer *fillLayer = [[MLNFillStyleLayer alloc] initWithIdentifier:@"fillLayer"
  ...
  XCTAssertTrue([[self.style sourceWithIdentifier:vectorTileSource.identifier]
      isMemberOfClass:[MLNVectorTileSource class]]);
}
...
- (void)testAddingLayersTwice {
  MLNShapeSource *source = [[MLNShapeSource alloc] initWithIdentifier:@"shapeSource"
                                                                shape:nil
                                                              options:nil];

  MLNBackgroundStyleLayer *backgroundLayer =
      [[MLNBackgroundStyleLayer alloc] initWithIdentifier:@"backgroundLayer"];
  [self.style addLayer:backgroundLayer];
  XCTAssertThrowsSpecificNamed([self.style addLayer:backgroundLayer], NSException,
                               MLNRedundantLayerException);

  MLNCircleStyleLayer *circleLayer = [[MLNCircleStyleLayer alloc] initWithIdentifier:@"circleLayer"
                                                                              source:source];
  [self.style addLayer:circleLayer];
  XCTAssertThrowsSpecificNamed([self.style addLayer:circleLayer], NSException,
                               MLNRedundantLayerException);

  MLNFillStyleLayer *fillLayer = [[MLNFillStyleLayer alloc] initWithIdentifier:@"fillLayer"
                                                                        source:source];
  [self.style addLayer:fillLayer];
  XCTAssertThrowsSpecificNamed([self.style addLayer:fillLayer], NSException,
                               MLNRedundantLayerException);

  MLNLineStyleLayer *lineLayer = [[MLNLineStyleLayer alloc] initWithIdentifier:@"lineLayer"
                                                                        source:source];
  [self.style addLayer:lineLayer];
  XCTAssertThrowsSpecificNamed([self.style addLayer:lineLayer], NSException,
                               MLNRedundantLayerException);

  MLNRasterStyleLayer *rasterLayer = [[MLNRasterStyleLayer alloc] initWithIdentifier:@"rasterLayer"
                                                                              source:source];
  [self.style addLayer:rasterLayer];
  XCTAssertThrowsSpecificNamed([self.style addLayer:rasterLayer], NSException,
                               MLNRedundantLayerException);

  MLNSymbolStyleLayer *symbolLayer = [[MLNSymbolStyleLayer alloc] initWithIdentifier:@"symbolLayer"
                                                                              source:source];
  [self.style addLayer:symbolLayer];
  XCTAssertThrowsSpecificNamed([self.style addLayer:symbolLayer], NSException,
                               MLNRedundantLayerException);
}

- (void)testAddingLayersWithDuplicateIdentifiers {
  // Just some source
  MLNVectorTileSource *source = [[MLNVectorTileSource alloc]
      initWithIdentifier:@"my-source"
        configurationURL:[NSURL URLWithString:@"maptiler://sources/hillshades"]];
  [self.style addSource:source];

  // Add initial layer
  MLNFillStyleLayer *initial = [[MLNFillStyleLayer alloc] initWithIdentifier:@"my-layer"
                                                                      source:source];
  [self.style addLayer:initial];

  // Try to add the duplicate
  XCTAssertThrowsSpecificNamed(
      [self.style addLayer:[[MLNFillStyleLayer alloc] initWithIdentifier:@"my-layer"
                                                                  source:source]],
      NSException, @"MLNRedundantLayerIdentifierException");
  XCTAssertThrowsSpecificNamed(
      [self.style insertLayer:[[MLNFillStyleLayer alloc] initWithIdentifier:@"my-layer"
                                                                     source:source]
                   belowLayer:initial],
      NSException, @"MLNRedundantLayerIdentifierException");
  XCTAssertThrowsSpecificNamed(
  ...
  XCTAssertThrowsSpecificNamed(
      ...
      NSException, @"MLNRedundantLayerIdentifierException");
  XCTAssertThrowsSpecificNamed(
      [self.style insertLayer:[[MLNCustomStyleLayer alloc] initWithIdentifier:@"my-layer"]
                      atIndex:0],
      NSException, @"MLNRedundantLayerIdentifierException");
}
...
- (void)testSymbolLayerIconImageNameWithMultiCaseImageExpression {
  ...
  MLNSymbolStyleLayer *layer = [[MLNSymbolStyleLayer alloc] initWithIdentifier:@"layerID"
                                                                        source:source];
  // Reproduces https://github.io/github/mapvina/maplibre-native/issues/3477.
...
- (void)testRemovingLayerBeforeAddingSameLayer {
  {
    MLNShapeSource *source =
        [[MLNShapeSource alloc] initWithIdentifier:@"shape-source-removing-before-adding"
                                             shape:nil
                                           options:nil];

    // Attempting to find a layer with identifier will trigger an exception if the source associated
    // with the layer is not added
    [self.style addSource:source];

    MLNFillStyleLayer *fillLayer = [[MLNFillStyleLayer alloc] initWithIdentifier:@"fill-layer"
                                                                          source:source];
    [self.style removeLayer:fillLayer];
    [self.style addLayer:fillLayer];
    XCTAssertNotNil([self.style layerWithIdentifier:fillLayer.identifier]);

    MLNSymbolStyleLayer *symbolLayer =
        [[MLNSymbolStyleLayer alloc] initWithIdentifier:@"symbol-layer" source:source];
    [self.style removeLayer:symbolLayer];
    [self.style addLayer:symbolLayer];
    XCTAssertNotNil([self.style layerWithIdentifier:symbolLayer.identifier]);

    MLNLineStyleLayer *lineLayer = [[MLNLineStyleLayer alloc] initWithIdentifier:@"line-layer"
                                                                          source:source];
    ...
    MLNCircleStyleLayer *circleLayer =
        [[MLNCircleStyleLayer alloc] initWithIdentifier:@"circle-layer" source:source];
    [self.style removeLayer:circleLayer];
    [self.style addLayer:circleLayer];
    XCTAssertNotNil([self.style layerWithIdentifier:circleLayer.identifier]);

    MLNBackgroundStyleLayer *backgroundLayer =
        [[MLNBackgroundStyleLayer alloc] initWithIdentifier:@"background-layer"];
  ...
  {
    MLNRasterTileSource *rasterSource =
        [[MLNRasterTileSource alloc] initWithIdentifier:@"raster-tile-source"
                                       tileURLTemplates:@[]
                                                options:nil];
    ...
    MLNRasterStyleLayer *rasterLayer =
        [[MLNRasterStyleLayer alloc] initWithIdentifier:@"raster-layer" source:rasterSource];
...
- (void)testAddingLayerOfTypeABeforeRemovingLayerOfTypeBWithSameIdentifier {
  MLNShapeSource *source = [[MLNShapeSource alloc] initWithIdentifier:@"shape-source-identifier"
                                                                shape:nil
                                                              options:nil];
  [self.style addSource:source];

  // Add a fill layer
  MLNFillStyleLayer *fillLayer = [[MLNFillStyleLayer alloc] initWithIdentifier:@"some-identifier"
                                                                        source:source];
  [self.style addLayer:fillLayer];

  // Attempt to remove a line layer with the same identifier as the fill layer
  MLNLineStyleLayer *lineLayer = [[MLNLineStyleLayer alloc] initWithIdentifier:fillLayer.identifier
                                                                        source:source];
  [self.style removeLayer:lineLayer];

  XCTAssertTrue([[self.style layerWithIdentifier:fillLayer.identifier]
      isMemberOfClass:[MLNFillStyleLayer class]]);
}

- (NSString *)stringWithContentsOfStyleHeader {
  NSURL *styleHeaderURL = [[[NSBundle mgl_frameworkBundle].bundleURL
      URLByAppendingPathComponent:@"Headers"
                      isDirectory:YES] URLByAppendingPathComponent:@"MLNStyle.h"];
  NSError *styleHeaderError;
  NSString *styleHeader = [NSString stringWithContentsOfURL:styleHeaderURL
                                               usedEncoding:nil
                                                      error:&styleHeaderError];
...
- (void)testLayersOrder {
  ...
  MLNCircleStyleLayer *layer1 = [[MLNCircleStyleLayer alloc] initWithIdentifier:@"layer1"
                                                                         source:source];
  [self.style addLayer:layer1];

  MLNCircleStyleLayer *layer3 = [[MLNCircleStyleLayer alloc] initWithIdentifier:@"layer3"
                                                                         source:source];
  [self.style addLayer:layer3];

  MLNCircleStyleLayer *layer2 = [[MLNCircleStyleLayer alloc] initWithIdentifier:@"layer2"
                                                                         source:source];
  [self.style insertLayer:layer2 aboveLayer:layer1];

  MLNCircleStyleLayer *layer4 = [[MLNCircleStyleLayer alloc] initWithIdentifier:@"layer4"
                                                                         source:source];
  [self.style insertLayer:layer4 aboveLayer:layer3];

  MLNCircleStyleLayer *layer0 = [[MLNCircleStyleLayer alloc] initWithIdentifier:@"layer0"
                                                                         source:source];
...
- (void)testLanguageMatching {
  {
    ...
    XCTAssertEqualObjects(
        [MLNVectorTileSource preferredMapboxStreetsLanguageForPreferences:preferences], @"en");
  }
  {
  ...
  }
  {
    ...
    XCTAssertEqualObjects(
        [MLNVectorTileSource preferredMapboxStreetsLanguageForPreferences:preferences], @"fr");
  }
  {
    NSArray *preferences = @[ @"zh-Hans" ];
    XCTAssertEqualObjects(
        [MLNVectorTileSource preferredMapboxStreetsLanguageForPreferences:preferences], @"zh-Hans");
  }
  {
  ...
  }
  {
    ...
    XCTAssertEqualObjects(
        [MLNVectorTileSource preferredMapboxStreetsLanguageForPreferences:preferences], @"zh-Hant");
  }
  {
...
- (void)testPerformsPlacementTransitions {
  XCTAssertTrue(self.style.performsPlacementTransitions,
                @"The default value for enabling placement transitions should be YES.");

  self.style.performsPlacementTransitions = NO;
  XCTAssertFalse(self.style.performsPlacementTransitions,
                 @"Enabling placement transitions should be NO.");
}
...
- (void)testStyleJSON {
  ...
  NSError *error = nil;
  id jsonObject =
      [NSJSONSerialization JSONObjectWithData:[styleJSON dataUsingEncoding:NSUTF8StringEncoding]
                                      options:0
                                        error:&error];
  ...
  XCTAssertTrue([jsonObject isKindOfClass:[NSDictionary class]],
                @"Style JSON should represent a dictionary");
}
