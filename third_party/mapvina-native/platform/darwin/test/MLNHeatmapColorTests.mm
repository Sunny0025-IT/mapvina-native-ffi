...
- (void)testProperties {
  ...
  NSExpression *constantExpression2 =
      [NSExpression expressionWithFormat:@"%@", [MLNColor blueColor]];
#if TARGET_OS_IPHONE
  XCTExpectFailure(
      @"Awaiting unit test refactoring for https://github.io/github/mapvina/maplibre-native/issues/331");
#endif
  ...
  functionExpression =
      [NSExpression expressionWithFormat:@"mgl_step:from:stops:($zoomLevel, %@, %@)",
                                         constantExpression, @{@18 : constantExpression}];
  XCTAssertThrowsSpecificNamed(layer.heatmapColor = functionExpression, NSException,
                               NSInvalidArgumentException,
                               @"MLNHeatmapLayer should raise an exception if a camera expression "
                               @"is applied to heatmapColor.");
  functionExpression = [NSExpression expressionForKeyPath:@"bogus"];
  XCTAssertThrowsSpecificNamed(layer.heatmapColor = functionExpression, NSException,
                               NSInvalidArgumentException,
                               @"MLNHeatmapLayer should raise an exception if a data expression is "
                               @"applied to heatmapColor.");
  functionExpression =
      [NSExpression expressionWithFormat:@"mgl_step:from:stops:(bogus, %@, %@)", constantExpression,
  ...
  XCTAssertThrowsSpecificNamed(
      layer.heatmapColor = functionExpression, NSException, NSInvalidArgumentException,
      @"MLNHeatmapLayer should raise an exception if a camera-data expression is applied to a "
      @"property that does not support key paths to feature attributes.");
}
