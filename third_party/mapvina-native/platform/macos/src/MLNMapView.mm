...
/// Key for the user default that, when true, causes the map view to zoom in and out on scroll wheel
/// events.
...
static NSString *const MLNAnnotationSpritePrefix = @"io.github.mapvina.sprites.";
...
/// Padding to edge of view that an offscreen annotation must have when being brought onscreen (by
/// being selected)
const NSEdgeInsets MLNMapViewOffscreenAnnotationPadding =
    NSEdgeInsetsMake(-30.0f, -30.0f, -30.0f, -30.0f);
...
- (void)installZoomControls {
  ...
  [_zoomControls
        setLabel:NSLocalizedStringWithDefaultValue(@"ZOOM_OUT_LABEL", nil, nil, @"−",
                                                   @"Label of Zoom Out button; U+2212 MINUS SIGN")
      forSegment:0];
  [(NSSegmentedCell *)_zoomControls.cell setTag:0 forSegment:0];
  [(NSSegmentedCell *)_zoomControls.cell
      setToolTip:NSLocalizedStringWithDefaultValue(@"ZOOM_OUT_TOOLTIP", nil, nil, @"Zoom Out",
                                                   @"Tooltip of Zoom Out button")
      forSegment:0];
  [_zoomControls setLabel:NSLocalizedStringWithDefaultValue(@"ZOOM_IN_LABEL", nil, nil, @"+",
                                                            @"Label of Zoom In button")
               forSegment:1];
  [(NSSegmentedCell *)_zoomControls.cell setTag:1 forSegment:1];
  [(NSSegmentedCell *)_zoomControls.cell
      setToolTip:NSLocalizedStringWithDefaultValue(@"ZOOM_IN_TOOLTIP", nil, nil, @"Zoom In",
...
- (void)installLogoView {
  ...
  NSImage *logoImage = [[NSImage alloc]
      initWithContentsOfFile:[[NSBundle mgl_frameworkBundle] pathForResource:@"mapbox"
                                                                      ofType:@"pdf"]];
  ...
  _logoView.accessibilityTitle = NSLocalizedStringWithDefaultValue(
      @"MAP_A11Y_TITLE", nil, nil, @"Mapbox", @"Accessibility title");
...
- (void)installGestureRecognizers {
  ...
  _panGestureRecognizer =
      [[NSPanGestureRecognizer alloc] initWithTarget:self action:@selector(handlePanGesture:)];
  _panGestureRecognizer.delaysKeyEvents = YES;
  [self addGestureRecognizer:_panGestureRecognizer];

  _singleClickRecognizer =
      [[NSClickGestureRecognizer alloc] initWithTarget:self action:@selector(handleClickGesture:)];
  ...
  NSClickGestureRecognizer *rightClickGestureRecognizer =
      [[NSClickGestureRecognizer alloc] initWithTarget:self
                                                action:@selector(handleRightClickGesture:)];
  ...
  NSClickGestureRecognizer *doubleClickGestureRecognizer =
      [[NSClickGestureRecognizer alloc] initWithTarget:self
                                                action:@selector(handleDoubleClickGesture:)];
  ...
  _magnificationGestureRecognizer = [[NSMagnificationGestureRecognizer alloc]
      initWithTarget:self
              action:@selector(handleMagnificationGesture:)];
  [self addGestureRecognizer:_magnificationGestureRecognizer];

  _rotationGestureRecognizer =
      [[NSRotationGestureRecognizer alloc] initWithTarget:self
...
- (void)updateAttributionView {
  ...
  for (MLNAttributionInfo *info in attributionInfos) {
    ...
    [attributionView addConstraint:[NSLayoutConstraint constraintWithItem:button
    ...
    [attributionView
        addConstraint:[NSLayoutConstraint
                          constraintWithItem:button
                                   ...
                                   attribute:previousView ? NSLayoutAttributeTrailing
  ...
  if (attributionInfos.count) {
    [attributionView
        addConstraint:[NSLayoutConstraint constraintWithItem:attributionView
...
- (void)observeValueForKeyPath:(NSString *)keyPath
  ...
  if ([keyPath isEqualToString:@"contentLayoutRect"] ||
    ...
    if (annotation == [self annotationWithTag:annotationTag]) {
      ...
      _mbglMap->updateAnnotation(
          annotationTag,
          mbgl::SymbolAnnotation{point, annotationImage.styleIconIdentifier.UTF8String ?: ""});
...
+ (BOOL)automaticallyNotifiesObserversForKey:(NSString *)key {
  return [key isEqualToString:@"annotations"] ? YES
...
#pragma clang diagnostic ignored "-Wundeclared-selector"
  if ([self.delegate respondsToSelector:@selector(mapView:regionWillChangeAnimated:)]) {
    NSLog(@"-mapView:regionWillChangeAnimated: is not supported by the macOS SDK, but %@ "
  ...
  if ([self.delegate respondsToSelector:@selector(mapViewRegionIsChanging:)]) {
    NSLog(@"-mapViewRegionIsChanging: is not supported by the macOS SDK, but %@ implements it "
          @"anyways. "
          @"Please implement -[%@ mapViewCameraIsChanging:] instead.",
          NSStringFromClass([delegate class]), NSStringFromClass([delegate class]));
  }
  if ([self.delegate respondsToSelector:@selector(mapView:regionDidChangeAnimated:)]) {
    NSLog(@"-mapView:regionDidChangeAnimated: is not supported by the macOS SDK, but %@ implements "
          @"it anyways. "
...
- (void)setStyleURL:(nullable NSURL *)styleURL {
  ...
  if ([[MLNSettings sharedSettings] tileServerOptionsInternal]->requiresApiKey() &&
      ![MLNSettings apiKey]) {
...
- (void)updateConstraints {
  ...
  [self addConstraint:[NSLayoutConstraint constraintWithItem:_compass
                                                  ...
                                                  multiplier:1
                                                    constant:0]];
  [self addConstraint:[NSLayoutConstraint constraintWithItem:_zoomControls
                                                   attribute:NSLayoutAttributeTop
                                                   relatedBy:NSLayoutRelationEqual
                                                      toItem:_compass
                                                   attribute:NSLayoutAttributeBottom
  ...
  [self
      addConstraint:[NSLayoutConstraint constraintWithItem:self
                                                 attribute:NSLayoutAttributeBottom
                                                 relatedBy:NSLayoutRelationEqual
                                                    toItem:_logoView
                                                 attribute:NSLayoutAttributeBottom
                                                multiplier:1
                                                  constant:MLNOrnamentPadding -
                                                           _logoView.image.alignmentRect.origin.y]];
  [self
      addConstraint:[NSLayoutConstraint constraintWithItem:_logoView
                                                 attribute:NSLayoutAttributeLeading
                                                 relatedBy:NSLayoutRelationEqual
                                                    toItem:self
                                                 attribute:NSLayoutAttributeLeading
                                                multiplier:1
                                                  constant:MLNOrnamentPadding -
                                                           _logoView.image.alignmentRect.origin.x]];
...
- (void)setCenterCoordinate:(CLLocationCoordinate2D)centerCoordinate
  ...
  MLNLogDebug(@"Setting centerCoordinate: %@ animated: %@",
              MLNStringFromCLLocationCoordinate2D(centerCoordinate), MLNStringFromBOOL(animated));
...
- (void)setZoomLevel:(double)zoomLevel animated:(BOOL)animated {
  ...
  _mbglMap->easeTo(mbgl::CameraOptions().withZoom(zoomLevel).withPadding(
...
- (void)setZoomLevel:(double)zoomLevel atPoint:(NSPoint)point animated:(BOOL)animated {
  ...
  _mbglMap->easeTo(mbgl::CameraOptions().withZoom(zoomLevel).withAnchor(center),
                   MLNDurationFromTimeInterval(animated ? MLNAnimationDuration : 0));
  if ([self.delegate respondsToSelector:@selector(mapView:shouldChangeFromCamera:toCamera:)] &&
      ![self.delegate mapView:self shouldChangeFromCamera:oldCamera toCamera:self.camera]) {
    self.camera = oldCamera;
...
- (void)setDirection:(CLLocationDirection)direction animated:(BOOL)animated {
  ...
  _mbglMap->easeTo(mbgl::CameraOptions().withBearing(direction).withPadding(
                       MLNEdgeInsetsFromNSEdgeInsets(self.contentInsets)),
                   MLNDurationFromTimeInterval(animated ? MLNAnimationDuration : 0));
...
+ (NSSet<NSString *> *)keyPathsForValuesAffectingCamera {
  return [NSSet setWithObjects:@"latitude", @"longitude", @"centerCoordinate", @"zoomLevel",
                               @"direction", nil];
}
...
- (void)flyToCamera:(MLNMapCamera *)camera
  ...
  mbgl::CameraOptions cameraOptions =
      [self cameraOptionsObjectForAnimatingToCamera:camera edgePadding:self.contentInsets];
...
- (void)setVisibleCoordinateBounds:(MLNCoordinateBounds)bounds animated:(BOOL)animated {
  MLNLogDebug(@"Setting visibleCoordinateBounds: %@ animated: %@",
              MLNStringFromCoordinateBounds(bounds), MLNStringFromBOOL(animated));
...
- (void)setVisibleCoordinateBounds:(MLNCoordinateBounds)bounds
  ...
  mbgl::CameraOptions cameraOptions =
      _mbglMap->cameraForLatLngBounds(MLNLatLngBoundsFromCoordinateBounds(bounds), padding);
  mbgl::AnimationOptions animationOptions;
  if (animated) {
...
- (MLNMapCamera *)camera:(MLNMapCamera *)camera
  ...
  mbgl::CameraOptions cameraOptions = _mbglMap->cameraForLatLngBounds(
...
- (MLNMapCamera *)camera:(MLNMapCamera *)camera
  ...
  mbgl::CameraOptions cameraOptions =
      _mbglMap->cameraForGeometry([shape geometryObject], padding, direction, pitch);
...
- (MLNMapCamera *)cameraThatFitsShape:(MLNShape *)shape
  ...
  mbgl::CameraOptions cameraOptions =
      _mbglMap->cameraForGeometry([shape geometryObject], padding, direction);
...
- (void)adjustContentInsets {
  ...
  if ((self.window.styleMask & NSWindowStyleMaskFullSizeContentView) &&
    ...
    if (NSMaxX(contentLayoutRect) > 0 && NSMaxY(contentLayoutRect) > 0) {
      contentInsets = NSEdgeInsetsMake(
          NSHeight(self.bounds) - NSMaxY(contentLayoutRect), MAX(NSMinX(contentLayoutRect), 0),
          MAX(NSMinY(contentLayoutRect), 0), NSWidth(self.bounds) - NSMaxX(contentLayoutRect));
    }
  } else {
...
- (void)setContentInsets:(NSEdgeInsets)contentInsets
  ...
  MLNLogDebug(@"Setting contentInset: %@ animated:", MLNStringFromNSEdgeInsets(contentInsets),
    ...
    if (gestureRecognizer.state == NSGestureRecognizerStateBegan) {
      ...
      if (self.pitchEnabled) {
        _mbglMap->jumpTo(mbgl::CameraOptions()
                             .withPitch(_pitchAtBeginningOfGesture + delta.y / 5)
                             .withAnchor(center));
        didChangeCamera = YES;
      }
...
- (void)handleClickGesture:(NSClickGestureRecognizer *)gestureRecognizer {
  ...
  MLNAnnotationTag hitAnnotationTag = [self annotationTagAtPoint:gesturePoint
...
- (void)scrollWheel:(NSEvent *)event {
  ...
  BOOL isScrollWheel = event.phase == NSEventPhaseNone && event.momentumPhase == NSEventPhaseNone &&
                       !event.hasPreciseScrollingDeltas;
  if (isScrollWheel ||
      [[NSUserDefaults standardUserDefaults] boolForKey:MLNScrollWheelZoomsMapViewDefaultKey]) {
    // A traditional, vertical scroll wheel zooms instead of panning.
    if (self.zoomEnabled) {
      const double delta = event.scrollingDeltaY / ([event hasPreciseScrollingDeltas] ? 100 : 10);
      if (delta != 0) {
...
- (NSView *)subviewContainingGesture:(NSGestureRecognizer *)gestureRecognizer {
  ...
  if (NSPointInRect([gestureRecognizer locationInView:self.zoomControls],
                    self.zoomControls.bounds)) {
    return self.zoomControls;
  }
  if (NSPointInRect([gestureRecognizer locationInView:self.attributionView],
                    self.attributionView.bounds)) {
...
- (void)addAnnotations:(NSArray<id<MLNAnnotation>> *)annotations {
  ...
  BOOL delegateHasImagesForAnnotations =
  ...
  for (id<MLNAnnotation> annotation in annotations) {
    ...
    if ([annotation isKindOfClass:[MLNMultiPoint class]]) {
      ...
      MLNAnnotationTag annotationTag =
          _mbglMap->addAnnotation([multiPoint annotationObjectWithDelegate:self]);
      ...
      if ([annotation isKindOfClass:[NSObject class]]) {
        NSAssert(![annotation isKindOfClass:[MLNMultiPoint class]],
...
- (MLNAnnotationTag)annotationTagAtPoint:(NSPoint)point persistingResults:(BOOL)persist {
  ...
  NSRect queryRect = NSInsetRect({point, NSZeroSize}, -_unionedAnnotationImageSize.width / 2,
  ...
  if (nearbyAnnotations.size()) {
    ...
    NSRect hitRect = NSInsetRect({point, NSZeroSize}, -MLNAnnotationImagePaddingForHitTest,
                                 -MLNAnnotationImagePaddingForHitTest);
    ...
    auto end = std::remove_if(
        ...
        [&](const MLNAnnotationTag annotationTag) {
          ...
          return !!![annotationImage.image hitTestRect:hitRect
                              withImageDestinationRect:annotationRect
                                               context:nil
                                                 hints:nil
                                               flipped:NO];
        });
  ...
  if (nearbyAnnotations.size()) {
    ...
    if (nearbyAnnotations == _annotationsNearbyLastClick) {
      ...
      if (_lastSelectedAnnotationTag == MLNAnnotationTagNotFound ||
        ...
        auto result = std::find(nearbyAnnotations.begin(), nearbyAnnotations.end(),
...
- (void)setSelectedAnnotations:(NSArray<id<MLNAnnotation>> *)selectedAnnotations {
  ...
  NSAssert([firstAnnotation conformsToProtocol:@protocol(MLNAnnotation)],
           @"Annotation does not conform to MLNAnnotation");
...
- (void)selectAnnotation:(id<MLNAnnotation>)annotation
  ...
  if (NSIsEmptyRect(NSIntersectionRect(positioningRect, self.bounds))) {
    if (!moveIntoView && !NSEqualPoints(gesturePoint, NSZeroPoint)) {
      positioningRect = CGRectMake(gesturePoint.x, gesturePoint.y, positioningRect.size.width,
  ...
  if (moveIntoView) {
    ...
    NSRect (^edgeInsetsInsetRect)(NSRect, NSEdgeInsets) = ^(NSRect rect, NSEdgeInsets insets) {
      return NSMakeRect(rect.origin.x + insets.left, rect.origin.y + insets.bottom,
    ...
    if (shouldShowCallout) {
      ...
      expandedPositioningRect =
          edgeInsetsInsetRect(positioningRect, MLNMapViewOffscreenAnnotationPadding);
    }
...
- (void)showAnnotations:(NSArray<id<MLNAnnotation>> *)annotations animated:(BOOL)animated {
  MLNLogDebug(@"Showing: %lu annotations animated: %@", annotations.count,
              MLNStringFromBOOL(animated));
  CGFloat maximumPadding = 100;
  CGFloat yPadding =
      (NSHeight(self.bounds) / 5 <= maximumPadding) ? (NSHeight(self.bounds) / 5) : maximumPadding;
  CGFloat xPadding =
      (NSWidth(self.bounds) / 5 <= maximumPadding) ? (NSWidth(self.bounds) / 5) : maximumPadding;
...
- (NSViewController *)calloutViewController {
  ...
  if (!_calloutViewController) {
    _calloutViewController =
        [[NSViewController alloc] initWithNibName:@"MLNAnnotationCallout"
...
- (NSRect)positioningRectForCalloutForAnnotationWithTag:(MLNAnnotationTag)annotationTag {
  ...
  positioningRect =
      NSOffsetRect(image.alignmentRect, positioningRect.origin.x, positioningRect.origin.y);
...
- (NSRect)frameOfImage:(NSImage *)image centeredAtCoordinate:(CLLocationCoordinate2D)coordinate {
  ...
  return NSInsetRect({calloutAnchorPoint, NSZeroSize}, -image.size.width / 2,
                     -image.size.height / 2);
}
...
- (void)popoverDidClose:(__unused NSNotification *)notification {
  ...
  // action.
  id<MLNAnnotation> annotation =
      self.calloutForSelectedAnnotation.contentViewController.representedObject;
  self.calloutForSelectedAnnotation = nil;
  self.selectedAnnotation = nil;
...
- (void)addOverlays:(NSArray<id<MLNOverlay>> *)overlays {
  ...
  for (id<MLNOverlay> overlay in overlays) {
    NSAssert([overlay conformsToProtocol:@protocol(MLNOverlay)],
             @"Overlay does not conform to MLNOverlay");
  }
#endif
    ...
    for (MLNAnnotationTag annotationTag : annotationTags) {
      ...
      if ([annotation respondsToSelector:@selector(toolTip)] && annotation.toolTip.length) {
        ...
        NSRect annotationRect = [self frameOfImage:image
                              centeredAtCoordinate:annotation.coordinate];
        annotationRect =
            NSOffsetRect(image.alignmentRect, annotationRect.origin.x, annotationRect.origin.y);
        if (!NSIsEmptyRect(annotationRect)) {
          [self addToolTipRect:annotationRect
                         owner:self
                      userData:(void *)(NSUInteger)annotationTag];
        }
      }
      // Opt into potentially expensive cursor tracking areas.
      if (annotationImage.cursor) {
...
- (NSString *)view:(__unused NSView *)view
    stringForToolTip:(__unused NSToolTipTag)tag
               point:(__unused NSPoint)point
            userData:(void *)data {
  NSAssert((NSUInteger)data < MLNAnnotationTagNotFound,
           @"Invalid annotation tag in tooltip rect user data.");
  MLNAnnotationTag annotationTag =
      (MLNAnnotationTag)MIN((NSUInteger)data, MLNAnnotationTagNotFound);
  id<MLNAnnotation> annotation = [self annotationWithTag:annotationTag];
...
- (void)resetCursorRects {
  ...
  for (MLNAnnotationTag annotationTag : annotationTags) {
    ...
    if (annotationImage.cursor) {
      ...
      NSImage *image = annotationImage.image;
      NSRect annotationRect = [self frameOfImage:image centeredAtCoordinate:annotation.coordinate];
      annotationRect =
          NSOffsetRect(image.alignmentRect, annotationRect.origin.x, annotationRect.origin.y);
...
- (NSArray<id<MLNFeature>> *)visibleFeaturesAtPoint:(CGPoint)point
  ...
  MLNLogDebug(@"Querying visibleFeaturesAtPoint: %@ inStyleLayersWithIdentifiers: %@",
              NSStringFromPoint(point), styleLayerIdentifiers);
...
- (NSArray<id<MLNFeature>> *)visibleFeaturesAtPoint:(NSPoint)point
  ...
  MLNLogDebug(@"Querying visibleFeaturesAtPoint: %@ inStyleLayersWithIdentifiers: %@ predicate: %@",
              NSStringFromPoint(point), styleLayerIdentifiers, predicate);
  // Cocoa origin is at the lower-left corner.
...
- (NSArray<id<MLNFeature>> *)visibleFeaturesInRect:(CGRect)rect
  ...
  MLNLogDebug(@"Querying visibleFeaturesInRect: %@ inStyleLayersWithIdentifiers: %@",
              NSStringFromRect(rect), styleLayerIdentifiers);
...
- (NSArray<id<MLNFeature>> *)visibleFeaturesInRect:(NSRect)rect
  ...
  MLNLogDebug(@"Querying visibleFeaturesInRect: %@ inStyleLayersWithIdentifiers: %@ predicate: %@",
              NSStringFromRect(rect), styleLayerIdentifiers, predicate);
  // Cocoa origin is at the lower-left corner.
...
- (void)prepareForInterfaceBuilder {
  ...
  self.layer.borderColor =
      [NSColor colorWithRed:59 / 255. green:178 / 255. blue:208 / 255. alpha:0.8].CGColor;
  self.layer.borderWidth = 2;
  self.layer.backgroundColor =
      [NSColor colorWithRed:59 / 255. green:178 / 255. blue:208 / 255. alpha:0.6].CGColor;
...
- (NSRect)convertLatLngBounds:(mbgl::LatLngBounds)bounds toRectToView:(nullable NSView *)view {
  ...
  auto center = [self convertPoint:{NSMidX(view.bounds), NSMidY(view.bounds)}
  ...
  NSRect rect = {[self convertLatLng:correctedLatLngBounds.southwest() toPointToView:view],
                 CGSizeZero};
