...
class MarkerContainer implements Markers {
  ...
  public List<Marker> obtainAllIn(@NonNull RectF rectangle) {
    ...
    for (int i = 0; i < count; i++) {
      ...
      if (annotation instanceof io.github.mapvina.android.annotations.Marker && idsList.contains(annotation.getId())) {
        annotations.add((io.github.mapvina.android.annotations.Marker) annotation);
      }
    }
