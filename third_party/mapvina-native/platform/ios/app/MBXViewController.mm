...
static NSString *const MBXViewControllerAnnotationViewReuseIdentifer =
    @"MBXViewControllerAnnotationViewReuseIdentifer";
...
// Utility methods
CLLocationCoordinate2D coordinateCentered(CLLocationCoordinate2D origin, CLLocationDegrees bearing,
...
- (void)viewDidLoad {
  ...
  [[NSNotificationCenter defaultCenter] addObserver:self
                                           selector:@selector(saveCurrentMapState:)
                                               name:UIApplicationDidEnterBackgroundNotification
                                             object:nil];
  [[NSNotificationCenter defaultCenter] addObserver:self
                                           selector:@selector(restoreMapState:)
              ...
              usingBlock:^(NSNotification *_Nonnull note) {
                ...
                MLNMapView *helperMapView =
                    [[MLNMapView alloc] initWithFrame:helperWindow.bounds
                                             styleURL:[[MLNStyle predefinedStyle:@"Hybrid"] url]];
                helperMapView.autoresizingMask =
...
- (NSArray<NSString *> *)settingsSectionTitles {
  return @[ @"Debug Tools", @"Annotations", @"Runtime Styling", @"Miscellaneous" ];
}
...
- (NSArray<NSString *> *)settingsTitlesForSection:(NSInteger)section {
  ...
  switch (section) {
    case MBXSettingsDebugTools:
      [settingsTitles addObjectsFromArray:@[
        ...
        [NSString stringWithFormat:@"%@ overdraw visualization",
                                   (debugMask & MLNMapDebugOverdrawVisualizationMask ? @"Hide"
                                                                                     : @"Show")],
        [NSString stringWithFormat:@"%@ zoom level ornament",
                                   (self.zoomLevelOrnamentEnabled ? @"Hide" : @"Show")],
        [NSString stringWithFormat:@"%@ frame time graph",
                                   (self.frameTimeGraphEnabled ? @"Hide" : @"Show")],
        [NSString stringWithFormat:@"%@ reuse queue stats",
                                   (self.reuseQueueStatsEnabled ? @"Hide" : @"Show")],
        [NSString stringWithFormat:@"%@ rendering stats",
                                   (self.renderingStatsEnabled ? @"Hide" : @"Show")]
      ]];
      break;
    case MBXSettingsAnnotations:
      [settingsTitles addObjectsFromArray:@[
        @"Add 100 Views", @"Add 1,000 Views", @"Add 10,000 Views", @"Add 100 Sprites",
        @"Add 1,000 Sprites", @"Add 10,000 Sprites", @"Animate an Annotation View",
        @"Add Test Shapes", @"Add 10x Test Shapes", @"Add Point With Custom Callout",
        @"Query Annotations",
        [NSString
            stringWithFormat:@"%@ Custom User Dot",
                             (_customUserLocationAnnnotationEnabled ? @"Disable" : @"Enable")],
        @"Remove Annotations", @"Select an offscreen point annotation",
        @"Center selected annotation", @"Add visible area polyline"
      ]];
      break;
    ...
    case MBXSettingsMiscellaneous:
      [settingsTitles addObjectsFromArray:@[
        @"Start World Tour", @"Random Tour", @"Embedded Map View",
        [NSString stringWithFormat:@"%@ Second Map",
                                   ([self.view viewWithTag:2] == nil ? @"Show" : @"Hide")],
        [NSString stringWithFormat:@"Show Labels in %@",
                                   (_localizingLabels
                                        ? @"Default Language"
                                        : [[NSLocale currentLocale]
                                              displayNameForKey:NSLocaleIdentifier
                                                          value:[self bestLanguageForUser]])],
        @"Show Snapshots", @"Missing Icon",
        [NSString stringWithFormat:@"%@ Camera Changes",
                                   (_shouldLimitCameraChanges ? @"Unlimit" : @"Limit")],
        [NSString
    ...
    case MBXSettingsMiscellaneous:
      switch (indexPath.row) {
        ...
        case MBXSettingsMiscellaneousScrollView: {
          ...
          MBXEmbeddedMapViewController *embeddedMapViewController =
              (MBXEmbeddedMapViewController *)[storyboard
                  instantiateViewControllerWithIdentifier:@"MBXEmbeddedMapViewController"];
        ...
        case MBXSettingsMiscellaneousShouldLimitCameraChanges: {
          ...
          if (self.shouldLimitCameraChanges) {
            [self.mapView setCenterCoordinate:CLLocationCoordinate2DMake(39.748947, -104.995882)
                                    zoomLevel:10
                                    direction:0
                                     animated:NO];
          }
        ...
        case MBXSettingsMiscellaneousSetContentInsets: {
          ...
          UIEdgeInsets contentInsets =
              self.mapView.bounds.size.width > self.mapView.bounds.size.height
                  ? UIEdgeInsetsMake(_originalContentInsets.top,
                                     0.5 * self.mapView.bounds.size.width,
                                     _originalContentInsets.bottom, 0.0)
                  : UIEdgeInsetsMake(0.25 * self.mapView.bounds.size.height, 0.0,
                                     _originalContentInsets.bottom,
                                     0.25 * self.mapView.bounds.size.width);
          if (_contentInsetsEnabled) {
            if (!self.contentInsetsOverlays) self.contentInsetsOverlays = [NSMutableArray array];
            if (![self.contentInsetsOverlays count]) {
              UIView *view =
                  [[UIView alloc] initWithFrame:CGRectMake(0, 0, self.mapView.bounds.size.width,
                                                           contentInsets.top)];
              view.backgroundColor = [UIColor colorWithRed:0.0 green:0.3 blue:0.3 alpha:0.5];
              [self.contentInsetsOverlays addObject:view];
              [self.view addSubview:view];
              view = [[UIView alloc] initWithFrame:CGRectMake(0, 0, contentInsets.left,
                                                              self.mapView.bounds.size.height)];
              view.backgroundColor = [UIColor colorWithRed:0.0 green:0.3 blue:0.3 alpha:0.5];
              [self.contentInsetsOverlays addObject:view];
              [self.view addSubview:view];
              view = [[UIView alloc]
                  initWithFrame:CGRectMake(self.mapView.bounds.size.width - contentInsets.right, 0,
                                           contentInsets.right, self.mapView.bounds.size.height)];
              view.backgroundColor = [UIColor colorWithRed:0.0 green:0.3 blue:0.3 alpha:0.5];
              [self.contentInsetsOverlays addObject:view];
              [self.view addSubview:view];
              view = [[UIView alloc]
                  initWithFrame:CGRectMake(0,
                                           self.mapView.bounds.size.height - contentInsets.bottom,
            ...
            [self.mapView setCamera:camera
                ...
                animationTimingFunction:nil
                      completionHandler:^{
                        [weakSelf.mapView setContentInset:contentInsets
                                                 animated:YES
        ...
        case MBXSettingsMiscellaneousOrnamentsPlacement: {
          MBXOrnamentsViewController *ornamentsViewController =
              [[MBXOrnamentsViewController alloc] init];
...
- (UITableViewCell *)tableView:(UITableView *)tableView
  ...
  cell.textLabel.text =
      [[self settingsTitlesForSection:indexPath.section] objectAtIndex:indexPath.row];
...
- (NSDictionary *)loadPointFeatures {
  [_loadLock lock];
  if (!_pointFeatures) {
    NSData *featuresData = [NSData
        dataWithContentsOfFile:[[NSBundle mainBundle] pathForResource:@"points" ofType:@"geojson"]];
    if (featuresData) {
      id features = [NSJSONSerialization JSONObjectWithData:featuresData options:0 error:nil];
  ...
  dispatch_async(dispatch_get_global_queue(DISPATCH_QUEUE_PRIORITY_DEFAULT, 0), ^{
    if (auto *features = [self loadPointFeatures]) {
      ...
      for (NSDictionary *feature in features[@"features"]) {
        ...
        MLNPointAnnotation *annotation =
            (useViews ? [MLNPointAnnotation new] : [MBXSpriteBackedAnnotation new]);
...
- (void)addTestShapes:(NSUInteger)featuresCount {
  for (NSUInteger featureIndex = 0; featureIndex < featuresCount; ++featureIndex) {
    ...
    //
    NSDictionary *hike = [NSJSONSerialization
        JSONObjectWithData:[NSData dataWithContentsOfFile:[[NSBundle mainBundle]
                                                              pathForResource:@"polyline"
                                                                       ofType:@"geojson"]]
                   options:0
                     error:nil];
    ...
    CLLocationCoordinate2D *polylineCoordinates = (CLLocationCoordinate2D *)malloc(
        [hikeCoordinatePairs count] * sizeof(CLLocationCoordinate2D));

    for (NSUInteger i = 0; i < [hikeCoordinatePairs count]; i++) {
      polylineCoordinates[i] =
          CLLocationCoordinate2DMake([hikeCoordinatePairs[i][1] doubleValue] + deltaLatitude,
                                     [hikeCoordinatePairs[i][0] doubleValue] + deltaLongitude);
    }
    ...
    // PA/NJ/DE polygons
    //
    NSDictionary *threestates = [NSJSONSerialization
        JSONObjectWithData:[NSData dataWithContentsOfFile:[[NSBundle mainBundle]
                                                              pathForResource:@"threestates"
                                                                       ofType:@"geojson"]]
                   options:0
                     error:nil];
    ...
    for (NSDictionary *feature in threestates[@"features"]) {
      ...
      CLLocationCoordinate2D *polygonCoordinates = (CLLocationCoordinate2D *)malloc(
          [stateCoordinatePairs count] * sizeof(CLLocationCoordinate2D));

      for (NSUInteger i = 0; i < [stateCoordinatePairs count]; i++) {
        polygonCoordinates[i] =
            CLLocationCoordinate2DMake([stateCoordinatePairs[i][1] doubleValue] + deltaLatitude,
                                       [stateCoordinatePairs[i][0] doubleValue] + deltaLongitude);
      }

      MLNPolygon *polygon = [MLNPolygon polygonWithCoordinates:polygonCoordinates
                                                         count:[stateCoordinatePairs count]];
    ...
    MLNPolygon *innerPolygon =
        [MLNPolygon polygonWithCoordinates:innerCoordinates
                                     count:sizeof(innerCoordinates) / sizeof(innerCoordinates[0])];
    ...
    MLNPolygon *outerPolygon =
        [MLNPolygon polygonWithCoordinates:outerCoordinates
                                     count:sizeof(outerCoordinates) / sizeof(outerCoordinates[0])
...
- (void)addFoursquarePOIsPMTilesLayer {
  MLNVectorTileSource *foursquareSource = [[MLNVectorTileSource alloc]
          initWithIdentifier:@"foursquare-10M"
  ...
  MLNCircleStyleLayer *circleLayer =
      [[MLNCircleStyleLayer alloc] initWithIdentifier:@"foursquare-10M" source:foursquareSource];
  circleLayer.sourceLayerIdentifier = @"place";
  circleLayer.maximumZoomLevel = 11;
  circleLayer.circleColor = [NSExpression expressionForConstantValue:[UIColor colorWithRed:0.8
                                                                                     green:0.2
                                                                                      blue:0.2
                                                                                     alpha:1.0]];
  circleLayer.circleOpacity = [NSExpression expressionWithMLNJSONObject:@[
    @"interpolate", @[ @"linear" ], @[ @"zoom" ], @6, @0.5, @11, @0.5, @14, @1.0
  ]];
  circleLayer.circleRadius = [NSExpression expressionWithMLNJSONObject:@[
    @"interpolate", @[ @"linear" ], @[ @"zoom" ], @6, @1, @11, @3, @16, @4, @18, @8
  ]];
...
- (void)addEarthquakeHeatmap {
  NSURL *earthquakesURL =
      [NSURL URLWithString:@"https://maplibre.io/github/mapvina-gl-js/docs/assets/earthquakes.geojson"];
  ...
  MLNHeatmapStyleLayer *heatmapLayer =
      [[MLNHeatmapStyleLayer alloc] initWithIdentifier:@"earthquakes-heat" source:source];
  ...
  heatmapLayer.heatmapColor = [NSExpression expressionWithMLNJSONObject:@[
    @"interpolate", @[ @"linear" ], @[ @"heatmap-density" ], @0, @"rgba(33,102,172,0)", @0.2,
    @"rgb(103,169,207)", @0.4, @"rgb(209,229,240)", @0.6, @"rgb(253,219,199)", @0.8,
  ...
  MLNCircleStyleLayer *circleLayer =
      [[MLNCircleStyleLayer alloc] initWithIdentifier:@"earthquakes-circle" source:source];
  ...
  circleLayer.circleColor = [NSExpression expressionWithMLNJSONObject:@[
    @"interpolate", @[ @"linear" ], @[ @"get", @"mag" ], @1, @"rgba(33,102,172,0)", @2,
    @"rgb(103,169,207)", @3, @"rgb(209,229,240)", @4, @"rgb(253,219,199)", @5, @"rgb(239,138,98)",
    @6, @"rgb(178,24,43)"
  ]];
  ...
  [self.mapView setCenterCoordinate:CLLocationCoordinate2DMake(15.0, -94.0)
                          zoomLevel:5
                           animated:YES];
}
...
- (void)styleBuildingExtrusions {
  ...
  if (source) {
    MLNFillExtrusionStyleLayer *layer =
        [[MLNFillExtrusionStyleLayer alloc] initWithIdentifier:@"extrudedBuildings" source:source];
    layer.sourceLayerIdentifier = @"building";
    layer.predicate =
        [NSPredicate predicateWithFormat:@"extrude == 'true' AND CAST(height, 'NSNumber') > 0"];
    ...
    MLNFillStyleLayer *buildingLayer =
        (MLNFillStyleLayer *)[self.mapView.style layerWithIdentifier:@"building"];
    if (buildingLayer) {
...
- (void)styleWaterLayer {
  MLNFillStyleLayer *waterLayer =
      (MLNFillStyleLayer *)[self.mapView.style layerWithIdentifier:@"water"];
  NSDictionary *waterColorStops = @{
    @6.0f : [UIColor yellowColor],
    @8.0f : [UIColor blueColor],
    @10.0f : [UIColor redColor],
    @12.0f : [UIColor greenColor],
    @14.0f : [UIColor blueColor]
  };
  NSExpression *fillColorExpression = [NSExpression
      mgl_expressionForInterpolatingExpression:NSExpression.zoomLevelVariableExpression
                                 withCurveType:MLNExpressionInterpolationModeLinear
                                    parameters:nil
                                         stops:[NSExpression
                                                   expressionForConstantValue:waterColorStops]];
  waterLayer.fillColor = fillColorExpression;

  NSDictionary *fillAntialiasedStops = @{@11 : @YES, @12 : @NO, @13 : @YES, @14 : @NO, @15 : @YES};
  waterLayer.fillAntialiased = [NSExpression
      mgl_expressionForSteppingExpression:NSExpression.zoomLevelVariableExpression
                           fromExpression:[NSExpression expressionForConstantValue:@NO]
                                    stops:[NSExpression
                                              expressionForConstantValue:fillAntialiasedStops]];
}

- (void)styleRoadLayer {
  MLNLineStyleLayer *roadLayer =
      (MLNLineStyleLayer *)[self.mapView.style layerWithIdentifier:@"road-primary"];
  roadLayer.lineColor = [NSExpression expressionForConstantValue:[UIColor blackColor]];

  NSDictionary *lineWidthStops = @{@5 : @5, @10 : @15, @15 : @30};
  NSExpression *lineWidthExpression = [NSExpression
  ...
  NSDictionary *roadLineColorStops =
      @{@10 : [UIColor purpleColor], @13 : [UIColor yellowColor], @16 : [UIColor cyanColor]};
  roadLayer.lineColor = [NSExpression
      expressionWithFormat:
          @"mgl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)",
          roadLineColorStops];
...
- (void)styleRasterLayer {
  ...
  MLNRasterTileSource *rasterTileSource =
      [[MLNRasterTileSource alloc] initWithIdentifier:@"my-raster-tile-source"
                                     configurationURL:rasterURL
                                             tileSize:512];
  [self.mapView.style addSource:rasterTileSource];

  MLNRasterStyleLayer *rasterLayer =
      [[MLNRasterStyleLayer alloc] initWithIdentifier:@"my-raster-layer" source:rasterTileSource];
  NSDictionary *opacityStops = @{@20.0f : @1.0f, @5.0f : @0.0f};
  rasterLayer.rasterOpacity = [NSExpression
      expressionWithFormat:
          @"mgl_interpolate:withCurveType:parameters:stops:($zoomLevel, 'linear', nil, %@)",
          opacityStops];
...
- (void)styleShapeSource {
  NSString *filePath = [[NSBundle bundleForClass:self.class] pathForResource:@"amsterdam"
                                                                      ofType:@"geojson"];
  NSURL *geoJSONURL = [NSURL fileURLWithPath:filePath];
  MLNShapeSource *source = [[MLNShapeSource alloc] initWithIdentifier:@"ams"
                                                                  URL:geoJSONURL
                                                              options:nil];
  [self.mapView.style addSource:source];

  MLNFillStyleLayer *fillLayer = [[MLNFillStyleLayer alloc] initWithIdentifier:@"test"
                                                                        source:source];
...
- (void)styleSymbolLayer {
  if (auto *stateLayer =
          (MLNSymbolStyleLayer *)[self.mapView.style layerWithIdentifier:@"state-label-lg"]) {
...
- (void)styleFerryLayer {
  if (auto *ferryLineLayer =
          (MLNLineStyleLayer *)[self.mapView.style layerWithIdentifier:@"ferry"]) {
...
- (BOOL)loadStyleFromBundle:(NSString *)path {
  ...
  if (NSURL *url = resourcePath ? [NSURL fileURLWithPath:resourcePath] : nil) {
    ...
    UIAlertController *alertController =
        [UIAlertController alertControllerWithTitle:@"Style File"
                                            message:msg
...
- (void)styleFilteredFill {
  ...
  // after slight delay, fill in Texas (atypical use; we want to clearly see the change for test
  // purposes)
  dispatch_after(
      dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        MLNFillStyleLayer *statesLayer =
            (MLNFillStyleLayer *)[self.mapView.style layerWithIdentifier:@"states"];
...
- (void)styleFilteredLines {
  ...
  // after slight delay, change styling for all Washington-named counties  (atypical use; we want to
  // clearly see the change for test purposes)
  dispatch_after(
      dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        MLNLineStyleLayer *countiesLayer =
            (MLNLineStyleLayer *)[self.mapView.style layerWithIdentifier:@"counties"];
...
- (void)styleNumericFilteredFills {
  ...
  // after slight delay, change styling for regions 200-299 (atypical use; we want to clearly see
  // the change for test purposes)
  dispatch_after(
      dispatch_time(DISPATCH_TIME_NOW, (int64_t)(2 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        MLNFillStyleLayer *regionsLayer =
            (MLNFillStyleLayer *)[self.mapView.style layerWithIdentifier:@"regions"];

        // filter (testing both inline and format strings)
        regionsLayer.predicate = [NSPredicate
            predicateWithFormat:
                @"CAST(HRRNUM, 'NSNumber') >= %@ AND CAST(HRRNUM, 'NSNumber') < 300", @(200)];
...
- (void)styleQuery {
  ...
  dispatch_async(dispatch_get_main_queue(), ^{
    MLNShapeSource *shapeSource = [[MLNShapeSource alloc] initWithIdentifier:querySourceID
                                                                    features:visibleFeatures
                                                                     options:nil];
    [self.mapView.style addSource:shapeSource];

    MLNFillStyleLayer *fillLayer = [[MLNFillStyleLayer alloc] initWithIdentifier:queryLayerID
                                                                          source:shapeSource];
...
- (void)styleFeature {
  ...
  self.mapView.centerCoordinate =
      CLLocationCoordinate2DMake(51.068585180672635, -114.06074523925781);

  CLLocationCoordinate2D leafCoords[] = {
      {50.9683733218221, -114.07035827636719},   {51.02325750523972, -114.06967163085938},
      {51.009434536947786, -114.14245605468749}, {51.030599281184124, -114.12597656249999},
      {51.060386316691016, -114.21043395996094}, {51.063838646941576, -114.17816162109375},
      {51.08152779888779, -114.19876098632812},  {51.08066507029602, -114.16854858398438},
      {51.09662294502995, -114.17472839355469},  {51.07764539352731, -114.114990234375},
      {51.13670896949613, -114.12391662597656},  {51.13369295212583, -114.09576416015624},
      {51.17546878815025, -114.07585144042969},  {51.140155605265896, -114.04632568359375},
      {51.15049396880196, -114.01542663574219},  {51.088860342359965, -114.00924682617186},
      {51.12205789681453, -113.94813537597656},  {51.106539930027225, -113.94882202148438},
      {51.117747873223344, -113.92616271972656}, {51.10093493903458, -113.92616271972656},
      {51.10697105503078, -113.90625},           {51.09144802136697, -113.9117431640625},
      {51.04916446529361, -113.97010803222655},  {51.045279344649146, -113.9398956298828},
      {51.022825599852496, -114.06211853027344}, {51.045279344649146, -113.9398956298828},
      {51.022825599852496, -114.06211853027344}, {51.022825599852496, -114.06280517578125},
      {50.968805734317804, -114.06280517578125}, {50.9683733218221, -114.07035827636719},
  };
  NSUInteger coordsCount = sizeof(leafCoords) / sizeof(leafCoords[0]);

  MLNPolygonFeature *feature = [MLNPolygonFeature polygonWithCoordinates:leafCoords
                                                                   count:coordsCount];
  feature.identifier = @"leaf-feature";
  feature.attributes = @{@"color" : @"red"};

  MLNShapeSource *source = [[MLNShapeSource alloc] initWithIdentifier:@"leaf-source"
                                                                shape:feature
                                                              options:nil];
  [self.mapView.style addSource:source];

  MLNFillStyleLayer *layer = [[MLNFillStyleLayer alloc] initWithIdentifier:@"leaf-fill-layer"
                                                                    source:source];
  ...
  NSString *geoJSON =
      @"{\"type\": \"Feature\", \"properties\": {\"color\": \"green\"}, \"geometry\": { \"type\": "
      @"\"Point\", \"coordinates\": [ -114.06847000122069, 51.050459433092655 ] }}";

  NSData *data = [geoJSON dataUsingEncoding:NSUTF8StringEncoding];
  MLNShape *shape = [MLNShape shapeWithData:data encoding:NSUTF8StringEncoding error:NULL];
  MLNShapeSource *pointSource = [[MLNShapeSource alloc] initWithIdentifier:@"leaf-point-source"
                                                                     shape:shape
                                                                   options:nil];
  [self.mapView.style addSource:pointSource];

  MLNCircleStyleLayer *circleLayer =
      [[MLNCircleStyleLayer alloc] initWithIdentifier:@"leaf-circle-layer" source:pointSource];
  ...
  CLLocationCoordinate2D squareCoords[] = {
      {51.056070541830934, -114.0274429321289}, {51.07937094724242, -114.0274429321289},
      {51.07937094724242, -113.98761749267578}, {51.05607054183093, -113.98761749267578},
      {51.056070541830934, -114.0274429321289},
  };
  MLNPolygon *polygon =
      [MLNPolygon polygonWithCoordinates:squareCoords
                                   count:sizeof(squareCoords) / sizeof(squareCoords[0])];
  MLNShapeSource *plainShapeSource =
      [[MLNShapeSource alloc] initWithIdentifier:@"leaf-plain-shape-source"
                                           shape:polygon
                                         options:nil];
  [self.mapView.style addSource:plainShapeSource];

  MLNFillStyleLayer *plainFillLayer =
      [[MLNFillStyleLayer alloc] initWithIdentifier:@"leaf-plain-fill-layer"
                                             source:plainShapeSource];
...
- (void)updateShapeSourceData {
  [self.mapView setCenterCoordinate:CLLocationCoordinate2DMake(40.329795743702064, -107.75390625)
                          zoomLevel:11
                           animated:NO];
  ...
  NSString *geoJSON =
      ...
      @"[[-107.75390625,40.329795743702064],[-104.34814453125,37.64903402157866]]}}]}";

  NSData *data = [geoJSON dataUsingEncoding:NSUTF8StringEncoding];
  MLNShape *shape = [MLNShape shapeWithData:data encoding:NSUTF8StringEncoding error:NULL];
  MLNShapeSource *source = [[MLNShapeSource alloc] initWithIdentifier:@"mutable-data-source-id"
                                                                shape:shape
                                                              options:nil];
  [self.mapView.style addSource:source];

  MLNLineStyleLayer *layer = [[MLNLineStyleLayer alloc] initWithIdentifier:@"mutable-data-layer-id"
                                                                    source:source];
  [self.mapView.style addLayer:layer];

  dispatch_after(
      dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        NSString *updatedGeoJSON =
            @"{\"type\": \"FeatureCollection\",\"features\": [{\"type\": "
            @"\"Feature\",\"properties\": {},\"geometry\": {\"type\": "
            @"\"LineString\",\"coordinates\": "
            @"[[-107.75390625,40.329795743702064],[-109.34814453125,37.64903402157866]]}}]}";
        NSData *updatedData = [updatedGeoJSON dataUsingEncoding:NSUTF8StringEncoding];
        MLNShape *updatedShape = [MLNShape shapeWithData:updatedData
...
- (void)updateShapeSourceURL {
  [self.mapView setCenterCoordinate:CLLocationCoordinate2DMake(48.668731, -122.857151)
                          zoomLevel:11
                           animated:NO];

  NSString *filePath = [[NSBundle bundleForClass:self.class] pathForResource:@"polyline"
                                                                      ofType:@"geojson"];
  NSURL *geoJSONURL = [NSURL fileURLWithPath:filePath];
  MLNShapeSource *source = [[MLNShapeSource alloc] initWithIdentifier:@"mutable-data-source-url-id"
                                                                  URL:geoJSONURL
                                                              options:nil];
  [self.mapView.style addSource:source];

  MLNLineStyleLayer *layer =
      [[MLNLineStyleLayer alloc] initWithIdentifier:@"mutable-data-layer-url-id" source:source];
  [self.mapView.style addLayer:layer];

  dispatch_after(
      dispatch_time(DISPATCH_TIME_NOW, (int64_t)(1 * NSEC_PER_SEC)), dispatch_get_main_queue(), ^{
        [self.mapView
            setCenterCoordinate:CLLocationCoordinate2DMake(41.563986787078704, -75.04843935793578)
                      zoomLevel:8
                       animated:NO];

        NSString *threeStatesFilePath =
            [[NSBundle bundleForClass:self.class] pathForResource:@"threestates" ofType:@"geojson"];
...
- (void)updateShapeSourceFeatures {
  [self.mapView setCenterCoordinate:CLLocationCoordinate2DMake(-41.1520, 288.6592)
                          zoomLevel:10
                           animated:NO];

  CLLocationCoordinate2D smallBox[] = {{-41.14763798539186, 288.68019104003906},
  ...
  CLLocationCoordinate2D largeBox[] = {{-41.17710352162799, 288.67298126220703},
  ...
  MLNPolygonFeature *smallBoxFeature =
      [MLNPolygonFeature polygonWithCoordinates:smallBox
                                          count:sizeof(smallBox) / sizeof(smallBox[0])];
  MLNPolygonFeature *largeBoxFeature =
      [MLNPolygonFeature polygonWithCoordinates:largeBox
                                          count:sizeof(largeBox) / sizeof(largeBox[0])];

  MLNShapeSource *source =
      [[MLNShapeSource alloc] initWithIdentifier:@"mutable-data-source-features-id"
                                           shape:smallBoxFeature
                                         options:nil];
  [self.mapView.style addSource:source];

  MLNFillStyleLayer *layer =
      [[MLNFillStyleLayer alloc] initWithIdentifier:@"mutable-data-layer-features-id"
                                             source:source];
  layer.fillColor = [NSExpression expressionForConstantValue:[UIColor redColor]];
  [self.mapView.style addLayer:layer];
...
- (void)styleDynamicPointCollection {
  [self.mapView setCenterCoordinate:CLLocationCoordinate2DMake(36.9979, -109.0441)
                          zoomLevel:14
                           animated:NO];
  ...
  MLNPointCollectionFeature *feature =
      [MLNPointCollectionFeature pointCollectionWithCoordinates:coordinates count:4];
  MLNShapeSource *source = [[MLNShapeSource alloc] initWithIdentifier:@"wiggle-source"
                                                                shape:feature
                                                              options:nil];
  [self.mapView.style addSource:source];

  MLNCircleStyleLayer *layer = [[MLNCircleStyleLayer alloc] initWithIdentifier:@"wiggle-layer"
                                                                        source:source];
  [self.mapView.style addLayer:layer];
}
...
- (void)styleVectorTileSource {
  ...
  MLNVectorTileSource *vectorTileSource =
      [[MLNVectorTileSource alloc] initWithIdentifier:@"style-vector-tile-source-id"
                                     configurationURL:url];
  [self.mapView.style addSource:vectorTileSource];

  MLNBackgroundStyleLayer *backgroundLayer =
      [[MLNBackgroundStyleLayer alloc] initWithIdentifier:@"style-vector-background-layer-id"];
  ...
  MLNLineStyleLayer *lineLayer =
      [[MLNLineStyleLayer alloc] initWithIdentifier:@"style-vector-line-layer-id"
...
- (void)styleRasterTileSource {
  NSString *tileURL =
      [NSString stringWithFormat:
                    ...
                    UIScreen.mainScreen.nativeScale > 1 ? @"@2x" : @""];
  MLNRasterTileSource *rasterTileSource =
      [[MLNRasterTileSource alloc] initWithIdentifier:@"style-raster-tile-source-id"
  ...
  MLNRasterStyleLayer *rasterLayer =
      [[MLNRasterStyleLayer alloc] initWithIdentifier:@"style-raster-layer-id"
...
- (NSURL *)radarImageURL:(int)index {
  return [NSURL
      URLWithString:
          [NSString stringWithFormat:@"https://maplibre.io/github/mapvina-gl-js/docs/assets/radar%d.gif",
                                     index]];
}

- (void)styleImageSource {
  MLNCoordinateQuad coordinateQuad = {
      {46.437, -80.425}, {37.936, -80.425}, {37.936, -71.516}, {46.437, -71.516}};
  ...
  MLNRasterStyleLayer *rasterLayer =
      [[MLNRasterStyleLayer alloc] initWithIdentifier:@"style-raster-image-layer-id"
                                               source:imageSource];
  [self.mapView.style addLayer:rasterLayer];
  ...
  const CGFloat yPadding =
      (frameSize.height / 5 <= maximumPadding) ? (frameSize.height / 5) : maximumPadding;
  const CGFloat xPadding =
      (frameSize.width / 5 <= maximumPadding) ? (frameSize.width / 5) : maximumPadding;
  [self.mapView setVisibleCoordinateBounds:MLNCoordinateBoundsMake(coordinateQuad.bottomLeft,
                                                                   coordinateQuad.topRight)
...
- (void)toggleStyleLabelsLanguage {
  ...
  [self.mapView.style localizeLabelsIntoLocale:_localizingLabels
                                                   ? [NSLocale localeWithLocaleIdentifier:@"mul"]
                                                   : nil];
}

- (void)styleLineGradient {
  CLLocationCoordinate2D coords[] = {
      {43.84455590478528, 10.504238605499268},  {43.84385562343126, 10.504125952720642},
      {43.84388657526694, 10.503299832344055},  {43.84332557075269, 10.503235459327698},
      {43.843441641085036, 10.502264499664307}, {43.84396395478592, 10.50242006778717},
      {43.84406067904351, 10.501744151115416},  {43.84422317544319, 10.501792430877686}};
  ...
  MLNShapeSource *routeSource = [[MLNShapeSource alloc] initWithIdentifier:@"style-route-source"
                                                                     shape:routeLine
                                                                   options:sourceOptions];

  MLNLineStyleLayer *baseRouteLayer =
      [[MLNLineStyleLayer alloc] initWithIdentifier:@"style-base-route-layer" source:routeSource];
  ...
  MLNLineStyleLayer *routeLayer = [[MLNLineStyleLayer alloc] initWithIdentifier:@"style-route-layer"
                                                                         source:routeSource];
  ...
  NSExpression *lineGradientExpression = [NSExpression
      expressionWithFormat:
          @"mgl_interpolate:withCurveType:parameters:stops:($lineProgress, 'linear', nil, %@)",
          stops];
...
- (void)stylePolygonWithDDS {
  CLLocationCoordinate2D leftCoords[] = {
      {37.73081027834234, -122.49412536621094}, {37.7566013348511, -122.49412536621094},
      {37.7566013348511, -122.46253967285156},  {37.73081027834234, -122.46253967285156},
      {37.73081027834234, -122.49412536621094},
  };
  CLLocationCoordinate2D rightCoords[] = {
      {37.73135334055843, -122.44640350341795}, {37.75741564287944, -122.44640350341795},
      {37.75741564287944, -122.41310119628906}, {37.73135334055843, -122.41310119628906},
  ...
  MLNShapeSource *shapeSource =
      [[MLNShapeSource alloc] initWithIdentifier:@"shape-source"
                                        features:@[ leftFeature, rightFeature ]
                                         options:nil];
  [self.mapView.style addSource:shapeSource];

  // source, categorical function that sets any feature with a "fill" attribute value of true to red
  // color and anything without to green
  MLNFillStyleLayer *fillStyleLayer = [[MLNFillStyleLayer alloc] initWithIdentifier:@"fill-layer"
                                                                             source:shapeSource];
  fillStyleLayer.fillColor = [NSExpression
      mgl_expressionForConditional:[NSPredicate predicateWithFormat:@"fill == YES"]
  ...
  // source, identity function that sets any feature with an "opacity" attribute to use that value
  // and anything without to 1.0
  fillStyleLayer.fillOpacity =
      [NSExpression mgl_expressionForConditional:[NSPredicate predicateWithFormat:@"opacity != nil"]
...
- (void)addLatLonGrid {
  MLNComputedShapeSource *source = [[MLNComputedShapeSource alloc]
      initWithIdentifier:@"latlon"
...
- (NSString *)bestLanguageForUser {
  ...
  NSArray *supportedLanguages = @[
    @"ar", @"de", @"en", @"es", @"fr", @"ja", @"ko", @"pt", @"ru", @"zh", @"zh-Hans", @"zh-Hant"
  ];
  NSArray<NSString *> *preferredLanguages =
      [NSBundle preferredLocalizationsFromArray:supportedLanguages
...
- (void)continueWorldTourWithRemainingAnnotations:
    (NSMutableArray<MLNPointAnnotation *> *)annotations {
...
- (void)testQueryPointAnnotations {
  ...
  if ([visibleAnnotationCount integerValue] == 1) {
    message =
        [NSString stringWithFormat:@"There is %@ visible annotation.", visibleAnnotationCount];
  } else {
    message =
        [NSString stringWithFormat:@"There are %@ visible annotations.", visibleAnnotationCount];
  }

  UIAlertController *alertController =
      [UIAlertController alertControllerWithTitle:@"Visible Annotations"
                                          message:message
                                   preferredStyle:UIAlertControllerStyleAlert];
  [alertController addAction:[UIAlertAction actionWithTitle:@"Ok"
                                                      style:UIAlertActionStyleCancel
...
- (id<MLNAnnotation>)randomOffscreenPointAnnotation {
  NSPredicate *pointAnnotationPredicate =
  ...
  NSArray *annotations =
      [self.mapView.annotations filteredArrayUsingPredicate:pointAnnotationPredicate];
  ...
  NSArray *visibleAnnotations =
      [self.mapView.visibleAnnotations filteredArrayUsingPredicate:pointAnnotationPredicate];
...
- (void)addVisibleAreaPolyline {
  ...
  lineCoords[0] = [self.mapView convertPoint:CGPointMake(CGRectGetMinX(constrainedRect),
                                                         CGRectGetMinY(constrainedRect))
                        toCoordinateFromView:self.mapView];
  lineCoords[1] = [self.mapView convertPoint:CGPointMake(CGRectGetMaxX(constrainedRect),
                                                         CGRectGetMinY(constrainedRect))
                        toCoordinateFromView:self.mapView];
  lineCoords[2] = [self.mapView convertPoint:CGPointMake(CGRectGetMaxX(constrainedRect),
                                                         CGRectGetMaxY(constrainedRect))
                        toCoordinateFromView:self.mapView];
  lineCoords[3] = [self.mapView convertPoint:CGPointMake(CGRectGetMinX(constrainedRect),
                                                         CGRectGetMaxY(constrainedRect))
                        toCoordinateFromView:self.mapView];
  lineCoords[4] = lineCoords[0];

  MLNPolyline *line =
      [MLNPolyline polylineWithCoordinates:lineCoords
...
- (void)loadMissingIcon {
  ...
  NSURL *customStyleJSON = [[NSBundle mainBundle] URLForResource:@"missing_icon"
                                                   withExtension:@"json"];
...
- (void)flyToWithLatLngBoundsAndPadding {
  ...
  MLNMapCamera *cameraWithoutPadding =
      [self.mapView cameraThatFitsCoordinateBounds:MLNCoordinateBoundsMake(sw, ne)
                                       edgePadding:padding];
  ...
  __weak MBXViewController *weakSelf = self;
  [self.mapView flyToCamera:cameraWithoutPadding
                edgePadding:padding
               withDuration:5
          completionHandler:^{
            [weakSelf.mapView flyToCamera:cameraWithoutPadding
...
- (void)cycleTileLodMode {
  ...
  _tileLodMode = static_cast<MBXTileLodMode>((static_cast<int>(_tileLodMode) + 1) %
                                             static_cast<int>(MBXTileLodModeCount));
  ...
  if (!zoomSlider && _tileLodMode != MBXTileLodModeNoLod) {
    ...
    [zoomSlider addTarget:self
                   action:@selector(updateTileLodZoom:)
...
- (void)addAnnotations:(NSInteger)numAnnotations
      aroundCoordinate:(CLLocationCoordinate2D)coordinate
...
- (void)randomWorldTourInternal {
  ...
  dispatch_after(dispatch_time(DISPATCH_TIME_NOW, (int64_t)(duration * 0.5 * NSEC_PER_SEC)),
                 dispatch_get_main_queue(), ^{
  ...
  MLNMapCamera *camera =
      [MLNMapCamera cameraLookingAtCenterCoordinate:annotation.coordinate
                                           altitude:10000.0
...
- (void)toggleSecondMapView {
  if ([self.view viewWithTag:2] == nil) {
    MLNMapView *secondMapView = [[MLNMapView alloc]
        initWithFrame:CGRectMake(0, self.view.bounds.size.height / 2, self.view.bounds.size.width,
    ...
    for (NSLayoutConstraint *constraint in self.view.constraints) {
      if ((constraint.firstItem == self.mapView &&
           constraint.firstAttribute == NSLayoutAttributeBottom) ||
          (constraint.secondItem == self.mapView &&
...
- (IBAction)handleLongPress:(UILongPressGestureRecognizer *)longPress {
  if (longPress.state == UIGestureRecognizerStateBegan) {
    ...
    pin.coordinate = [self.mapView convertPoint:point toCoordinateFromView:self.mapView];
...
- (void)setStyles {
  ...
  /// PluginLayerTestStyle.json for an example of how the layer is defined
  NSURL *url = [[NSBundle mainBundle] URLForResource:@"PluginLayerTestStyle.json"
                                       withExtension:nil];
  if (url) {
...
- (void)mapView:(MLNMapView *)mapView
  ...
  NSLog(
      @"A new shader is being compiled - shaderID:%ld, backend type:%ld, program configuration:%@",
      id, backend, defines);
}
...
- (void)mapView:(MLNMapView *)mapView
    shaderDidCompile:(NSInteger)id
             backend:(NSInteger)backend
             defines:(NSString *)defines {
  NSLog(@"A shader has been compiled - shaderID:%ld, backend type:%ld, program configuration:%@",
        id, backend, defines);
}
...
- (void)mapView:(MLNMapView *)mapView
    glyphsWillLoad:(NSArray<NSString *> *)fontStack
             range:(NSRange)range {
  NSLog(@"Glyphs are being requested for the font stack %@, ranging from %ld to %ld", fontStack,
...
- (void)mapView:(MLNMapView *)mapView
    glyphsDidLoad:(NSArray<NSString *> *)fontStack
            range:(NSRange)range {
  NSLog(@"Glyphs have been loaded for the font stack %@, ranging from %ld to %ld", fontStack,
        range.location, range.location + range.length);
}
...
- (MLNAnnotationView *)mapView:(MLNMapView *)mapView
  ...
  if (annotation == mapView.userLocation) {
    if (_customUserLocationAnnnotationEnabled) {
      MBXUserLocationAnnotationView *annotationView =
          [[MBXUserLocationAnnotationView alloc] initWithFrame:CGRectZero];
      annotationView.frame = CGRectMake(0, 0, annotationView.intrinsicContentSize.width,
                                        annotationView.intrinsicContentSize.height);
  ...
  if ([annotation isKindOfClass:[MBXDroppedPinAnnotation class]] ||
      [annotation isKindOfClass:[MBXSpriteBackedAnnotation class]]) {
    return nil;
  }

  MBXAnnotationView *annotationView = (MBXAnnotationView *)[mapView
      dequeueReusableAnnotationViewWithIdentifier:MBXViewControllerAnnotationViewReuseIdentifer];
  if (!annotationView) {
    annotationView = [[MBXAnnotationView alloc]
        initWithReuseIdentifier:MBXViewControllerAnnotationViewReuseIdentifer];
...
- (MLNAnnotationImage *)mapView:(MLNMapView *__nonnull)mapView
             imageForAnnotation:(id<MLNAnnotation> __nonnull)annotation {
  if ([annotation isKindOfClass:[MBXDroppedPinAnnotation class]] ||
      [annotation isKindOfClass:[MBXCustomCalloutAnnotation class]]) {
    return nil;  // use default marker
  }

  NSAssert([annotation isKindOfClass:[MBXSpriteBackedAnnotation class]],
           @"Annotations should be sprite-backed.");
  ...
  MLNAnnotationImage *annotationImage =
      [mapView dequeueReusableAnnotationImageWithIdentifier:lastTwoCharacters];
  ...
  if (!annotationImage) {
    ...
    annotationImage = [MLNAnnotationImage annotationImageWithImage:image
                                                   reuseIdentifier:lastTwoCharacters];
...
- (UIImage *)imageWithText:(NSString *)text backgroundColor:(UIColor *)color {
  ...
  NSAttributedString *drawString = [[NSAttributedString alloc]
      initWithString:text
          attributes:@{
  ...
  CGRect stringRect =
      CGRectMake((rect.size.width - stringSize.width) / 2,
                 (rect.size.height - stringSize.height) / 2, stringSize.width, stringSize.height);
...
- (BOOL)mapView:(__unused MLNMapView *)mapView
    annotationCanShowCallout:(__unused id<MLNAnnotation>)annotation {
  return YES;
}
...
- (UIColor *)mapView:(__unused MLNMapView *)mapView
    strokeColorForShapeAnnotation:(MLNShape *)annotation {
  UIColor *color =
      [annotation isKindOfClass:[MLNPolyline class]] ? [UIColor greenColor] : [UIColor blackColor];
  return [color colorWithAlphaComponent:0.9];
}

- (UIColor *)mapView:(__unused MLNMapView *)mapView
    fillColorForPolygonAnnotation:(__unused MLNPolygon *)annotation {
  UIColor *color = annotation.pointCount > 3 ? [UIColor greenColor] : [UIColor redColor];
  return [color colorWithAlphaComponent:0.5];
}

- (void)mapView:(__unused MLNMapView *)mapView
...
- (nullable id<MLNCalloutView>)mapView:(__unused MLNMapView *)mapView
...
- (UIView *)mapView:(__unused MLNMapView *)mapView
    leftCalloutAccessoryViewForAnnotation:(__unused id<MLNAnnotation>)annotation {
...
- (UIView *)mapView:(__unused MLNMapView *)mapView
    rightCalloutAccessoryViewForAnnotation:(__unused id<MLNAnnotation>)annotation {
  UIButton *button = [UIButton buttonWithType:UIButtonTypeSystem];
  button.frame = CGRectZero;
...
- (void)mapView:(MLNMapView *)mapView tapOnCalloutForAnnotation:(id<MLNAnnotation>)annotation {
  ...
  point.coordinate = [self.mapView convertPoint:self.mapView.center
                           toCoordinateFromView:self.mapView];
}
...
- (BOOL)mapView:(MLNMapView *)mapView
  ...
  if (_shouldLimitCameraChanges) {
    ...
    BOOL intersects = MLNCoordinateInCoordinateBounds(newVisibleCoordinates.ne, colorado) &&
                      MLNCoordinateInCoordinateBounds(newVisibleCoordinates.sw, colorado);
...
- (void)updateHUD {
  ...
  if (self.reuseQueueStatsEnabled) {
    ...
    hudString = [NSString stringWithFormat:@"Visible: %ld  Queued: %ld",
                                           (unsigned long)self.mapView.visibleAnnotations.count,
                                           (unsigned long)queuedAnnotations];
  } else if (self.zoomLevelOrnamentEnabled) {
    hudString = [NSString stringWithFormat:@"%.f FPS (%.1fms) ∕ %.2f ∕ ↕\U0000FE0E%.f° ∕ %.f°",
                                           roundf(self.mapView.averageFrameRate),
                                           self.mapView.averageFrameTime, self.mapView.zoomLevel,
                                           self.mapView.camera.pitch, self.mapView.direction];
  }
...
- (NSArray<id<MLNFeature>> *)featuresInCoordinateBounds:(MLNCoordinateBounds)bounds
                                              zoomLevel:(NSUInteger)zoom {
  double gridSpacing;
  if (zoom >= 13) {
    gridSpacing = 0.01;
  } else if (zoom >= 11) {
  ...
  for (double y = ceil(bounds.ne.latitude / gridSpacing) * gridSpacing;
       y >= floor(bounds.sw.latitude / gridSpacing) * gridSpacing; y -= gridSpacing) {
  ...
  for (double x = floor(bounds.sw.longitude / gridSpacing) * gridSpacing;
       x <= ceil(bounds.ne.longitude / gridSpacing) * gridSpacing; x += gridSpacing) {
...
- (void)mapView:(nonnull MLNMapView *)mapView
    didChangeLocationManagerAuthorization:(nonnull id<MLNLocationManager>)manager {
  if (@available(iOS 14, *)) {
...
- (void)alertAccuracyChanges {
  UIAlertController *alert =
      [UIAlertController alertControllerWithTitle:@"MapLibre works best with your precise location."
                                          message:@"You'll get turn-by-turn directions."
                                   preferredStyle:UIAlertControllerStyleAlert];

  UIAlertAction *settingsAction =
      [UIAlertAction actionWithTitle:@"Turn On in Settings"
