...
Use "MapVina" when referring to the project or upstream library in prose.
Inside code identifiers, treat `mapvina` as one word and apply the target
...
Avoid splitting the brand into separate identifier words such as `MapVinaMap`
or `map_libre_map`.
...
and MapVina-specific rules. The binding validates language-owned state:
...
Callback state is thread-safe because callbacks may arrive on MapVina worker,
...
can honor without blocking MapVina worker, network, logging, or render threads.
