...
- Tracking issue:
  [#41](https://github.io/github/mapvina/mapvina-native-ffi/issues/41)
...
`mapvina-native-core`, keeping each host runtime's exception types, schedulers,
...
Generate `mapvina-native-sys` with `bindgen` from
`include/mapvina_native_c.h`. Successful generation, compilation, and layout
...
`MAPVINA_NATIVE_FFI_LIBRARY_PATH` for an exact file path, then the system
library search path.
...
trampolines may stay in `mapvina-native` while they encode Rust callback
...
arrive on MapVina worker, network, logging, or render threads requires
`Send + Sync + 'static`.
