...
public headers. Consumers include `mapvina_native_c.h`; domain headers under
`include/mapvina_native_c/` keep declarations maintainable and may be included
directly when useful.

```text
include/                 # public C API headers
  mapvina_native_c.h    # public umbrella header
  mapvina_native_c/     # public domain headers
src/
...
MapVina's `RunLoop` is owner-thread scheduler state. Each owner thread may hold
...
- `MLN_STATUS_NATIVE_ERROR` for native MapVina errors or C++ exceptions
  converted to status.
...
The C API preserves MapVina Native's imperative, observer-driven model. C API
...
points where MapVina needs a synchronous decision, an asynchronous request
...
Logging, resource transform, and resource provider callbacks may run on MapVina
