...
Keep `@cImport` declarations from `mapvina_native_c.h` private. Raw C handles,
...
materialization. Link to `mapvina-native-c` through Zig build configuration.
...
preserve MapVina value semantics: integer width, object member order, and
duplicate keys.
...
Callbacks may arrive on MapVina worker, network, logging, or render-related
...
Zig API against the real `mapvina-native-c` library and cover both native C API
