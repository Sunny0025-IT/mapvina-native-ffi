...
- Tracking issue:
  [#45](https://github.io/github/mapvina/mapvina-native-ffi/issues/45)
- [Foreign Function & Memory API](https://docs.oracle.com/en/java/javase/25/core/foreign-function-and-memory-api.html)
- [`jextract`](https://jdk.java.net/jextract/)
- [Java JNI conventions](/mapvina-native-ffi/development/bindings-java-jni/)

## Architecture

The binding layers public API over generated C declarations across concept
packages. The root package contains the process-global `Mapvina` entry point.
...
The `io.github.mapvina.nativeffi` module exports public concept packages. Internal
...
1. exact library file path from `io.github.mapvina.nativeffi.library.path`;
2. exact library file path from `MAPVINA_NATIVE_FFI_LIBRARY_PATH`;
3. `System.loadLibrary("mapvina-native-c")` through `java.library.path`.

## Public Types

Process-global operations live as static methods on `Mapvina`. Long-lived
...
`Status.check(…)` throws unchecked `MapvinaException` subclasses carrying
`MapvinaStatus`, the raw status code, and the copied diagnostic. The binding
...
Upcall stubs may run on MapVina worker, network, logging, or render-related
...
`NativePointer` values, and keeps the MapVina-owned texture alive until callers
